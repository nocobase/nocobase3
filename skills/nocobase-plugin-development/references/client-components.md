...
When a plugin needs the application's page structure or route overlays, copy the UI Library items that provide them and maintain the copy as plugin-owned code. Inside this repository, copy each file listed in `ui-library/registry/page/registry.json` from `ui-library/registry/page/<path>` to its `target` under the plugin, as `ui-library/USAGE.md` describes, rather than running `shadcn add`. These are source references, not runtime imports from the host App. Reuse an existing plugin copy before adding another; plugins that predate the library keep theirs in `client/components/`.

| Need                                        | Item               | Installs into                                  |
| ------------------------------------------- | ------------------ | ---------------------------------------------- |
| Page spacing, page heading and actions      | `page-ui`          | `client/extensions/nocobase-page-ui/`          |
| Route dialog, drawer or covering child page | `route-overlay-ui` | `client/extensions/nocobase-route-overlay-ui/` |

`page-ui` has no `@/` imports and compiles in a plugin as copied. `route-overlay-ui` imports the `button` and `dialog` primitives as `@/components/ui/<name>`; rewrite those to relative `.js` paths to the plugin's own primitives, generating missing ones with the workflow above. Copy the item whole rather than picking files, so both wrappers share the one Context its hook declares. Declare the packages each item lists in `dependencies` as peers. Keep these copies private unless an approved public export is required.

Merge `route-overlay-ui`'s `locales/en-US.ts` and `locales/zh-CN.ts` into the plugin's own `client/locales/` resources and register the lazy locale manifest in its Client declaration; see [internationalization](i18n.md). The close button reads `routeOverlay.close`, and the plugin supplies it in every language it supports rather than relying on the host App. Under the plugin's own route, the copied overlay inherits the plugin namespace; if it is intentionally exported for another owner to render, bind that namespace explicitly as described in the internationalization guide.
