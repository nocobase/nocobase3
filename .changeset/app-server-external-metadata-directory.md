...
`generateAppCollectionsArtifact()` treats `metadata.json` as the source on such a connection: regenerating only normalizes its formatting, and when the database no longer has a Collection the generated files are removed while `metadata.json` is kept and reported under `orphans`. The generator resolves each connection's directory through the same `resolveAppCollectionsDirectory()` the store default uses.
