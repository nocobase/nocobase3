...
Add reference pages, declared with `defineAppRoutes()` so every signed-in user reaches them and a production build ships them: an Examples group of eight complete business screens on mock data — a dashboard, orders, customers, a product form, an inbox, a survey, team settings and a schedule — and a Components group with one page per shadcn/ui primitive, both sharing the frame in `client/pages/reference/shared.tsx`. An example owns a folder holding its page beside the mock data that page reads, as `examples/orders/orders.tsx` beside `orders.data.ts`, so the screen and its records move together. Their wording is in both locales, and `tests/logic/locale-coverage.test.ts` fails on a key only one language has.
