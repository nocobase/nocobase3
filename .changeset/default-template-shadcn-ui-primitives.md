...
Add reference pages, declared with `defineAppRoutes()` so every signed-in user reaches them and a production build ships them: an Examples group of complete business screens on mock data and a Components group with one page per shadcn/ui primitive, both sharing the frame in `client/pages/reference/shared.tsx`. An example owns a folder holding its page beside the mock data that page reads, as `examples/orders/orders.tsx` beside `orders.data.ts`, so the screen and its records move together. The set ships page by page, so `client/routes.ts` declares one route together with its page and the routes whose page does not exist yet stay commented out there.
