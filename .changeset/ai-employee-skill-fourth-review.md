...
Three known gaps are now stated rather than left to be discovered after deployment: a built server does not read the application's `.env`, an application's `ai/skills` is not copied into its `dist/`, and nothing yet limits which employees a user can converse with, so the only boundary is each tool's own actor check. All three are open defects with fixes planned, and the Skill says so — an application cannot close them itself, but it should not be designed as though they were permanent either.
