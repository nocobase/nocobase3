...
Two known gaps are now stated rather than left to be discovered after deployment: a built server does not read the application's `.env`, and nothing yet limits which employees a user can converse with, so the only boundary is each tool's own actor check. Both are open defects with fixes planned, and the Skill says so — an application cannot close them itself, but it should not be designed as though they were permanent either. A third, an application's `ai/skills` never reaching its `dist/`, is fixed in `@nocobase/app-tools` in this same release, and the Skill describes the build carrying them instead.
...
`server-runs.md` also gains what an agent driven with nobody watching has to get right, and one correction it turned up. `invoke()` was documented as returning `message: null` for a turn suspended awaiting a tool decision; it does not — it rejects with a `GraphInterrupt`, passed through rather than wrapped, so a caller matching on `AgentServiceErrorCode` never sees it. The reliable arrangement is not to suspend at all: a background run activates only tools that run automatically, and a caller that resumes one is deciding on the user's behalf and should say so with an explicit decision rather than a blanket approval.
