...
Where an LLM key lives is now a choice the Skill puts to the user rather than one it makes for them, and the key never passes through the agent. It offers three places in order — a system environment variable referenced as `${NAME}`, which behaves the same in development and in a built server; the value written into `config.yml` once that file is confirmed ignored and untracked; and `.env` last, stated as working under `pnpm dev` only in this version. Before any of them the agent checks that the files involved are ignored and untracked, then hands the user a command with a placeholder to run in their own terminal, never asking for the key, writing it, or printing a file that holds it. The Skill also stops saying a configuration change needs no restart: the server reads its environment, `config.yml` and `.env` when it starts, and a new variable reaches it only from a terminal that has sourced it. A deployment is called out as configured separately — its own `config.yml` beside `dist/`, and the variable set where the service manager starts the process rather than in a login shell's profile.
