...
`CreateEmployeeOptions` takes that execution whole — `state` and `runtime` — instead of listing `sessionId`, `translate` and `getHeader` beside a logger the factory chose. `AIEmployeeAgentContextProviderOptions` stops restating six fields its `agentContext` already holds, deriving the session, actor, web-search flag, frontend tools, language and headers from it, so two records of one value can no longer come apart. It also takes `aiEmployeesManager` rather than a closure bound to its `resolveModel`, since the provider already holds the employee and already depends on four other managers directly.
