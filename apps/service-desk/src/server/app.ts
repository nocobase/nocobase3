...
import { createServiceDeskApiRoutes } from './routes.js';
...
export interface CreateAppOptions {
  ...
  serviceDeskRuntime?: ServiceDeskRuntime;
}
...
export function createApp(options: CreateAppOptions = {}): Hono {
  ...
  app.get('/healthz', async (context) => {
    const database = await options.serviceDeskRuntime?.databaseStatus();
    return context.json(
      {
  ...
  app.route(
    apiPath,
    options.serviceDeskRuntime
      ? createServiceDeskApiRoutes(options.serviceDeskRuntime)
      : new Hono(),
  );
