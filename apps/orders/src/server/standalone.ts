...
async function start(): Promise<void> {
  ...
  const appName = getEnvString(env, 'APP_NAME') ?? 'orders';
  const basePath = normalizeBasePath(
    getEnvString(env, 'APP_BASE_PATH') ?? `/${appName}`,
  );
  const host = getEnvString(env, 'APP_SERVER_HOST') ?? '127.0.0.1';
  const port = Number(getEnvString(env, 'APP_SERVER_PORT') ?? 13010);
  const dataDir =
    getEnvString(env, 'ORDERS_DATA_DIR') ?? path.join(packageRoot, 'data');
  ...
  const server = serve({ fetch: app.fetch, hostname: host, port }, () =>
    console.log(`Orders app: http://${host}:${port}${basePath}/`),
  );
  const close = (): void => {
