...
function AppSettingsEntry({ compact = false }: { compact?: boolean }) {
  return (
    <CanAccess roles={{ anyOf: ['crm-admin'] }}>
      <Button
        variant='outline'
        ...
        aria-label={compact ? 'App 设置' : undefined}
      >
        <Settings2 />
        {compact ? <span className='sr-only'>App 设置</span> : 'App 设置'}
      </Button>
    </CanAccess>
  );
}
