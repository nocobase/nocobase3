...
const resolveVitePort = (source: EnvironmentSource, portalPort: number) => {
  const configured = readValue(source, 'NOCOBASE_E2E_VITE_PORT');
  const port = configured ? Number(configured) : portalPort + 1;
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error('NOCOBASE_E2E_VITE_PORT must be a valid TCP port.');
  }
  if (port === portalPort) {
    throw new Error(
      'NOCOBASE_E2E_VITE_PORT must differ from NOCOBASE_E2E_PORT.',
    );
  }
  return port;
};
...
const resolveBrowserApiURL = (
  ...
  const configured =
    readValue(source, 'NOCOBASE_E2E_BROWSER_API_URL') ??
    readValue(source, 'NOCOBASE_API_URL');
  if (configured && !/^https?:\/\//i.test(configured)) {
    return `/${configured.replace(/^\/+|\/+$/g, '')}`;
  }
  return `${portalBase.replace(/\/$/, '')}/v2/api`;
};
