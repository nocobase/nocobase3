...
describe('Portal E2E environment', () => {
  ...
  it('keeps the browser API on the Portal origin and the upstream API server-only', () => {
    const environment = loadPortalE2EEnvironment({
      APP_BASE_PATH: '/crm',
    ...
    expect(environment.baseURL).toBe('http://127.0.0.1:4173/crm/');
    expect(environment.browserApiURL).toBe('/crm/v2/api');
    expect(environment.apiURL).toBe('http://127.0.0.1:13000/api');
  });

  it('rejects a Vite port that collides with the Portal server', () => {
    expect(() =>
      loadPortalE2EEnvironment({
        NOCOBASE_E2E_PORT: '4173',
        NOCOBASE_E2E_VITE_PORT: '4173',
      }),
    ).toThrow(/must differ/);
  });
