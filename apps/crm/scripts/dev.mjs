...
const appBasePath = String(
  nextEnv.APP_BASE_PATH || `/${nextEnv.APP_NAME || 'crm'}`,
)
  .trim()
  .replace(/^\/+|\/+$/g, '');
...
// Vite serves the client directly in development, so it cannot receive the
// runtime config injected by the standalone server. Keep zero-config startup
// aligned with production by exposing the same proxied API path to the SDK.
nextEnv.NOCOBASE_API_URL = proxyApiPath;
