...
export function ProvidedBy() {
  ...
  const page = pages.find(
    (page) =>
      page.lang === lang && page.frontmatter?.packageName === frontmatter?.pkg,
  );
  if (!page) {
    return null;
  }
    ...
    <span style={{ display: 'inline-flex', gap: '5px' }}>
      {frontmatter?.pkg && (
        <Badge type='info'>
          <Link
            target='_blank'
            href={transformHref(`/plugins/${frontmatter?.pkg}`, lang)}
          >
            {page?.frontmatter?.displayName as string}
          </Link>
        </Badge>
      )}
      {page?.frontmatter?.editionLevel >= 0 && (
        <Badge
          type={EditionLevelsTypes[page?.frontmatter?.editionLevel as number]}
        >
          <Link target='_blank' href={commercialLink}>
            {levels[page?.frontmatter?.editionLevel as number]}+
          </Link>
        </Badge>
      )}
    </span>
