...
OSS releases use the same App credentials with installation tokens scoped separately to `nocobase3`. Both sets of workflows create local Git commits with the verified `nocobase[bot]` account ID `179432756`. See [release identity and CI skipping](release-bot.md) for the OSS merge-commit policy and the distinction between native GitHub CI and Pro's webhook-forwarded checks.
