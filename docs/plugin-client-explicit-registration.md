...
| server providers / routes  | `server/plugins/resolve.ts` → `server/app.ts` |
...
**插件的 server 入口目前由 manifest 与文件约定共同解析**：`resolve.ts` 优先读取 `nocobase.plugin.server`，未声明时按 `server/provider.ts`、`server/routes/index.ts` 等候选路径探测。`package.json` 的 `exports` 里没有统一的 server 插件注册子路径。改成 App 源码显式 import 后，每个插件需要提供稳定的 server 注册 export，并且 `dev-plugin-watches.mjs` 和 `build.mjs` 需要改成从 `server/plugins.ts` 解析包名。
