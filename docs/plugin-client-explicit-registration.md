...
| #   | 议题                                                                                                                  | 建议                                |
| --- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| 1   | options 里的组件，类型上只接受 loader，还是同时接受组件值？（§7.5）                                                   | 两种都接受，文档和模板统一用 loader |
| 2   | 本期是否在模板里演示 `authentication({ loginPage })`？（§4，与 auth-ui 扩展冲突）                                     | 不演示，采用 §4 方案 A              |
| 3   | 是否本期一并删除 `nocobase.plugin.client` 字段和 resolve.ts 的 client 解析？（§9.2，涉及 config.test.ts 约 7 处断言） | 一并删除                            |
| 4   | `packages/authorization/skills/` 是否迁移到 `.agents/skills/`？该包是库而非插件。（§8.7）                             | 迁移目录以统一约定，同步范围不含库  |
| 5   | `create-plugin.mjs` 生成的 `version: 0.1.0` 与 AGENTS.md 要求的 `0.0.1` 不一致，是否一并修正？（§8.6）                | 一并修正                            |
