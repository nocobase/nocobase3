...
function transformMetaValue(value, context) {
  ...
  if (typeof value.link === 'string') {
    const normalized = normalizeMetaTarget(
      value.link,
      context.file,
      context.langRoot,
    );
...
function normalizeMetaTarget(target, file, langRoot) {
  ...
  if (base.endsWith('/')) {
    ...
    if (noSlashFile)
      return { type: 'extraSlash', to: `${baseNoSlash}${suffix}` };
  ...
  if (base.endsWith('/index')) {
    ...
    if (
      exact &&
      parentIndex &&
      path.resolve(exact) === path.resolve(parentIndex)
    ) {
  ...
  if (indexFile && !noSlashFile)
    return { type: 'missingSlash', to: `${baseNoSlash}/${suffix}` };
...
function summarizeChanges(changes, keys) {
  ...
  for (const key of keys)
    summary[key] = changes.filter((change) => change.type === key).length;
