...
// js-yaml 来自 pnpm install 装下来的 docs/node_modules（本包显式声明的 devDependency）。
...
function loadJsYaml() {
  ...
  console.error(
    '找不到 js-yaml。请在 NocoBase 主仓库根目录或 docs/ 目录下运行（确保已执行 pnpm install）。',
  );
  ...
  if (fm && Array.isArray(fm.features)) {
    out.features = fm.features.map((f) => ({
      items: Array.isArray(f?.items)
        ? f.items.map((it) => normalizeLink(it?.link))
        : [],
    }));
  }
  return out;
...
function diff(cn, lang) {
  ...
  if (cn.heroActions.length !== lang.heroActions.length) {
    issues.push(
      `hero.actions 数量不一致 (cn=${cn.heroActions.length}, lang=${lang.heroActions.length})`,
    );
  } else {
    for (let i = 0; i < cn.heroActions.length; i++) {
      if (cn.heroActions[i] !== lang.heroActions[i]) {
        issues.push(
          `hero.actions[${i}].link: cn=${cn.heroActions[i] ?? '∅'} lang=${lang.heroActions[i] ?? '∅'}`,
        );
      }
    }
  }
  ...
  if (cn.features.length !== lang.features.length) {
    issues.push(
      `features 数量不一致 (cn=${cn.features.length}, lang=${lang.features.length})`,
    );
    return issues;
  }
  ...
  for (let i = 0; i < cn.features.length; i++) {
    ...
    if (cnItems.length !== langItems.length) {
      issues.push(
        `features[${i}].items 数量不一致 (cn=${cnItems.length}, lang=${langItems.length})`,
      );
      continue;
    }
    for (let j = 0; j < cnItems.length; j++) {
      if (cnItems[j] !== langItems[j]) {
        issues.push(
          `features[${i}].items[${j}].link: cn=${cnItems[j] ?? '∅'} lang=${langItems[j] ?? '∅'}`,
        );
      }
    }
  }
