...
async function readChangedFiles(args) {
  ...
  if (process.stdin.isTTY) {
    ...
    console.error(
      "用法示例：gh pr view <pr> --json files --jq '.files[].path' | node check-i18n-coverage.mjs",
    );
    process.exit(2);
  }
  const chunks = [];
...
async function main() {
  ...
  for (const lang of TARGET_LANGS) {
    ...
    const missing = [...cnChanges]
      .filter((rel) => !langChanges.has(rel))
      .sort();
    if (missing.length === 0) {
      console.log(`[${lang}] OK`);
      continue;
    ...
    bad++;
    console.log(
      `[${lang}] STALE: ${missing.length} cn change(s) without ${lang} sync`,
    );
