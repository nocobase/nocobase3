...
邮件插件把个人操作和跨用户管理分开。应用接入组件或接口时，按实际职责授予权限：

| 权限                          | 范围                                           |
| ----------------------------- | ---------------------------------------------- |
| `page:mail.workspace/access`  | 当前用户自己的账户、邮件、附件、发送和同步记录 |
| `page:mail.admin/access`      | 跨用户账户总览和设置中的操作日志               |
| `page:mail.management/access` | 全用户邮件查询、详情、附件和批量操作           |

全用户邮件管理目前没有部门或组织范围，授予前先确认管理员职责。隐藏页面或按钮不能替代 API 权限校验；应用自定义接口也要认证调用者，并再次检查对应权限和账户所有权。
...
以下地址用于开发调试和参考组件接入方式。路径相对于应用挂载位置，比如挂载在 `/main` 时，邮件中心地址为 `/main/dev/mail/center`。
...
`/dev/mail/*` 是开发参考页面，不进入生产构建。生产应用需要自行提供账户管理、邮件入口和需要的日志链接；如果包含 OAuth 关联，还要处理授权完成后的返回页面。当前插件默认返回开发账户页，不能仅添加邮件工作区就认为生产账户关联已完成。
...
业务代码应通过 `mailServiceToken` 解析服务，使用 `MailService.sendMessage()`，或调用 `POST /api/mail/messages/send`。个人 API 校验登录、邮件工作区权限和账户所有权，服务端接入也应保留同样的访问边界。不要直接读写邮件插件的数据表，也不要绕过 Mail service 自行调用服务商接口。
