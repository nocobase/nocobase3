...
确认 Agent 的方案回答了本页列出的选型、事件身份、节点边界、输入与参数、副作用以及节点能力问题。节点的具体约束见[节点概览](./nodes/index.md)，数据库事务和 Run 模块边界见[Run 节点](./nodes/run.md)。如果 Agent 只展示流程图而没有解释这些决定，方案尚不足以进入实现。
