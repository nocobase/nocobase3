...
另外，聊天框默认选中的是所有已启用服务按 `sort` 排序后的第一个模型。服务的 `sort` 每次启动都会从配置重写，决定选中哪个服务；该服务 `enabledModels` 的第一项决定选中哪个模型。这个第一项在服务创建时来自 `config.yml`，之后只有打开这个开关才会继续由配置决定，否则以数据库里的现状为准。
