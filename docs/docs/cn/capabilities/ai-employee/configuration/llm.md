...
另外，聊天框默认选中的是所有已启用服务按 `sort` 排序后的第一个模型。服务的 `sort` 每次启动都会从配置重写，决定选中哪个服务；该服务 `enabledModels` 的第一项决定选中哪个模型。这个第一项在服务创建时来自 `config.yml`，之后只有打开这个开关才会继续由配置决定，否则以数据库里的现状为准。如果员工在设置页开启了自己的模型设置，这个员工的聊天只列出它允许的模型，按员工配置的顺序排列，默认选中其中第一个，服务端也只会用这些模型。
