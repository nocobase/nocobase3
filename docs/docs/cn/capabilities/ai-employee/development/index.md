---
title: '定义自己的 AI 员工'
description: '在 NocoBase 应用中定义并注册 AI Employee、Tool、Skill 和 MCP 服务。'
keywords: 'NocoBase,AI Employee,defineAIEmployee,defineTools,SKILL.md,defineMCP'
---

# 定义自己的 AI 员工

当内置员工的角色或能力不能覆盖业务需求时，可以在应用源码中定义自己的 AI 员工。通常来说，一个完整的员工会组合四类资源：Employee 保存身份和角色设定，Tool 执行业务操作，Skill 描述工作方法，MCP 把外部服务提供的 Tool 接入运行时。

这些资源都接入 AI 员工插件已经创建的 `AIManager`。不要在普通应用中调用 `createAIManager()`，也不要从 `@nocobase/app-plugin-ai-employee/server/agent/*` 这类私有路径导入实现。

## 注册顺序

应用使用 `AIResourceRegistrar` 组织资源，执行顺序固定为：

```text
Tool → MCP → Skill → AI Employee
```

这个顺序让 Employee 可以安全引用已经注册的 Skill 和 Tool。Employee 和 Tool 通过 TypeScript 静态 import 聚合；MCP 由 `MCPLoader` 读取 `ai/mcp`；Skill 由 `SkillsLoader` 读取 `SKILL.md`。
...
└── mcp/company-search.ts
```

## 聚合应用资源

先在 `server/ai/index.ts` 中聚合 Employee 和 Tool。MCP 目录通过 Registrar 的构造参数交给 Loader；应用根目录的 `ai/skills` 已由 AI 员工插件自动加载，不需要在这里重复添加。
...
最后把 Provider 加入 `server/providers/index.ts` 的 `serviceProviders` 数组，并确保它排在 AI Employee 插件 Provider 完成 `boot()` 之后。`AIResourceRegistrar` 是显式生命周期 API，插件当前没有接收自定义 Registrar 的装配选项，因此应用 Provider 必须主动调用它；从目录加载 MCP 后还要重建 Client。应用 Runtime 会统一驱动 `register → boot → start → ready → shutdown`，不要在模块顶层启动连接或执行注册。
...
- [注册 MCP 服务](./mcp.md) — 使用代码加载外部 MCP 服务
- [配置 MCP](../configuration/mcp.md) — 在 `config.yml` 中声明 MCP 连接
