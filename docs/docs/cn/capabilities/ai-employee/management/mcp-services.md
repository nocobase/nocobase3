...
默认权限按 MCP 服务端的 Tool 名推断：名称以 `get` 开头的 Tool 默认 Allow，其他默认 Ask。这只是根据名称的猜测，不代表 Tool 真的没有副作用。调整后的权限保存在数据库中，服务重启后保持不变。只能为已连上的服务列出的 Tool 设置权限；服务没连上时，它的 Tool 不会出现在这里，接口对这样的 Tool 名返回 404，不保存任何设置。在 `config.yml` 中删除或改名这个服务，会丢掉它的开关和 Tool 权限。
