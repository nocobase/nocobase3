...
项目的 Skills 已包含主题验证要求。Coding Agent 完成修改后，会按这些要求运行相关测试和检查，并在浏览器中验证实际页面，不需要你在提示词中重复交代。

验证会覆盖浅色、深色和跟随系统模式下的显示效果，文字和控件在窄屏中的布局，以及主题切换、刷新后恢复选择和删除后的回退行为。Coding Agent 会汇报验证结果；如果受运行环境限制无法完成某项检查，也应明确说明。
