...
`pnpm config:init --dialect` 提供以下八种选项，应用运行时具有对应的官方驱动加载入口。具体数据库版本与业务插件的兼容性仍需在目标环境验证。
...
主数据库在应用内选择，而不是在创建时：先安装驱动，再运行 `pnpm config:init --dialect <dialect>` 生成对应的连接配置。模板已依赖 `@nocobase/db-sqlite`，使用 SQLite 无需额外安装。非 SQLite 数据库仍需填写实际连接信息；配置成功不代表数据库连接已验证。以下先列出 SQLite、PostgreSQL 和 MySQL 的配置示例。
...
- **数据库驱动**：应用能使用哪些方言由它声明的驱动决定。`pnpm config:init` 不安装任何东西——驱动缺失时它会报出来且不写入配置——所以要先用 `pnpm add` 安装。已有项目改用其他数据库时，需确认目标驱动已安装并重新构建；官方驱动包名为 `@nocobase/db-<dialect>`，例如 `@nocobase/db-mssql`。
...
只有已声明的环境变量才会覆盖对应字段；不要按名称自行推断 `DB_HOST` 等变量一定可用。在应用目录（构建产物则在 `dist/`）执行 `pnpm config:env`，可以列出应用读取的全部环境变量、各自对应的配置路径，以及是否已设置。
...
| 检查项       | 验证方法                                                                                                              |
| ------------ | --------------------------------------------------------------------------------------------------------------------- |
| 整体配置     | 独立部署在目标机器的 `dist/` 中运行 `pnpm config:check`；它按服务启动的方式加载配置，并连接除 SQLite 以外的所有数据库 |
