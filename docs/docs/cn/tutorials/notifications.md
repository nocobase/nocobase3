...
审批通过后，申请人应该马上收到一条消息。本教程沿用前面审批流程的场景，使用 NocoBase 通知服务发送站内信，并说明如何按需增加邮件、避免重复发送以及检查投递结果。

:::tip 前置阅读

- [通知](../capabilities/notification.md)——了解渠道（Channel）、服务商（Provider）、投递（Delivery）和通知状态
- [工作流快速开始](../capabilities/workflow/quick-start.md)——了解如何让应用智能体（Agent）开发和验证工作流
...
- 通知日志中可以看到每个渠道、服务商和投递尝试（Attempt）的状态
- 如果配置了邮件，还可以把同一业务结果发送到邮箱
...
站内信使用用户 ID 作为接收人，不需要 SMTP 或 Webhook 凭据。先用站内信完成闭环，再增加邮件或即时通讯，排查范围会更小。

## 第二步：让应用智能体（Agent）接入审批结果

不要先指定某个不存在的“通知节点”。默认工作流提供 `Run` 节点，应用智能体应先检查当前应用的工作流技能（Workflow Skill）和服务（Service）契约，再在审批通过的业务路径中调用通知服务。

可以把下面的需求交给应用智能体：

> 请先读取当前应用的通知技能（Skill）、工作流定义、审批服务（Service）和用户数据结构。
>
> 审批状态变为 approved 后，给申请人发送一条站内信。消息标题为“审批已通过”，正文包含订单号，点击后进入订单详情页。调用必须通过已注册的 `notificationServiceToken`，不能直接调用服务商（Provider）。
>
...
这段代码中的字段需要替换成当前应用真实的审批字段。不要直接照抄 `approval.requesterId`、`approval.orderNo` 或 `approval.orderId`，先让智能体检查实际数据结构（Schema）和服务（Service）。

## 第三步：按需增加邮件

如果申请人还需要收到邮件，在 `config.yml` 中增加邮件渠道（`email`）。SMTP 是最直接的选择：
...
如果使用邮件渠道（`email`）发送给 `{ type: 'user', id: '...' }`，应用还需要配置用户 ID 到邮箱地址的解析器（`resolver`）。也可以在业务代码中直接使用邮箱地址：
...
如果站内信和邮件都要发送，可以在一次调用中写 `channels: ['in-app', 'email']`。不过只有当所有接收人都能被两个渠道解析时，才适合共用一次调用；否则应拆成两次发送，并分别使用稳定的幂等键。

## 第四步：检查代码并运行迁移

要求应用智能体报告实际执行的命令和结果。至少检查：
...
先选择「站内信」，接收用户 ID 可以填写自己的 ID，消息标题和正文使用测试内容即可。点击「发送」后，页面会将测试消息交给正式的通知管理器（Notification Manager），并在日志中创建通知、投递和投递尝试记录。

如果没有可选择的通知方式，检查下面几项：

- 对应的渠道插件已经注册
- `config.yml` 中的渠道和服务商都设置了 `enabled: true`
- 服务商的 `type` 与插件定义一致
- 修改配置后应用已经重启
- 当前账号拥有 `page:notification.logs` 的 `access` 权限；提交测试消息还需要 `notification:test` 的 `send` 权限

测试消息会产生真实的站内信或外部消息。发送邮件或即时通讯消息前，先确认收件地址和目标群组。
...
`completed` 表示所有服务商都接受了提交。它不保证邮件已经到达收件箱，也不代表用户已经阅读。`unknown` 表示提交结果无法确认，先查看服务商后台和目标收件箱，再决定是否重试。
...
`title`、`body` 和 `actionUrl` 是所有内置渠道都能理解的通用字段。只有邮件需要额外字段时，才使用 `channelOverrides`：
...
正文中不要放密码、Token、Webhook 地址或完整的敏感数据。日志会隐藏消息正文和收件人快照，业务定位信息应通过 `source.type` 和 `source.referenceId` 记录。
...
先检查队列工作进程（Worker）是否启动、通知相关迁移是否完成，以及服务端是否记录了队列派发错误。不要因为页面没有立即更新就重复触发审批；重复执行应继续复用同一个幂等键。
...
`unknown` 代表服务商可能已经收到消息。先根据投递尝试和服务商后台确认外部结果，再决定是否调用 `retryDelivery()`。没有服务商幂等保证时，重试意味着可能重复发送。

## 小结

| 目标         | 做法                                                 |
| ------------ | ---------------------------------------------------- |
| 发站内信     | 启用站内信渠道（`in-app`），接收人使用用户 ID        |
| 发邮件       | 启用邮件渠道（`email`），配置 SMTP 或 Resend         |
| 发飞书、钉钉 | 启用即时通讯渠道（`im`），配置群机器人 Webhook       |
| 避免重复发送 | 为同一业务事件复用稳定的 `idempotencyKey`            |
| 检查投递结果 | 查看「设置 / 通知 / 通知日志」中的投递和投递尝试记录 |
