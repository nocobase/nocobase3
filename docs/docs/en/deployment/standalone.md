...
Open `https://apps.example.com/crm/` and sign in with the username and password from the [initial administrator configuration](./configuration#configure-the-initial-administrator). With the default configuration unchanged, the username is `nocobase` and the password `admin123`; the email `admin@nocobase.com` also signs in. When the default password is in use, change it right after the first sign-in and before opening access. An existing application keeps its original account, and changing the initialization settings does not reset its password; customized initialization tasks follow the project's own rules.
