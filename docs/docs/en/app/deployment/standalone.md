...
**Running with pm2.** When systemd is not an option, [pm2](https://pm2.keymetrics.io/) can supervise the process. The application project ships an `ecosystem.config.js` that has pm2 run `node ./dist/server/standalone.js` directly (`interpreter: 'none'`) and sets `NODE_ENV=production`. Do not point `script` at `standalone.js` instead: pm2 would load it through its own wrapper, `standalone.js` would find it is not the main module and never start the server, and pm2 would still report the process as online. The file is not part of the deployment archive: copy it from the project into the deployment root, beside `dist`. Pass the remaining runtime settings as environment variables or add them to the file's `env` block, then run the following from the deployment root:
