...
For an unmodified Hub on a Node.js server without Docker, `@nocobase/hub-installer` installs, upgrades and rolls back the Hub. It generates the Hub from the published template, builds it on the server, keeps only the deployment archive under `releases/<version>/hub`, and runs it under pm2. `config.yml`, `hub.env` and `storage/` sit beside the releases and are kept across upgrades. It needs Linux or macOS (WSL on Windows), Node.js 24 or later, pnpm 11 or later, `tar`, and pm2 installed globally with `npm install -g pm2`. NocoBase 3 packages are published to `https://npm.nocobase.ai`, not the public npm, so name the registry:
