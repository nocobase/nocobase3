...
声明 Route；多个页面共享 React Context 时使用 `client/react-providers/`，不要把 React Provider
...
- `client:inspect` 读取静态 Client Route 和 React Provider declarations，但不实例化 ServiceProvider、不执行 lifecycle、不加载页面 `componentLoader`、不渲染 React Provider，也不验证 Server 安全。
