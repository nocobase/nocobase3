...
`verify` defaults to the Default template and SQLite. It creates an application, installs dependencies, validates JSON output, synchronizes Skills, and verifies dev, build, and production start. `--timeout 420` sets the readiness timeout in seconds for each server. `--workdir /absolute/empty/directory` selects an empty output directory outside the repository; otherwise a temporary directory is allocated. Applications and logs are retained on success or failure, while application processes are stopped. The existing shared smoke test exercises all three templates' dev/build/start scripts.
