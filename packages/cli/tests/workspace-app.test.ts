...
async function createWorkspace(): Promise<string> {
  ...
  created.push(root);
  await writePackage(
    path.join(root, 'packages', 'app-template-default'),
    '@nocobase/app-template-default',
  );
  await writePackage(
    path.join(root, 'examples', 'custom-app'),
    '@example/custom-app',
  );
  return root;
}
...
describe('workspace app resolution', () => {
  it('defaults to app-template-default', async () => {
    ...
    await expect(resolveAppRoot({ workspaceRoot })).resolves.toBe(
      path.join(workspaceRoot, 'packages', 'app-template-default'),
    );
  });
  ...
  it('accepts a directory name or full package name', async () => {
    ...
    await expect(
      ...
      root: path.join(workspaceRoot, 'examples', 'custom-app'),
    });
    await expect(
