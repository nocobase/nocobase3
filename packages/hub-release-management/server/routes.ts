...
export function createReleaseManagementRoutes(
  ...
  routes.post('/apps/:appId/releases', async (context) => {
    ...
    const actor = await (options.authorizeArtifactUpload ?? options.authorize)(
      request,
    );
    const idempotencyKey = requireIdempotencyKey(request);
    const appId = context.req.param('appId');
    const releaseId = request.headers.get('x-nocobase-release-id')?.trim();
    if (!isSafeSegment(appId) || !releaseId || !isSafeSegment(releaseId)) {
      throw new ReleaseManagementError(
        'appId and X-NocoBase-Release-Id must be safe path segments',
        { status: 400, code: 'INVALID_RELEASE_TARGET' },
      );
    }
    ...
    if (contentType !== 'application/gzip') {
      throw new ReleaseManagementError(
        'Release upload content type must be application/gzip',
        { status: 415, code: 'INVALID_RELEASE_CONTENT_TYPE' },
      );
    }
    if (!request.body) {
      throw new ReleaseManagementError('Release archive is required', {
        status: 400,
        code: 'RELEASE_ARCHIVE_REQUIRED',
      });
    }
    const archiveBytes = optionalContentLength(request);
    const release = await options.service.uploadRelease({
      appId,
      releaseId,
    ...
    if (!options.approvalRequired) {
      const deployment = await options.service.execute({
        appId,
        releaseId,
        kind: 'deploy',
        idempotencyKey,
      ...
      return context.json({ release, deployment });
    }
    ...
    return context.json({ release, approval }, 202);
  });
  ...
  routes.post('/apps/:appId/rollbacks', async (context) => {
    ...
    if (!options.rollbackEnabled) {
      throw new ReleaseManagementError('Rollback is not enabled', {
        status: 404,
...
function optionalContentLength(request: Request): number | undefined {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0) {
    throw new ReleaseManagementError('Content-Length must be positive', {
      status: 400,
