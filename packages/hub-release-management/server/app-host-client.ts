...
export class AppHostClient {
  ...
  async uploadRelease(
    ...
    if (archiveBytes !== undefined) {
      headers.set('content-length', String(archiveBytes));
    }
    const payload = await this.send<{ release: AppHostReleaseUploadResult }>(
      `/__apps/${encodeURIComponent(appId)}/releases`,
      {
        method: 'POST',
        headers,
        body: archive,
        // Node.js requires this for streaming fetch request bodies. It is not
