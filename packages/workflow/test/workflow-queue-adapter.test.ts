...
async function countPendingJobs(database: DatabaseManager): Promise<number> {
  const rows = await database.query().selectFrom(QUEUE_TABLE).selectAll().where('status', '=', 'pending').execute();
  return rows.length;
}
...
describe('workflow queue adapter', () => {
  ...
  it('carries a task through the database driver and back into dispatch', async () => {
    ...
    const adapter = track(createWorkflowQueueAdapter({
      queue: queueManager,
      dispatch: async (task) => {
    ...
    await adapter.publish({ executionId: 7, nodeRunId: 42, rerun: { nodeKey: 'check', overwrite: true } });
    ...
    expect(dispatched).toEqual([
      { executionId: 7, nodeRunId: 42, rerun: { nodeKey: 'check', overwrite: true } },
    ]);
  ...
  it('survives a restart because the task stays in the database', async () => {
    queueManager = createQueueManager(databaseQueueConfig(), { database });
    const publisher = track(createWorkflowQueueAdapter({
      queue: queueManager,
    ...
    const dispatched: WorkflowQueueTask[] = [];
    queueManager = createQueueManager(databaseQueueConfig(), { database });
    const consumer = track(createWorkflowQueueAdapter({
      queue: queueManager,
      dispatch: async (task) => {
        dispatched.push(task);
      },
  ...
  it('keeps the delayed delivery capability available for later node types', async () => {
    ...
    await publishWorkflowTask(queueManager, { executionId: 3 }, { delay: '30s' });
    const rows = await database.query().selectFrom(QUEUE_TABLE).selectAll().execute();
  ...
  it('refuses a second adapter on the same queue name', async () => {
    queueManager = createQueueManager(databaseQueueConfig(), { database });
    const adapter = track(createWorkflowQueueAdapter({ queue: queueManager, dispatch: async () => undefined }));

    expect(() => createWorkflowQueueAdapter({ queue: queueManager!, dispatch: async () => undefined }))
      .toThrow(`A workflow queue adapter is already listening on queue "${WORKFLOW_QUEUE_NAME}"`);

    await adapter.stop();
    // After stop() the queue name is free again.
    const replacement = track(createWorkflowQueueAdapter({ queue: queueManager, dispatch: async () => undefined }));
  ...
  it('routes by queue name so two adapters can coexist', async () => {
    ...
    queueManager = createQueueManager({
      ...databaseQueueConfig(),
      worker: { queues: [WORKFLOW_QUEUE_NAME, 'workflow-other'], concurrency: 1, idleDelay: '10ms' },
    }, { database });

    const first = track(createWorkflowQueueAdapter({
      queue: queueManager,
      dispatch: async (task) => {
        defaultTasks.push(task);
      },
    }));
    const second = track(createWorkflowQueueAdapter({
      queue: queueManager,
