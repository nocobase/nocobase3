...
describe('timeout reaper', () => {
  ...
  async function insertRun(input: RunInput): Promise<WorkflowId> {
    await database.query().insertInto(WORKFLOW_COLLECTIONS.runs).values({
      ...
      startedAt: input.status == null ? null : new Date(Date.now() - 3_600_000).toISOString(),
  ...
  it('is idempotent, so a second sweep does not reclaim the same run twice', async () => {
    await insertRun({ eventKey: 'once', status: EXECUTION_STATUS.STARTED, expiresAt: minutesFromNow(-1) });
    const reaper = createTimeoutReaper({ database });
  ...
  it('deduplicates concurrent sweeps', async () => {
    await insertRun({ eventKey: 'concurrent', status: EXECUTION_STATUS.STARTED, expiresAt: minutesFromNow(-1) });
    const reaper = createTimeoutReaper({ database });
  ...
  it('sweeps on its interval and stops cleanly without leaking a timer', async () => {
    await insertRun({ eventKey: 'scheduled', status: EXECUTION_STATUS.STARTED, expiresAt: minutesFromNow(-1) });
