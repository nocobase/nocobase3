...
describe('workflow collections', () => {
  ...
  it('preserves workflow indexes and relation metadata without physical foreign keys', async () => {
    ...
    expect(workflowIndexes).toEqual(expect.arrayContaining([
    ...
    await expect(db.raw('PRAGMA foreign_key_list(workflow_node_runs)')).resolves.toEqual([]);
    ...
    await expect(db('workflow_node_runs').where({ workflow_run_id: runId })).resolves.toHaveLength(1);

    await expect(Promise.all(
      workflowCollectionSchemas.map(({ name }) => metadataStore.getCollection(name)),
    ...
    const dryRun = await createWorkflowCollections(database.builder(), { dryRun: true });
    const workflows = dryRun[0].operations[0] as {
      definition: CollectionDefinition;
    };
    expect(workflows.definition.fields).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: 'nodes', type: 'hasMany', target: WORKFLOW_COLLECTIONS.nodes }),
      expect.objectContaining({ name: 'revisions', constraints: false }),
      expect.objectContaining({ name: 'versionStats', target: WORKFLOW_COLLECTIONS.versionStats }),
    ]));
  });
});
