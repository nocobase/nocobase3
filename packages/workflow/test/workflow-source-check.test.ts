...
async function sourceFile(body: string): Promise<string> {
  ...
  await fs.writeFile(file, `import { defineWorkflow, node } from ${JSON.stringify(dslPath)};\n${body}`);
  return file;
}
...
describe('workflow check', () => {
  it.each([
    ['wrong config value type', `export default defineWorkflow({ title: 'x', nodes: [node.run({ key: 'run', config: { script: 1 } })] });`, 'TS2322'],
    ['wrong branch name', `export default defineWorkflow({ title: 'x', nodes: [node.condition({ key: 'c', config: {} }).branch({ maybe: [] })] });`, 'TS2353'],
    ['branch on non-branch node', `export default defineWorkflow({ title: 'x', nodes: [node.run({ key: 'run', config: { script: './x.ts' } }).branch({})] });`, 'TS2339'],
    ['unaggregated node type', `export default defineWorkflow({ title: 'x', nodes: [node.approval({ key: 'a', config: {} })] });`, 'TS2339'],
    ['unknown condition operator', `export default defineWorkflow({ title: 'x', nodes: [node.condition({ key: 'c', config: { expression: { execute: ['process.exit()'] } } })] });`, 'TS2353'],
  ...
  it('runs typecheck, bundle, evaluate, schema, semantic, and compile without writing a database', async () => {
    const file = await sourceFile(`export default defineWorkflow({ title: 'x', nodes: [node.condition({ key: 'c', config: {} }).branch({ yes: [node.run({ key: 'inside', config: { script: './inside.ts' } })] }), node.run({ key: 'after', config: { script: './after.ts' } })] });`);
  ...
  it('reports a JSON Logic validation error at the expression path', async () => {
    const file = await sourceFile(`export default defineWorkflow({ title: 'x', nodes: [node.condition({ key: 'c', config: { expression: { var: 'context.constructor.secret' } } })] });`);
  ...
  it('checks a trigger-free definition and exposes the same context schema in AST and flat IR', async () => {
    const file = await sourceFile(`export default defineWorkflow({ title: 'x', contextSchema: { type: 'object', required: ['active'], properties: { active: { type: 'boolean' } }, additionalProperties: false }, nodes: [] });`);
  ...
  it('reports unsupported context schema capabilities with a structured path', async () => {
    const file = await sourceFile(`export default defineWorkflow({ title: 'x', contextSchema: { type: 'object', properties: { id: { type: 'string', format: 'uuid' } } }, nodes: [] });`);
    await expect(checkWorkflowPackage(file)).rejects.toMatchObject({ issues: [expect.objectContaining({ code: 'INVALID_CONTEXT_SCHEMA', astPath: 'workflow.contextSchema.properties.id.format', contractType: 'ContextSchema' })] });
  });
  ...
  it.each([
    ...
    expect(() => validateWorkflowFlatIrTopology({ title: 'bad', contextSchema: { type: 'object' }, start: 'start', nodes })).toThrow(message);
  });
});
