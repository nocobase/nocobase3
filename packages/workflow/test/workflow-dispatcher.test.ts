...
describe('workflow dispatcher and processor', () => {
  ...
  it('dispatches and processes nodes connected only by upstreamKey and downstreamKey', async () => {
    ...
    const workflowRow = await database.query()
  ...
  it('recovers an undispatched execution through the same key-based processor path', async () => {
    ...
    await expect(database.query()
      ...
      .value('status')).resolves.toBe(EXECUTION_STATUS.RESOLVED);
  });
});
...
describe('Processor public API', () => {
  ...
  async function createProcessor(
    ...
    const processor = new Processor({ database, workflow: definition, execution, instructions });
  ...
  beforeEach(async () => {
    ...
    workflow = await createTestWorkflow(database, {
      key: 'processor-api',
      nodes: [
        { key: 'head', type: 'echo', config: { value: 'head' }, downstreamKey: 'gate' },
        { key: 'gate', type: 'condition', config: {}, upstreamKey: 'head', downstreamKey: 'after' },
        { key: 'b1', type: 'echo', config: { value: 'b1' }, upstreamKey: 'gate', branchKey: 'yes', downstreamKey: 'b2' },
        { key: 'b2', type: 'echo', config: { value: 'b2' }, upstreamKey: 'b1' },
        { key: 'other', type: 'echo', config: { value: 'other' }, upstreamKey: 'gate', branchKey: 'no' },
        { key: 'after', type: 'echo', config: { value: 'after' }, upstreamKey: 'gate' },
      ],
    });
  });
  ...
  it('exposes a query adapter bound to the configured connection', async () => {
    ...
    await expect(processor.query
      .selectFrom(WORKFLOW_COLLECTIONS.runs)
      .where('id', '=', runId)
      .value('eventKey')).resolves.toBe('processor-1');
  ...
  it('saves a nodeRun, remembers it, and finds it again while it is pending', async () => {
    ...
    await expect(processor.findPendingNodeRun(pending.id)).resolves.toMatchObject({
      id: pending.id,
      result: { waiting: true },
      meta: { note: 'meta' },
      log: 'saved',
  ...
  it('navigates the branch topology from any node', async () => {
    ...
    expect(processor.getBranches(gate!).map((node) => node.key)).toEqual(['other', 'b1']);
    ...
    const gateNodeRun = await processor.saveNodeRun({ nodeId: gate!.id, nodeKey: 'gate', status: NODE_RUN_STATUS.RESOLVED, result: true });
    const b1NodeRun = await processor.saveNodeRun({ nodeId: b1!.id, nodeKey: 'b1', status: NODE_RUN_STATUS.RESOLVED, result: 'b1' });
    const b2NodeRun = await processor.saveNodeRun({ nodeId: b2!.id, nodeKey: 'b2', status: NODE_RUN_STATUS.RESOLVED, result: 'b2' });

    expect(processor.findBranchParentNodeRun(b2NodeRun, gate!)).toEqual(gateNodeRun);
  ...
  it('builds a scope and resolves variables against it', async () => {
    ...
    expect(processor.getParsedValue('amount is {{$context.amount}}', b1)).toBe('amount is 7');
    expect(processor.getParsedValue({ nested: '{{$extra.value}}' }, b1, {
  ...
  it('resolves a workflow that has no nodes at all', async () => {
    const empty = await createTestWorkflow(database, { key: 'empty', nodes: [] });
  ...
  it('refuses a workflow that does not have exactly one head node', async () => {
    ...
    expect(run.output).toMatchObject({ message: expect.stringContaining('found 2') });
    await expect(listNodeRuns(database, runId)).resolves.toEqual([]);
  });

  it('refuses to re-run an unknown node or an execution that is not started', async () => {
    const { processor } = await createProcessor();
    await expect(processor.rerun({ nodeKey: 'missing' })).rejects.toThrow(/was not found/);
