...
export async function createTestWorkflow(
  ...
  if (input.nodes.length) {
    await database.query().insertInto(WORKFLOW_COLLECTIONS.nodes).values(input.nodes.map((node) => ({
      workflowId,
      key: node.key,
      title: node.key,
      type: node.type,
...
export async function findRun(database: DatabaseManager, eventKey: string): Promise<Row> {
...
export async function insertTestRun(database: DatabaseManager, input: TestRunInput): Promise<WorkflowId> {
...
export async function readRun(database: DatabaseManager, id: WorkflowId): Promise<WorkflowRun> {
...
export async function jobTrace(database: DatabaseManager, runId: WorkflowId): Promise<string[]> {
