...
import { compileToFlatIr, createNodeExpression, defineWorkflow, restoreFromFlatIr } from '../src/workflow-source/core.js';
import type { NodeExpression, WorkflowNodeSourceInput } from '../src/index.js';

class LeafInstruction {
  static readonly type: 'leaf' = 'leaf';
  static readonly branches: null = null;
  static create(source: WorkflowNodeSourceInput<{ value: string }>): NodeExpression {
...
class BranchInstruction {
  ...
  static create(source: WorkflowNodeSourceInput<{ enabled: boolean }>): NodeExpression<'yes' | 'no'> {
    return createNodeExpression(BranchInstruction, source);
  }
}
const leaf: typeof LeafInstruction.create = LeafInstruction.create.bind(LeafInstruction);
const branch: typeof BranchInstruction.create = BranchInstruction.create.bind(BranchInstruction);
...
describe('workflow source authoring', () => {
  it('builds an immutable canonical AST and omits empty branches', () => {
    ...
    const ast = defineWorkflow({ title: 'Example', nodes: [expression, leaf({ key: 'after', config: { value: 'common' } })] });
  ...
  it('compiles tree blocks to deterministic flat IR without mutating the AST', () => {
    const ast = defineWorkflow({ title: 'Example', nodes: [branch({ key: 'choose', config: { enabled: true } }).branch({ yes: [leaf({ key: 'inside', config: { value: 'x' } })] }), leaf({ key: 'after', config: { value: 'y' } })] });
  ...
  it.each([new Date(), new Map(), { bad: () => true }])('rejects non-JSON config %#', (bad) => {
    expect(() => defineWorkflow({ title: 'Bad', nodes: [leaf({ key: 'bad', config: { value: bad } as never })] })).toThrow(/JSON-compatible|class instances/);
  });

  it('rejects duplicate keys while compiling', () => {
    const ast = defineWorkflow({ title: 'Bad', nodes: [leaf({ key: 'same', config: { value: 'a' } }), leaf({ key: 'same', config: { value: 'b' } })] });
  ...
  it('preserves node options through AST and flat IR', () => {
    const ast = defineWorkflow({
      title: 'Timeout slot',
      nodes: [leaf({ key: 'limited', config: { value: 'x' }, options: { timeout: 30_000 } })],
    });
  ...
  it('preserves an explicit result schema through AST and flat IR restoration', () => {
    const ast = defineWorkflow({ title: 'Result', nodes: [leaf({ key: 'value', result: { type: 'string', description: 'A value' }, config: { value: 'x' } })] });
    expect(ast.nodes[0].result).toEqual({ type: 'string', description: 'A value' });
    expect(restoreFromFlatIr(compileToFlatIr(ast))).toEqual(ast);
  });

  it.each([0, -1, Number.POSITIVE_INFINITY])('rejects invalid node timeout %s', (timeout) => {
    expect(() => defineWorkflow({
      title: 'Invalid timeout',
      nodes: [leaf({ key: 'limited', config: { value: 'x' }, options: { timeout } })],
