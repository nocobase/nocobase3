...
import { computeWorkflowArtifactDigest, type WorkflowArtifactDefinition, type WorkflowArtifactDigestFile } from './artifact-builder.js';
...
export interface WorkflowArtifactStore {
  ...
  readWorkflow(workflowKey: string, digest: string): Promise<WorkflowArtifactDefinition>;
}

export interface LocalWorkflowArtifactStoreOptions { storeRoot: string; }

export class WorkflowArtifactIntegrityError extends Error {
  constructor(message: string) { super(message); this.name = 'WorkflowArtifactIntegrityError'; }
}

function validatePart(value: string, label: string): void {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(value)) throw new WorkflowArtifactIntegrityError(`Invalid ${label} "${value}"`);
}
...
export class LocalWorkflowArtifactStore implements WorkflowArtifactStore {
  ...
  async commit(workflowKey: string, digest: string, source: string): Promise<void> {
    await this.verifyDirectory(source, workflowKey, digest);
    const destination = this.artifactPath(workflowKey, digest);
    if (await exists(destination)) { await this.verifyDirectory(destination, workflowKey, digest); return; }
  ...
  async readWorkflow(workflowKey: string, digest: string): Promise<WorkflowArtifactDefinition> {
    const directory = await this.materialize(workflowKey, digest);
    return JSON.parse(await fs.readFile(path.join(directory, 'workflow.json'), 'utf8')) as WorkflowArtifactDefinition;
  }

  private artifactPath(workflowKey: string, digest: string): string {
    validatePart(workflowKey, 'workflow key');
    if (!/^[a-f0-9]{64}$/.test(digest)) throw new WorkflowArtifactIntegrityError(`Invalid artifact digest "${digest}"`);
    return path.join(this.options.storeRoot, 'workflows', workflowKey, digest);
  }

  private async verifyDirectory(directory: string, workflowKey: string, digest: string): Promise<void> {
    ...
    catch (error) { throw new WorkflowArtifactIntegrityError(`Artifact ${workflowKey}/${digest} workflow.json cannot be read: ${error instanceof Error ? error.message : String(error)}`); }
    if (workflow.formatVersion !== 1 || workflow.key !== workflowKey) throw new WorkflowArtifactIntegrityError(`Artifact ${workflowKey}/${digest} workflow.json is invalid`);
    ...
    const visit = async (current: string, relative: string): Promise<void> => {
      for (const entry of await fs.readdir(current, { withFileTypes: true })) {
        ...
        else if (entry.isFile()) files.push({ path: next, content: await fs.readFile(target) });
      }
    };
    await visit(directory, '');
    if (computeWorkflowArtifactDigest(files) !== digest) throw new WorkflowArtifactIntegrityError(`Artifact ${workflowKey}/${digest} bytes do not match its content address`);
  }
}

export function createLocalWorkflowArtifactStore(disk: FsDriveDiskConfig, namespaceRoot?: string): WorkflowArtifactStore {
  if (disk.driver !== 'fs' || disk.visibility !== 'private') throw new Error('Workflow artifact disk must be a private fs/local disk');
  return new LocalWorkflowArtifactStore({ storeRoot: namespaceRoot ?? disk.location });
}
