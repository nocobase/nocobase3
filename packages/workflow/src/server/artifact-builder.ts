...
import type { WorkflowServerEntryManifest } from './run-entry-builder.js';
...
export interface WorkflowPackageBuildOptions {
  instructions: Map<string, import('./types.js').WorkflowInstructionClass>;
...
function normalizeJson(value: import('./types.js').JsonValue): import('./types.js').JsonValue {
...
export function canonicalWorkflowJson(value: WorkflowArtifactDefinition): string {
  return `${JSON.stringify(normalizeJson(value as object as import('./types.js').JsonObject), null, 2)}\n`;
}

export function buildWorkflowArtifact(input: WorkflowArtifactBuildInput): WorkflowArtifactBuildResult {
  const runEntries = Object.fromEntries(
    Object.values(input.serverEntries ?? {})
      ...
      .map((entry) => [entry.source.replaceAll('\\', '/').replace(/^\.\//, ''), entry.output]),
  );
  const workflow: WorkflowArtifactDefinition = {
    formatVersion: 1,
    key: input.scanned.key,
    ...input.flatIr,
    ...(Object.keys(runEntries).length === 0 ? {} : { server: { run: runEntries } }),
  };
  ...
  return {
    digest: computeWorkflowArtifactDigest([...files].map(([filePath, content]) => ({ path: filePath, content }))),
    workflow,
    files,
  };
}
...
export async function buildWorkflowPackage(
  ...
  const [{ scanWorkflowPackage }, { parseWorkflowSource }, { validateWorkflowSourceAst }, { compileWorkflowSource }, { WorkflowSourceCheckError }, { buildWorkflowServerEntries }] = await Promise.all([
  ...
  const issues = validateWorkflowSourceAst(parsed.ast, filePath, { instructions: options.instructions });
  if (issues.length > 0) throw new WorkflowSourceCheckError(issues);
  const flatIr = compileWorkflowSource(parsed.ast, filePath, { instructions: options.instructions });
  const server = await buildWorkflowServerEntries(scanned, flatIr, {
    ...(options.bareImportAllowlist === undefined ? {} : { bareImportAllowlist: options.bareImportAllowlist }),
  });
...
export async function writeWorkflowArtifact(result: WorkflowArtifactBuildResult, distRoot: string): Promise<string> {
  const keyRoot = path.join(distRoot, result.workflow.key);
  await fs.mkdir(distRoot, { recursive: true });
  const temporaryKeyRoot = await fs.mkdtemp(path.join(distRoot, `.${result.workflow.key}-tmp-`));
...
export function computeWorkflowArtifactDigest(files: readonly WorkflowArtifactDigestFile[]): string {
  ...
  for (const file of [...files].sort(comparePaths)) {
    ...
    hash.update(normalizedPath).update('\0').update(String(content.byteLength)).update('\0').update(content).update('\0');
  }
