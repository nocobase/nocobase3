...
import type { WorkflowQueue, WorkflowQueueTask } from './types.js';

export const WORKFLOW_QUEUE_NAME = 'workflow';
export const WORKFLOW_TASK_JOB_NAME = 'nocobase.workflow.task';

export type WorkflowTaskDispatch = (task: WorkflowQueueTask) => Promise<unknown>;
...
export class WorkflowTaskJob extends Job<WorkflowQueueTask> {
  ...
  async execute(): Promise<void> {
    ...
    if (!dispatch) {
      throw new Error(`No workflow queue adapter is listening on queue "${this.context.queue}"`);
    }
 ...
 */
export function createWorkflowQueueAdapter(options: WorkflowQueueAdapterOptions): WorkflowQueueAdapter {
  ...
  if (existing && existing !== options.dispatch) {
    throw new Error(`A workflow queue adapter is already listening on queue "${queueName}"`);
  }
