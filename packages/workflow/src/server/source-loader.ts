...
export interface WorkflowSourceLoadResult { created: number; replaced: number; }
...
interface LoadedSource { key: string; hash: string; filePath: string; ir: WorkflowFlatIr; }
...
export default class WorkflowSourceLoader {
  ...
  get defaultRootPath(): string {
    return this.options.defaultRootPath ?? path.join(process.env.STORAGE_PATH ?? process.cwd(), 'workflows');
  }

  async load(rootPath: string = this.defaultRootPath): Promise<WorkflowSourceLoadResult> {
    if (this.loading) return this.loading;
    const loading = this.performLoad(rootPath);
    this.loading = loading;
    try { return await loading; } finally { if (this.loading === loading) this.loading = undefined; }
  }

  async activate(workflowId: WorkflowId): Promise<void> {
    await this.options.database.transaction(async (connection) => activateWorkflowSource(connection.query, workflowId), this.options.connectionName);
    await this.options.onWorkflowUpdated?.(workflowId);
  }

  private async performLoad(rootPath: string): Promise<WorkflowSourceLoadResult> {
    ...
    const result = await this.options.database.transaction(async (connection) => {
      ...
      for (const source of sources) {
        const materialized = await materializeWorkflowSource(source, connection.query);
        if (materialized.action === 'created') {
          created += 1;
          updated.push(materialized.workflowId);
          if (this.options.autoActivate === true) await activateWorkflowSource(connection.query, materialized.workflowId);
        }
      }
      return { created, replaced: 0 };
    }, this.options.connectionName);
    for (const workflowId of updated) await this.options.onWorkflowUpdated?.(workflowId);
    return result;
  }

  private async readSources(rootPath: string): Promise<LoadedSource[]> {
    let directories;
    try { directories = await fs.readdir(rootPath, { withFileTypes: true }); }
