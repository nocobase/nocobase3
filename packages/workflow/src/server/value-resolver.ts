function readPath(source: unknown, path: string): unknown {
  const parts = path.replace(/^ctx\./, '').split('.').filter(Boolean);
  let current = source;
...
export function resolveWorkflowValue(value: unknown, scope: Record<string, unknown>): unknown {
  if (Array.isArray(value)) {
    return value.map((item) => resolveWorkflowValue(item, scope));
  }
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, resolveWorkflowValue(item, scope)]),
    );
  }
