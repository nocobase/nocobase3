export type WorkflowSourcePhase = 'typecheck' | 'bundle' | 'evaluate' | 'schema' | 'semantic' | 'compile' | 'publish';
...
export function formatWorkflowSourceIssue(issue: WorkflowSourceIssue): string {
  const subject = issue.nodeKey ? `Node "${issue.nodeKey}" (${issue.contractType})` : issue.contractType;
