...
export default class Processor {
  ...
  abortExecution(reason?: string): void {
    ...
    if (!this.abortSignal.aborted) {
      this.abortController.abort(new Error(reason === EXECUTION_REASON.TIMEOUT
  ...
  createBackgroundAbortHandle(): BackgroundAbortHandle {
    ...
    if (this.abortSignal.aborted) {
      ...
      this.abortSignal.addEventListener('abort', sourceListener, { once: true });
    }
    ...
    if (remaining != null) {
      if (remaining <= 0) {
        ...
        timeoutGuard = setTimeout(() => abort(new Error('Workflow execution timed out')), remaining);
      }
    }
    ...
    return {
      ...
      throwIfAborted: () => {
        if (controller.signal.aborted) {
          throw controller.signal.reason ?? new Error('Workflow execution was aborted');
        }
      },
    };
  }

  async findPendingNodeRun(nodeRunId: WorkflowId): Promise<WorkflowNodeRun | null> {
  ...
  async start(): Promise<WorkflowRun> {
    ...
    try {
      ...
      if (heads.length !== 1) {
        this.logger.warn(`Expected one head node, found ${heads.length}`, { workflowId: this.workflow.id });
  ...
  async resume(nodeRun: WorkflowNodeRun): Promise<WorkflowRun> {
    ...
    try {
      ...
      if (!node) {
        throw new Error(`Node "${nodeRun.nodeKey}" was not found in workflow "${this.workflow.key}"`);
      }
  ...
  async rerun(options: ProcessorRerunOptions = {}): Promise<WorkflowRun> {
    ...
    try {
      ...
      if ((options.nodeKey != null || options.nodeId != null) && !targetNodeRun) {
        throw new Error(`Node run of node "${node.key}" was not found in execution "${this.execution.id}"`);
      }
      this.rerunContext = { overwrite: options.overwrite === true, targetNodeRun };
      const input = node.upstreamKey ? this.nodeRunsMapByNodeKey[node.upstreamKey] : { result: this.execution.context };
      if (node.upstreamKey && !input) {
        throw new Error(`Upstream node run of node "${node.key}" was not found`);
      }
  ...
  async exit(status?: number | true, output: unknown = this.lastSavedNodeRun?.result ?? null): Promise<null> {
    this.leaveRunningState();
    if (status === true) {
      return null;
    }
  ...
  async saveNodeRun(
    ...
    if (overwrite) {
      ...
      const inserted = await this.query
        ...
        .executeTakeFirst<Row>();
      if (!inserted) {
        throw new Error(`Failed to reload inserted node run for node "${payload.nodeKey}"`);
      }
      nodeRun = hydrateNodeRun({ ...inserted, id: inserted.id ?? insert.insertId });
    }
  ...
  getBranches(node: WorkflowNode): WorkflowNode[] {
    return this.nodes
      .filter((candidate) => candidate.upstreamKey === node.key && candidate.branchKey != null)
      .sort((left, right) => {
  ...
  findBranchStartNode(node: WorkflowNode, parent?: WorkflowNode): WorkflowNode | null {
    for (let current: WorkflowNode | undefined = node; current; current = current.upstream) {
  ...
  findBranchParentNodeRun(_nodeRun: WorkflowNodeRun, node: WorkflowNode): WorkflowNodeRun | null {
  ...
  findBranchLastNodeRun(node: WorkflowNode): WorkflowNodeRun | null {
    ...
    for (let current: WorkflowNode | undefined = this.findBranchEndNode(node); current && current !== node.upstream; current = current.upstream) {
      const nodeRun = this.nodeRunsMapByNodeKey[current.key];
      if (nodeRun) {
        nodeRuns.push(nodeRun);
      }
    }
    nodeRuns.sort((left, right) => String(left.id).localeCompare(String(right.id), 'en', { numeric: true }));
    return nodeRuns.at(-1) ?? null;
  }

  getScope(sourceNode?: WorkflowNode | WorkflowId, includeSelfScope: boolean = false): Record<string, unknown> {
    const node = typeof sourceNode === 'object'
      ? sourceNode
      : sourceNode == null
        ? undefined
        : this.nodesById.get(String(sourceNode)) ?? this.nodesMap.get(String(sourceNode));
    const scopes: Record<string, unknown> = {};
    for (
      let current = includeSelfScope && node ? node : this.findBranchParentNode(node);
      current;
  ...
  getParsedValue(
    ...
    options: { additionalScope?: Record<string, unknown>; includeSelfScope?: boolean } = {},
  ): unknown {
  ...
  private makeNodes(nodes: WorkflowNode[]): void {
    ...
    for (const node of nodes) {
      if (node.upstreamKey != null) {
        ...
        if (!node.upstream) {
          throw new Error(`Upstream node "${node.upstreamKey}" was not found for node "${node.key}"`);
        }
      }
      if (node.downstreamKey != null) {
        node.downstream = this.nodesMap.get(node.downstreamKey);
        if (!node.downstream) {
          throw new Error(`Downstream node "${node.downstreamKey}" was not found for node "${node.key}"`);
        }
      }
    }
  }
  ...
  private getRerunNode(options: ProcessorRerunOptions): WorkflowNode {
    if (options.nodeKey != null) {
      ...
      throw new Error(`Node "${options.nodeKey}" was not found in workflow "${this.workflow.key}"`);
    }
    if (options.nodeId != null) {
      ...
      throw new Error(`Node "${options.nodeId}" was not found in workflow "${this.workflow.key}"`);
    }
    const node = this.nodes.find((candidate) => candidate.upstreamKey == null);
    if (!node) {
      throw new Error(`Head node was not found in workflow "${this.workflow.key}"`);
    }
    return node;
  }
  ...
  private enterRunningState(): void {
    ...
    this.timeoutGuard = setTimeout(() => this.abortExecution(EXECUTION_REASON.TIMEOUT), remaining);
  }
