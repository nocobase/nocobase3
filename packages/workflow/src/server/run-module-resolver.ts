...
function assertSafeSourcePath(sourcePath: string): void {
  if (sourcePath.includes('\0')) {
    throw new WorkflowRunModuleError('Run script path must not contain a NUL character');
  }
  if (/^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(sourcePath)) {
    throw new WorkflowRunModuleError(`Run script path "${sourcePath}" must not be a URL`);
  }
  if (path.isAbsolute(sourcePath) || /^[a-zA-Z]:[\\/]/.test(sourcePath)) {
    throw new WorkflowRunModuleError(`Run script path "${sourcePath}" must not be absolute`);
  }
  ...
  if (sourcePath.split(/[\\/]/).includes('..')) {
    throw new WorkflowRunModuleError(`Run script path "${sourcePath}" must not contain ".."`);
  }
}
...
async function realpathOrThrow(target: string, sourcePath: string): Promise<string> {
  try {
    return await fs.realpath(target);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT' && target.endsWith('.ts')) {
      try {
        return await fs.realpath(`${target.slice(0, -3)}.js`);
      } catch (compiledError) {
        if ((compiledError as NodeJS.ErrnoException).code !== 'ENOENT') throw compiledError;
      }
    }
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new WorkflowRunModuleError(`Run script "${sourcePath}" was not found`);
    }
    throw error;
  }
}

function readRunExport(loaded: Record<string, unknown>, sourcePath: string): WorkflowRunFunction {
  const exported = loaded.run;
  if (typeof exported !== 'function') {
    throw new WorkflowRunModuleError(`Run script "${sourcePath}" must export a function named "run"`);
  }
  return exported as WorkflowRunFunction;
}

/** Loads a run module from the immutable source revision bound to a workflow hash. */
export function createSourceDirResolver(options: SourceDirResolverOptions): WorkflowRunModuleResolver {
  ...
  return {
    async resolve(request: WorkflowRunModuleRequest): Promise<WorkflowRunModule> {
      ...
      if (!isInside(root, target)) {
        throw new WorkflowRunModuleError(`Run script "${request.sourcePath}" escapes the source root`);
      }
      ...
      const loaded: Record<string, unknown> = await import(pathToFileURL(realTarget).href);
