...
export function hydrateWorkflow(row: Row, nodes: WorkflowNode[] = []): WorkflowDefinition {
  return {
    id: asId(row.id),
    key: String(row.key),
...
export async function loadWorkflow(query: QueryAdapter, id: WorkflowId): Promise<WorkflowDefinition | null> {
...
export async function loadRun(query: QueryAdapter, id: WorkflowId): Promise<WorkflowRun | null> {
  const row = await query
...
export async function loadNodeRun(query: QueryAdapter, id: WorkflowId): Promise<WorkflowNodeRun | null> {
  const row = await query
