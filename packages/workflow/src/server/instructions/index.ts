...
export const INSTRUCTION_TYPES: { readonly condition: 'condition'; readonly run: 'run' } = {
  condition: 'condition',
  run: 'run',
};
...
export const coreInstructions: ReadonlyMap<string, WorkflowInstructionClass> = new Map<string, WorkflowInstructionClass>([
  [ConditionInstruction.type, ConditionInstruction],
  [RunInstruction.type, RunInstruction],
]);

export { ConditionInstruction, CONDITION_BRANCH_KEYS, validateConditionConfig } from './condition.js';
