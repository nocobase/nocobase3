...
export interface MaterializedWorkflowSource { key: string; hash: string; filePath: string; ir: WorkflowFlatIr; }
export interface WorkflowSourceMaterializeResult { action: 'created' | 'unchanged'; workflowId: WorkflowId; }
...
export async function materializeWorkflowSource(loaded: MaterializedWorkflowSource, query: QueryAdapter): Promise<WorkflowSourceMaterializeResult> {
  const revisions = await query.selectFrom(WORKFLOW_COLLECTIONS.workflows).selectAll().where('key', '=', loaded.key).orderBy('id', 'desc').execute<Row>();
...
export async function activateWorkflowSource(query: QueryAdapter, workflowId: WorkflowId): Promise<void> {
  const selected = await query.selectFrom(WORKFLOW_COLLECTIONS.workflows).selectAll().where('id', '=', workflowId).executeTakeFirstOrThrow<Row>();
  const previous = await query.selectFrom(WORKFLOW_COLLECTIONS.workflows).selectAll().where('key', '=', String(selected.key)).where('current', '=', true).executeTakeFirst<Row>();
  const inheritedEnabled = previous ? Boolean(previous.enabled) : false;
  if (previous && previous.id !== selected.id) {
    await query.updateTable(WORKFLOW_COLLECTIONS.workflows).set({ current: null }).where('id', '=', previous.id).execute();
  }
  await query.updateTable(WORKFLOW_COLLECTIONS.workflows).set({ current: true, enabled: inheritedEnabled }).where('id', '=', workflowId).execute();
}
