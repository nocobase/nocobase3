...
import type { WorkflowNodeSourceContract, WorkflowSourceContracts } from './source-validator.js';
...
export const coreWorkflowSourceContracts: WorkflowSourceContracts = {
  nodes: new Map<string, WorkflowNodeSourceContract>([['condition', ConditionInstruction], ['run', RunInstruction]]),
};
...
export async function checkWorkflowPackage(packagePath: string, options: WorkflowSourceCheckOptions = {}): Promise<WorkflowSourceCheckResult> {
  ...
  return { file, ast: parsed.ast, ir: compileWorkflowSource(parsed.ast, file, contracts) };
}
