...
function diagnosticIssue(filePath: string, diagnostic: ts.Diagnostic): WorkflowSourceIssue {
  ...
  return {
    ...
    ...(start === undefined ? {} : { line: start.line + 1, column: start.character + 1 }),
    astPath: 'workflow',
    nodeKey: 'workflow',
...
export function typecheckWorkflowSource(filePath: string): WorkflowSourceIssue[] {
  ...
  return ts.getPreEmitDiagnostics(program).map((diagnostic) => diagnosticIssue(filePath, diagnostic));
}
...
function bundleIssue(filePath: string, message: Message): WorkflowSourceIssue {
  return {
    ...
    ...(message.location === null ? {} : { line: message.location.line, column: message.location.column + 1 }),
    astPath: 'workflow',
    nodeKey: 'workflow',
...
function assertSerializableDefinition(value: unknown, location: string, ancestors: Set<object> = new Set<object>()): void {
  if (value === null || typeof value === 'boolean' || typeof value === 'string') return;
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new TypeError(`${location} contains a non-finite number`);
    return;
  }
  if (typeof value !== 'object') throw new TypeError(`${location} contains a non-JSON ${typeof value} value`);
  ...
  if (Object.getOwnPropertySymbols(value).length) throw new TypeError(`${location} contains a symbol-keyed value`);
  ancestors.add(value);
  try {
    for (const [key, item] of Object.entries(value)) assertSerializableDefinition(item, `${location}.${key}`, ancestors);
  } finally {
    ancestors.delete(value);
  }
}

export async function parseWorkflowSource(filePath: string, runTypecheck: boolean = true): Promise<ParsedWorkflowSource> {
  if (runTypecheck) {
  ...
  try {
    ...
    const errors = typeof error === 'object' && error !== null && 'errors' in error
      ...
      : [];
    throw new WorkflowSourceCheckError(errors.length
      ? errors.map((message) => bundleIssue(filePath, message))
      : [{ phase: 'bundle', code: 'BUNDLE_FAILED', message: error instanceof Error ? error.message : String(error), file: filePath, nodeKey: 'workflow', astPath: 'workflow', contractType: 'WorkflowSourceAst' }]);
  }
  ...
  try {
    ...
    new vm.Script(bundle, { filename: filePath }).runInContext(context, { timeout: 5_000 });
    if (!isWorkflowSourceAst(module.exports.default)) {
      throw new WorkflowSourceCheckError([{
        phase: 'evaluate', code: 'INVALID_DEFAULT_EXPORT',
        message: 'workflow.ts must default-export the value returned by defineWorkflow()',
        file: filePath, nodeKey: 'workflow', astPath: 'workflow', contractType: 'WorkflowSourceAst',
      }]);
    }
    ...
    const ast = JSON.parse(JSON.stringify(module.exports.default)) as WorkflowSourceAst;
    return { ast, bundle };
  } catch (error) {
    if (error instanceof WorkflowSourceCheckError) throw error;
    throw new WorkflowSourceCheckError([{
      phase: 'evaluate', code: 'EVALUATION_FAILED', message: error instanceof Error ? error.message : String(error),
      file: filePath, nodeKey: 'workflow', astPath: 'workflow', contractType: 'WorkflowSourceAst',
    }]);
  }
}
