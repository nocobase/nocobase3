...
describe('createFileRoute', () => {
  ...
  it('enforces the scoped max-file count before upload', async () => {
  ...
  it('deletes object then record and returns 204', async () => {
    ...
    expect(calls).toEqual(['object', 'record']);
    expect(authorize).toHaveBeenCalledWith(
      expect.anything(),
      'delete',
      records[0],
  ...
  it('maps unavailable infrastructure to a stable 503', async () => {
    vi.mocked(store.list).mockRejectedValueOnce(new FilesUnavailableError());
    const response = await createApp().request(MOUNT_PATH);
...
function uploadBody(
  ...
  const boundary = 'nocobase-files-test-boundary';
