...
describe('@nocobase/app-plugin-notification client', () => {
  it('contributes notification logs and locale resources through the settings centre', async () => {
    ...
    expect(registration.locales).toBeTypeOf('function');
    await expect(registration.locales?.()).resolves.toMatchObject({
      default: {
