...
const currentUserVariableRegExp =
  /{{\s*(?:(ctx)\.)?(currentUser|\$user)(?:\.([^}]+))?\s*}}/g;
...
const toPlainObject = (value: unknown): unknown => value;
  ...
  return Array.from(
    new Set(
      paths
        ...
        .filter((append) => append && !Reflect.has(currentUser || {}, append)),
    ),
  );
};
...
async function getCurrentUser(ctx?: Context, template?: unknown) {
  ...
  const appends = getCurrentUserAppends(
    getCurrentUserReferencePaths(template),
    currentUser,
  );
  if (!appends.length) {
    return toPlainObject(currentUser);
...
export const normalizeMCPOptions = (options: MCPOptions): MCPOptions => {
  const normalized: MCPOptions = {
    ...
    useUserContext:
      options.transport === 'stdio' ? false : options.useUserContext === true,
  };
...
export async function renderMCPOptions(
  ...
  const currentUser = options.useUserContext
    ? await getCurrentUser(ctx, options)
    : undefined;
  const request = options.useUserContext
    ? getRequestVariables(ctx)
    : emptyRequestVariables;
  const variables = {
    $env: { ...process.env },
    currentUser,
    $user: currentUser,
