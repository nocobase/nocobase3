...
import type { MCPOptions, MCPRuntime } from './types.js';
import { renderMCPOptions } from './mcp-options-renderer.js';
...
export class UserContextMCPClientManager {
  ...
  async getToolsMap(
    ...
    try {
      ...
      this.options.runtime.logger?.warn?.(
        { error },
        'fail to get user-bound mcp tools',
      );
      return {};
    }
  }
    ...
    for (const [key, entry] of this.cache.entries()) {
      if (entry.expiresAt <= now) {
        ...
        this.closeEntry(entry).catch((error) => {
          this.options.runtime.logger?.warn?.(
            { error },
            'fail to close expired user-bound mcp client',
          );
        });
      }
    }
  ...
  private async evictOversized(): Promise<void> {
  ...
  private async closeEntry(entry: CacheEntry): Promise<void> {
    try {
      ...
      this.options.runtime.logger?.warn?.(
        { error },
        'fail to close user-bound mcp client',
      );
    }
  }
}
