...
export class AnthropicProvider extends LLMProvider {
  ...
  createModel() {
    ...
    const hasTemperature =
      sanitizedModelOptions.temperature !== undefined &&
      sanitizedModelOptions.temperature !== null;
    const hasTopP =
      (sanitizedModelOptions.topP !== undefined &&
        sanitizedModelOptions.topP !== null) ||
      (sanitizedModelOptions.top_p !== undefined &&
        sanitizedModelOptions.top_p !== null);
  ...
  private setMaxTokens(options: Record<string, any> = {}) {
    ...
    options.maxTokens = Object.entries(MAX_TOKENS_PRESET).find(([key]) =>
      options.model.startsWith(key),
    )?.[1];
  }
}
