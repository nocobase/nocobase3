...
describe('DeepSeek protocol contract', () => {
  it('freezes the official model protocol and web-search capability matrix', () => {
    ...
    expect(
      DEEPSEEK_MODEL_CAPABILITIES['deepseek-v4-flash'].supportsWebSearch,
    ).toBe(true);
    expect(DEEPSEEK_MODEL_CAPABILITIES['deepseek-v4-pro'].protocol).toBe(
      'responses',
    );
    expect(
      DEEPSEEK_MODEL_CAPABILITIES['deepseek-v4-pro'].supportsWebSearch,
    ).toBe(true);
    expect(DEEPSEEK_MODEL_CAPABILITIES['deepseek-chat'].protocol).toBe(
      'chat-completions',
    );
    expect(deepseekProviderOptions.webSearchModels).toEqual(
      deepSeekThinkingModeFixture.responsesModels,
    );
  });
  ...
  it.each([
    ...
    [
      'deepseek-v4-flash',
      'off',
      deepSeekThinkingModeFixture.responsesThinkingDisabled,
    ],
    ...
    [
      'deepseek-v4-flash',
      'high',
      deepSeekThinkingModeFixture.responsesThinkingEnabled,
    ],
    ...
    'maps %s reasoning mode %s to the official wire format',
    (model, mode, expected) => {
  ...
  it('does not inject DeepSeek-only fields into custom compatible models', () => {
    const config = resolveDeepSeekReasoningConfig(
      'private-deepseek-compatible',
      { mode: 'high' },
    );
  ...
  it('rejects unsupported reasoning switches for fixed-mode legacy models', () => {
    expect(() =>
      resolveDeepSeekReasoningConfig('deepseek-reasoner', { mode: 'off' }),
    ).toThrow(/does not support disabling reasoning/);
    expect(() =>
      resolveDeepSeekReasoningConfig('deepseek-chat', { mode: 'low' }),
  ...
  it('normalizes the final Chat Completions request body for legacy models', () => {
    ...
    normalizeDeepSeekChatRequest(
      request,
      resolveDeepSeekReasoningConfig('deepseek-reasoner', { mode: 'high' }),
    );
...
describe('DeepSeek final client routing', () => {
  ...
  it('converts native DeepSeek Responses reasoning and web-search stream events through LangChain', async () => {
    ...
    expect(
      chunks
        .map((chunk) => provider.parseReasoningContent(chunk))
        .filter(Boolean),
    ).toEqual([{ status: 'streaming', content: 'reasoning delta' }]);
    expect(
      chunks.flatMap((chunk) => provider.parseWebSearchAction(chunk)),
    ).toEqual([{ type: 'search', query: 'DeepSeek V4' }]);
    expect(
      chunks
        .map((chunk) => provider.parseResponseChunk(chunk.content))
        .filter(Boolean),
    ).toEqual(['answer']);
  });
...
describe('DeepSeek Responses parsing and persistence', () => {
  ...
  it('stores raw Responses reasoning and normalized display text without flattening the AI message', () => {
    ...
    expect(
      provider.prepareStoredAssistantAdditionalKwargs(
  ...
  it('restores persisted reasoning and citations from Responses metadata', () => {
    ...
    expect(parsed.content.reasoning).toEqual({
      status: 'stop',
      content: 'persisted reasoning',
    });
    expect(parsed.content.reference).toEqual([
      { title: 'DeepSeek docs', url: 'https://example.com' },
    ]);
  });
