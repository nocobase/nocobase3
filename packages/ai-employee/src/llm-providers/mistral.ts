...
const MISTRAL_REASONING_MODELS = new Set([
  'mistral-small-latest',
  'mistral-medium-3-5',
]);
const MISTRAL_REASONING_EFFORTS = new Set<ReasoningMode>([
  'off',
  'low',
  'medium',
  'high',
...
type ReasoningChatMistralAIFields = ConstructorParameters<
  typeof ChatMistralAI
>[0] & {
  [MISTRAL_REASONING_MODE_KEY]?: ReasoningMode;
};
...
function extractTextChunks(content: unknown): string {
  ...
  return content
    .filter(
      (chunk): chunk is MistralContentChunk =>
...
function extractThinkingChunks(content: unknown): string {
  ...
  return content
    ...
    .filter(
      (chunk) => chunk.type === 'thinking' && Array.isArray(chunk.thinking),
    )
    .map((chunk) => extractTextChunks(chunk.thinking))
    .join('');
}

export const injectMistralReasoningEffort: MistralBeforeRequestHook = async (
  request,
) => {
  ...
  if (
    ...
    !MISTRAL_REASONING_MODELS.has(body.model)
  ) {
...
class ReasoningChatMistralAI extends ChatMistralAI {
  ...
  constructor(fields: ReasoningChatMistralAIFields) {
    const { [MISTRAL_REASONING_MODE_KEY]: reasoningMode, ...modelFields } =
      fields;
    super(modelFields);
    this.reasoningMode = reasoningMode;
  }

  invocationParams(options?: MistralCallOptions): any {
...
export class MistralProvider extends LLMProvider {
  ...
  createModel() {
    ...
    const { responseFormat, structuredOutput, ...modelOptions } =
      this.modelOptions || {};
    ...
    const beforeRequestHooks = Array.isArray(modelOptions.beforeRequestHooks)
      ? ([
          ...
          ...modelOptions.beforeRequestHooks,
        ] as MistralBeforeRequestHook[])
      : [injectMistralReasoningEffort];
  ...
  async listModels(): Promise<{
    ...
    try {
      ...
      const errorMsg =
        ...
        (typeof data === 'object' && typeof data.detail === 'object'
          ? data.detail.message
          : undefined) ||
        (typeof data === 'object' && typeof data.detail === 'string'
          ? data.detail
          : undefined) ||
  ...
  parseResponseMessage(message: any) {
    ...
    const metadataReasoning =
      result?.content?.metadata?.additional_kwargs?.reasoning_content;
