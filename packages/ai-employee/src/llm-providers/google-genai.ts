...
export class GoogleGenAIProvider extends LLMProvider {
  ...
  parseResponseMessage(message: any) {
    ...
    if (
      metadata?.response_metadata?.groundingMetadata?.groundingChunks?.length
    ) {
      content.reference = content.reference ?? [];
      for (const groundingChunk of metadata?.response_metadata
  ...
  parseResponseMetadata(output: LLMResult) {
    ...
    return [
      ...
      {
        groundingMetadata:
          chatGenerationChunk?.generationInfo?.groundingMetadata,
      },
    ];
  }
