...
import type { ToolsManager } from '../manager/tools/types.js';
...
describe('Tools loader test cases', () => {
  ...
  it('should load tools when tools in directory for grouping', async () => {
    ...
    expect(await group1.invoke(null, null, null)).toEqual({
      status: 'success',
    });
    ...
    expect(await group2.invoke(null, null, null)).toEqual({
      status: 'success',
    });
    ...
    expect(await group3.invoke(null, null, null)).toEqual({
      status: 'success',
    });
  });
  ...
  it.skip('should load .js file', async () => {
    ...
    for (const toolsName of toolsNames) {
      ...
      expect(tools.definition.description).eq(
        'Fill the form with the given content',
      );
      expect(await tools.invoke(null, null, null)).toEqual({
        status: 'success',
      });
    }
  });
