import type { AIEmployeeToolSetting } from '../repository/index.js';
/**
...
export class AIEmployeeLoader extends LoadAndRegister<AIEmployeeLoaderOptions> {
  ...
  protected async import(): Promise<void> {
      ...
      Array.from(grouped.entries()).map(async ([employeeRoot, fds]) => {
        ...
        try {
          ...
          if (!employeeOptions || !employeeOptions.username) {
            this.logger?.warn(
              `ai employee [${name}] register ignored: invalid definition at ${file.path}`,
            );
            return null;
          }
          ...
          if (promptFile && existsSync(promptFile.path)) {
            try {
              employeeOptions.systemPrompt = await readFile(
                promptFile.path,
                'utf-8',
              );
            } catch (e) {
          ...
          this.logger?.error(
            { error: e },
            `ai employee [${name}] load fail: error occur when import ${file.path}`,
          );
          return null;
        }
      }),
    ...
    this.employeeDescriptors = descriptors.filter(
      (item): item is AIEmployeeDescriptor => Boolean(item),
    );
  }
  ...
  protected async register(): Promise<void> {
    ...
    for (const descriptor of this.employeeDescriptors) {
      try {
        ...
        this.logger?.info(
          `ai employee [${descriptor.options.username}] registered`,
        );
      } catch (e) {
        this.logger?.error(
          { error: e },
