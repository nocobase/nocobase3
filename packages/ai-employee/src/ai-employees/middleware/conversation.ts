...
import { AIMessageInput } from '../types/index.js';
import {
  AIMessage as AIConversationMessage,
  AIToolCall,
  AIToolMessage,
} from '../types/ai-message.type.js';
import z from 'zod';
...
export const conversationMiddleware = (
  ...
  const fillToolCall = (
    ...
    const initializedToolCallMap = new Map(
      initializedToolCalls.map((x) => [x.toolCallId, x]),
    );
      ...
      try {
        ...
        const newState = {
          ...
          lastMessageIndex: {
            ...
            lastAIMessageIndex: state.messages.filter((x) => x.type === 'ai')
              .length,
        ...
        if (values) {
          await aiEmployee.aiChatConversation.withTransaction(
            async (conversation, transaction) => {
              const result: AIConversationMessage =
