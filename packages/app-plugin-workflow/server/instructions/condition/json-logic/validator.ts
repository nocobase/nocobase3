...
const ALLOWED_VARIABLE_ROOTS: ReadonlySet<string> = new Set<string>(
  JSON_LOGIC_VARIABLE_ROOTS,
);
const FORBIDDEN_PATH_SEGMENTS: ReadonlySet<string> = new Set<string>([
  '__proto__',
  'prototype',
  'constructor',
]);
...
export const JSON_LOGIC_CAPABILITIES: JsonLogicCapabilities = Object.freeze({
  ...
  operators: Object.freeze<JsonLogicOperatorCapability[]>([
    ...
    },
    {
      ...
      arguments: 'two',
      returnType: 'boolean',
      description: 'Same-type numeric or string ordering',
    },
    {
      name: '>=',
      arguments: 'two',
    ...
    },
    {
      ...
      arguments: 'two',
      returnType: 'boolean',
      description: 'Array membership or string containment',
    },
    {
      name: 'var',
      arguments: 'variable',
      returnType: 'value',
      description: 'Read an allowed data binding path',
    },
    {
      name: 'startsWith',
      arguments: 'two',
...
function validateArity(
  ...
  const valid =
    operator === 'and' || operator === 'or'
      ...
      : operator === 'var'
        ? typeof value === 'string' ||
          (Array.isArray(value) && (count === 1 || count === 2))
        : count === (operator === '!' ? 1 : 2);
  if (!valid) {
    const expected =
      operator === 'and' || operator === 'or'
        ? 'an array with at least one argument'
        : operator === 'var'
          ? 'a path string or an array of one or two arguments'
          : operator === '!'
  ...
  if (Array.isArray(value) && value.length > JSON_LOGIC_MAX_ARRAY_LENGTH) {
    addIssue(
      ...
      'RESOURCE_LIMIT',
      path,
      `Array length exceeds ${JSON_LOGIC_MAX_ARRAY_LENGTH}`,
    );
  }
  return valid;
}
...
function validateVariable(
  ...
  if (variablePath.length > JSON_LOGIC_MAX_VARIABLE_PATH_LENGTH) {
    addIssue(
      ...
      'RESOURCE_LIMIT',
      path,
      `Variable path exceeds ${JSON_LOGIC_MAX_VARIABLE_PATH_LENGTH} characters`,
    );
    return;
  }
  ...
  if (segments.some((segment) => segment.length === 0)) {
    addIssue(
      ...
      'INVALID_VARIABLE',
      path,
      'Variable path must not contain empty segments',
    );
    ...
    addIssue(
      ...
      'INVALID_VARIABLE',
      path,
      `Variable root must be one of: ${[...ALLOWED_VARIABLE_ROOTS].join(', ')}`,
    );
    ...
    addIssue(
      ...
      'INVALID_VARIABLE',
      path,
      'Variable path contains a forbidden property',
    );
  }
}
...
function visit(
  ...
  if (state.nodes > JSON_LOGIC_MAX_NODES) {
    if (
      !state.issues.some(
        (issue) =>
          issue.code === 'RESOURCE_LIMIT' && issue.message.includes('nodes'),
      )
    ) {
      addIssue(
        state,
        'RESOURCE_LIMIT',
        path,
        `Expression exceeds ${JSON_LOGIC_MAX_NODES} nodes`,
      );
    }
    return;
  }
  if (depth > JSON_LOGIC_MAX_DEPTH) {
    addIssue(
      ...
      'RESOURCE_LIMIT',
      path,
      `Expression depth exceeds ${JSON_LOGIC_MAX_DEPTH}`,
    );
    return;
  }
  ...
  if (typeof value !== 'object') {
    addIssue(
      ...
      'INVALID_VALUE',
      path,
      'Expression must contain only JSON-serializable values',
    );
    return;
  }
  if (state.ancestors.has(value)) {
    addIssue(
      ...
      'INVALID_VALUE',
      path,
      'Expression must not contain circular references',
    );
    return;
  }
  ...
  try {
    if (Array.isArray(value)) {
      if (Reflect.ownKeys(value).some((key) => typeof key === 'symbol'))
        addIssue(
          ...
          'INVALID_VALUE',
          path,
          'Expression arrays must not contain symbol properties',
        );
      if (value.length > JSON_LOGIC_MAX_ARRAY_LENGTH)
        addIssue(
          state,
          'RESOURCE_LIMIT',
          path,
    ...
    if (prototype !== Object.prototype && prototype !== null) {
      addIssue(
        ...
        'INVALID_VALUE',
        path,
        'Expression objects must be plain JSON objects',
      );
      return;
    }
    if (Reflect.ownKeys(value).some((key) => typeof key === 'symbol')) {
      addIssue(
        ...
        'INVALID_VALUE',
        path,
        'Expression objects must not contain symbol properties',
      );
      return;
    }
    ...
    if (entries.length !== 1) {
      addIssue(
        ...
        'INVALID_OPERATION',
        path,
        'An operation object must contain exactly one operator',
      );
      return;
    }
    ...
    if (!OPERATORS.has(operatorName as JsonLogicOperator)) {
      addIssue(
        ...
        'INVALID_OPERATION',
        path,
        `Operator "${operatorName}" is not allowed`,
      );
      return;
    }
    ...
    if (operator === 'var')
      validateVariable(operand, `${path}.${operator}`, state);
    const args = operationArguments(operand);
    if (operator === 'var') {
      if (args.length === 2)
        visit(args[1], `${path}.${operator}[1]`, depth + 1, state);
      return;
    }
    args.forEach((item, index) =>
      visit(item, `${path}.${operator}[${index}]`, depth + 1, state),
    );
  } finally {
...
export function validateJsonLogicExpression(
  expression: unknown,
): JsonLogicValidationResult {
...
export class JsonLogicValidationError extends Error {
  ...
  constructor(issues: JsonLogicValidationIssue[]) {
    super(
      `Invalid JSON Logic expression: ${issues.map((issue) => `${issue.path}: ${issue.message}`).join('; ')}`,
    );
...
export function assertJsonLogicExpression(
  expression: unknown,
): asserts expression is JsonLogicExpression {
