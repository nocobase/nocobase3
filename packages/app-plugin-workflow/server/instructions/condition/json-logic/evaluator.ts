...
function evaluateOperation(
  ...
  raw: JsonLogicExpression | JsonLogicExpression[],
  data: JsonLogicDataBindings,
): unknown {
  ...
  if (
    ...
    return compareOrder(operator, left, right);
  if (operator === 'in') {
    if (Array.isArray(right)) return right.includes(left);
    if (typeof right === 'string' && typeof left === 'string')
      return right.includes(left);
    throw new TypeError(
      'Operator "in" requires a string needle and string haystack, or an array haystack',
    );
  }
  if (typeof left !== 'string' || typeof right !== 'string') {
...
function evaluate(
  ...
  const [operator, raw] = Object.entries(expression)[0] as [
    JsonLogicOperator,
