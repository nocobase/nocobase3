...
export type InboxLiveEventType = 'created' | 'updated' | 'deleted' | 'unread-count-changed';
...
export function createLivePublishingNotificationStore(
  ...
  const publish = (userId: string, type: InboxLiveEventType, ids?: readonly string[]): void => {
    try {
      ...
      logger.warn({ error }, 'Failed to publish a Portal Live inbox event; HTTP reconciliation will converge.');
    }
  };
  ...
  return {
    ...
    async transitionDelivery(input) {
      ...
      if (!result || (result.status !== 'delivered' && result.status !== 'accepted')) return result;
