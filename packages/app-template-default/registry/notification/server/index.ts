...
export interface NotificationService {
  ...
  trigger(principal: NotificationSystemPrincipal, input: NotificationTriggerInput): Promise<NotificationTriggerResult>;
}
...
export function createNotificationModule(options: CreateNotificationModuleOptions): NotificationModule {
  ...
  const dispatchDeliveryWakeup = async (deliveryId: string): Promise<void> => {
    try {
      ...
      options.logger.warn({ error, deliveryId }, 'Failed to publish a notification queue wake-up; the Reconciler will retry.');
    }
  };
    ...
    async trigger(
      ...
      await store.createNotificationBundle({
        notification: { id: notificationId, sourceType: input.source.type, sourceReferenceId: input.source.referenceId,
          principalService: principal.serviceId, triggeredAt: now, messageMode: input.message.kind === 'template' ? 'template' : 'direct',
        ...
        deliveries, statusEvents, userNotificationItems: items,
      });
      for (const delivery of deliveries) await dispatchDeliveryWakeup(delivery.id);
  ...
  const reconcile = (): Promise<void> => {
    ...
    reconcilePromise ??= reconcileNotificationDeliveries({
      ...
      .then((result) => {
        if (result.dispatchFailures > 0) options.logger.warn({ module: 'notification', ...result }, 'Notification reconciliation completed with queue wake-up failures.');
      })
      .catch((error) => options.logger.warn({ error, module: 'notification' }, 'Notification reconciliation failed; persisted Delivery state is unchanged.'))
  ...
  return {
    ...
    start(): Promise<void> {
      ...
      startPromise ??= options.queueManager.init().then(async () => {
        ...
        await reconcile();
        if (!closed) {
          reconcileTimer = setInterval(() => { void reconcile(); }, options.reconcileIntervalMs ?? 10_000);
    ...
    beginShutdown,
    close(closeOptions: NotificationCloseOptions): Promise<NotificationCloseResult> {
      beginShutdown();
...
async function closeNotificationRuntime(
  ...
  const errors: string[] = [];
  await settleBeforeDeadline(Promise.resolve(input.startPromise).catch(() => undefined), input.deadlineAt, 'startup', errors);
  await settleBeforeDeadline(Promise.resolve(input.reconcilePromise).catch(() => undefined), input.deadlineAt, 'reconciler', errors);
  ...
  input.logger.info({ module: 'notification', errors }, 'Notification module closed');
  return { status: 'closed', errors };
}

async function settleBeforeDeadline(task: Promise<void>, deadlineAt: number, label: string, errors: string[]): Promise<void> {
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((_resolve, reject) => {
        timer = setTimeout(() => reject(new Error('shutdown deadline exceeded')), remaining);
      }),
    ]);
  } catch (error) {
    errors.push(`${label}: ${error instanceof Error && error.message === 'shutdown deadline exceeded' ? error.message : 'close failed'}`);
  } finally {
...
export async function dispatchNotificationDelivery(store: NotificationStore, deliveryId: string): Promise<void> {
  ...
  const sending = await store.claimDelivery({ deliveryId, expectedVersion: delivery.version, leaseToken: randomUUID(), leaseOwner: 'notification-worker', leaseExpiresAt: new Date(Date.parse(now) + 30_000).toISOString(), claimedAt: now,
    ...
    event: { id: randomUUID(), deliveryId, sequence: events.length + 1, fromStatus: 'queued', toStatus: 'sending', occurredAt: now, metadataSchemaVersion: 1 }, });
  if (!sending) return;
  const deliveredAt = await store.now();
  await store.transitionDelivery({ deliveryId, expectedVersion: sending.version, fromStatus: 'sending', toStatus: 'delivered', statusChangedAt: deliveredAt, leaseToken: sending.leaseToken,
    event: { id: randomUUID(), deliveryId, sequence: events.length + 2, fromStatus: 'sending', toStatus: 'delivered', occurredAt: deliveredAt, metadataSchemaVersion: 1 }, });
}
...
function validateTrigger(
  ...
  if (!input.source.type || input.targets.length === 0 || input.targets.length > 1000) throw new NotificationModuleError('NOTIFICATION_TRIGGER_INVALID', 'Trigger requires a source and 1-1000 targets.');
  ...
  if (input.message.kind === 'template') validateVariableBytes(input.message.variables, maxCommonVariableBytes, 'common');
  const seen = new Set<string>();
  for (const target of input.targets) {
    validateVariableBytes(target.variables, maxRecipientVariableBytes, 'recipient');
...
function createNotificationRouter(store: NotificationStore): Hono {
  ...
  router.route(notificationRoutePaths.inbox, createNotificationInboxRouter({ store }));
