...
describe('application workflow Artifact startup', () => {
  it('imports, activates, executes, no-ops on restart, replaces without a run, and mints after a run', async () => {
    ...
    const replaceRuntime = runtime(f);
    await replaceRuntime.start();
    const replaced = await f.database
      .query()
      .selectFrom(WORKFLOW_COLLECTIONS.workflows)
      .selectAll()
      .where('key', '=', 'sample')
    ...
    await f.database
      ...
      .execute();
    await getWorkflowEngine(replaceRuntime).trigger(
      'sample',
      {},
      { eventKey: 'artifact-run' },
    );
    ...
    const nodeRun = await f.database
      ...
      .executeTakeFirstOrThrow<Row>();
    expect(JSON.parse(String(nodeRun.result))).toBe('v2');
    await replaceRuntime.stop();
    const v3 = await emit(f.distRoot, 'v3');
    const mintRuntime = runtime(f);
    await mintRuntime.start();
    ...
    expect(revisions[1].hash).toBe(v3);
    expect(run.hash).toBe(v2);
    await mintRuntime.stop();
  });
  it('rejects missing/tampered/multiple dist Artifacts before Runtime becomes ready and forbids production source diagnostics', async () => {
    const f = await fixture();
    const missing = runtime(f);
    await expect(missing.start()).rejects.toThrow(/dist Artifact root/);
    ...
    await expect(tampered.start()).rejects.toThrow(
      /sample.*failed startup synchronization/,
    );
    ...
    await expect(multiple.start()).rejects.toThrow(/exactly one digest/);
    await multiple.stop();
    expect(() =>
      createAppWorkflowRuntime({
        database: f.database,
