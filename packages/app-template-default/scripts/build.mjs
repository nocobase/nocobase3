...
const configOutputPath = path.join(distDir, 'config.yml');
...
const writeDistConfig = () => {
  const source = path.join(rootDir, 'config.yml');
  if (!fs.existsSync(source)) {
    console.log('\n> Copy server config');
    console.log('No config.yml found; skipped dist/config.yml');
    return;
  }

  fs.mkdirSync(distDir, { recursive: true });
  fs.copyFileSync(source, configOutputPath);
  console.log('\n> Copy server config');
  console.log(
    `Copied config.yml to ${path.relative(rootDir, configOutputPath)}`,
  );
};
...
console.log(
  '\nBuild complete: dist/client, dist/server, dist/scripts, dist/config.yml, and dist/package.json',
);
