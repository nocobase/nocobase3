...
export default function AppOverview() {
  ...
  const { scopedOverview, busy, error, refresh, runLifecycle, unregisterApp } =
    useReleaseManagement({
      appId,
    });
  const app = scopedOverview.apps[0];
  const managedApp = scopedOverview.managedApps?.[0];
  ...
  return (
    <div className='space-y-6'>
      <section className='relative overflow-hidden rounded-2xl border bg-[radial-gradient(circle_at_top_right,color-mix(in_oklch,var(--primary)_12%,transparent),transparent_42%),linear-gradient(135deg,var(--card),color-mix(in_oklch,var(--muted)_55%,var(--card)))] p-6 shadow-sm md:p-8'>
        ...
        <div className='relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end'>
          ...
          <div className='flex flex-wrap gap-2'>
            ...
            {managedApp && !deployed && app?.releases.length === 0 ? (
              <AppUnregisterAction
                appName={appName}
                busy={busy}
                onConfirm={() => {
                  void unregisterApp(appId).then((succeeded) => {
                    if (succeeded) void navigate('/apps');
                  });
                }}
              />
            ) : null}
      ...
      <section>
        <ActionCard
          title='部署'
          description='查看 Hub 已接收的构建产物和部署记录。'
          icon={Rocket}
          detail={`${app?.releases.length ?? 0} 个构建产物`}
