...
export async function createServer(scope: AppScope): Promise<Hono> {
  ...
  const releaseManagementPlugin = createReleaseManagementApiPlugin({
    ...
    appStorePath:
      getEnvString(releaseEnv, 'HUB_APP_STORE_PATH') ??
      path.join(dataDir, 'apps.json'),
    allowedRoles: parseRoles(
      getEnvString(releaseEnv, 'HUB_RELEASE_MANAGER_ROLES'),
    ),
    approvalRequired: getEnvBoolean(
      releaseEnv,
      'HUB_RELEASE_APPROVAL_REQUIRED',
    ),
    rollbackEnabled: getEnvBoolean(releaseEnv, 'HUB_RELEASE_ROLLBACK_ENABLED'),
    deployToken: getEnvString(releaseEnv, 'HUB_DEPLOY_TOKEN'),
  });
...
function resolveAppHostUrl(env: Record<string, string | undefined>): string {
  return (
    ...
    `http://127.0.0.1:${
      getEnvString(env, 'APP_HOST_PORT') ?? getEnvString(env, 'PORT') ?? '13010'
    }`
  );
}
