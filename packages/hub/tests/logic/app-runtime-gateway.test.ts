...
import { createAppRuntimeGatewayHeaders } from '../../server/app-runtime-gateway.ts';
...
describe('App runtime gateway', () => {
  ...
  it('replaces client-supplied forwarding metadata at the Hub boundary', () => {
    const headers = createAppRuntimeGatewayHeaders(
      new Request('https://apps.example.com/crm/', {
        headers: {
    ...
    expect(headers.get('x-forwarded-host')).toBe('apps.example.com');
    expect(headers.get('x-forwarded-proto')).toBe('https');
    expect(headers.get('x-forwarded-prefix')).toBe('/crm');
  });
