...
describe('App runtime presentation', () => {
  it('presents an idle App with an active Release as published and started on access', () => {
    const app = {
      id: 'crm',
      name: 'CRM',
      basePath: '/crm',
    ...
    expect(appStateLabel(app.state)).toBe('已发布 · 访问时启动');
  });

  it('uses clear labels for other runtime states', () => {
    expect(appStateLabel('active')).toBe('运行中');
    expect(appStateLabel('not-deployed')).toBe('未发布');
