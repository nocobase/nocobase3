...
Invalid usage — a flag or argument oclif rejects, or a command that does not exist — fails with `INVALID_USAGE` and exit code `2`, and its suggestions name what was probably meant: the closest flag (`Did you mean --connection?`) or command ids, the command's `--help`, and `commands --json`. `src/command/distance.ts` decides what counts as close. The message names the command's own flags, arguments and allowed values but never a value that was typed, because a mistyped line can leave a secret anywhere in oclif's wording; `src/command/usage.ts` rewrites it. An oclif error a command raises itself, such as `this.error()`, is not usage: it reports `COMMAND_FAILED`.
