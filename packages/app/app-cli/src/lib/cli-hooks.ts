...
//
// The result is shaped for the consumer rather than for this package: the build and dev scripts under
// `tools/scripts/` are plain Node and cannot read `cli/plugins.ts` at all, so the `build` and `dev` commands collect the
// hooks here and hand them over in `NOCOBASE_CLI_HOOKS`. That is why a hook carries a resolved `label` and a plain
// `command` array — neither script should have to know that a label is optional to run one.
//
// Both pipelines are collected together so either command can pass one serialized value without knowing the other's
// stage names.
import {
