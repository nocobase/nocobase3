 ...
 *
 * A source checkout reaches the CLI through the `nocobase` bin, but a built `dist/` has none: the dependency links are
 * materialized into real directories and every `.bin` is dropped with them, and a slim runtime image has no pnpm to
 * resolve one either. So the build writes this file, which knows its own location and needs neither — `node
 * dist/cli/index.js db apply` works from any working directory. The application no longer carries a `cli/index.ts`
 * of its own, so nothing the application wrote is overwritten.
 */
function writeCliEntry() {
  const entry = path.join(distDir, 'cli', 'index.js');
  fs.mkdirSync(path.dirname(entry), { recursive: true });
  fs.writeFileSync(
    entry,
    [
      '#!/usr/bin/env node',
      '// Generated by `nocobase build`. Edits are overwritten on the next build.',
      "import path from 'node:path';",
      "import { runAppCli } from '@nocobase/app-cli/runtime';",
      '',
...
const { build: buildHooks } = readCliHooks();
