...
Hub storage defaults distinguish Hub-owned state, Host runtime files, release archives, content-addressed expanded revisions and persistent application volumes. Source and compiled standalone Hub entries share storage outside `dist`; `HUB_STORAGE_DIR` overrides that root. Existing installations retain legacy defaults until an explicit offline migration. Preview `pnpm nocobase app storage migrate --json` and follow the Hub README before applying it with all writers stopped. Do not delete expanded revisions to test restart recovery: restore requires their installed metadata. `appRevisionsDir` and legacy `appDeploymentsDir` select different layouts and are not interchangeable aliases. Build archives now use `storage/exports/dist.tar.gz`; pass `--file` to upload an older archive from another location.
