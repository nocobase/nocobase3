...
Manual execution ignores `autoRun`. `--connection` and `--all` are mutually exclusive. `--all` uses the same connection order and stops at the first failure. `db apply --json` prints `ok`, an overall `status` and a `results` array with one entry per connection and kind, each completed, skipped, failed or not-run. An entry carries `connection`, `kind` (`migrations` or `seeds`) and its own `status`, plus `batch` (migrations only), `executed` and `skipped` once the task has run. Failures exit nonzero.
