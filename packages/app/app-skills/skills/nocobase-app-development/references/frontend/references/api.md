...
Import `toast` from `@nocobase/app-plugin-notification-provider/client/toast` and call `toast.add({ type, title, description })` in event handlers:

| Method            | Use                                                                                         |
| ----------------- | ------------------------------------------------------------------------------------------- |
| `type: 'success'` | The action succeeded; one sentence stating the result                                       |
| `type: 'info'`    | Information, for example that the record to delete has already been deleted by someone else |
| `type: 'error'`   | A single-click action without a dialog, or a background operation, failed                   |

- The registered `@nocobase/app-plugin-notification-provider` plugin mounts the Base UI Toaster globally. Call `toast.add` directly; do not mount another Toaster yourself.
- Do not use `@/components/ui/toast` for application notifications: it has a separate manager used by the reference pages.
