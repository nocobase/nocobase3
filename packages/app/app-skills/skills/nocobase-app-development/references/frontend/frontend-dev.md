...
All code examples use the example "projects" domain; its endpoints and types are at the start of `api.md`. Every example is complete: hooks are called at the top level of a component, a snippet comes with the component or function it belongs to, and omitted parts are marked with `// …`.
...
**Components are built on Base UI, not Radix** (see `styling.md` for details):
...
**Overlays**: create, edit and detail views are child routes by default, using `RouteDialog` / `RouteDrawer`, so a link opens them directly and a refresh restores them. Only a confirmation for a single action (`AlertDialog`) and a temporary panel (`Sheet`) use open state inside the component. See `overlay.md`.
...
**Copy**: all user-visible text goes through translation keys; see `i18n.md`.

## Look up by task

| Task                                                                                                  | Read              |
| ----------------------------------------------------------------------------------------------------- | ----------------- |
| New pages, menu entries, page permissions, settings pages, breadcrumbs, showing buttons by permission | `page.md`         |
| Child pages, tabs, covering child pages, navigation groups                                            | `child-routes.md` |
| Dialogs, drawers (child routes), confirmation dialogs, temporary panels                               | `overlay.md`      |
| Forms, field types, validation, submission, server errors                                             | `form.md`         |
| Calling endpoints, loading data, error handling, writes, toasts                                       | `api.md`          |
| Lists, tables, search and filters, row actions, list states                                           | `table.md`        |
| Choosing components, color, font size, spacing, buttons, icons, dark theme, header buttons            | `styling.md`      |
| Full theme token reference, creating or changing a theme preset                                       | `theme.md`        |
| Copy and translation, languages, date and number formatting                                           | `i18n.md`         |
...
- **Overlays that use component state**: create, edit and detail views must be child-route overlays; otherwise a link cannot open them directly and they disappear on refresh (see `overlay.md`).
- **Calling `useRouteOverlay()` in the component that renders the overlay**: it can be called only from a child component inside `RouteDialog`/`RouteDrawer` (see `overlay.md`).
...
- **Forgetting to update the route test**: `tests/logic/client-routes.test.ts` pins the route names of pages that require sign-in. After adding such a page (including a child route), add its name there, or the test fails (see section 12 of `page.md`).
- **Regular users cannot see the page**: a route without `authz` requires an access grant on `page:<name>` by default; for a page every signed-in user can use, write `authz: 'skip'` (see `page.md`).
- **Hard-coded colors or copy**: see `styling.md` and `i18n.md`.
- **Prefixes in endpoint paths**: the `path` passed to `api.request` includes neither `/api` nor `/main`.
- **Type errors from `useForm`**: do not write `useForm<z.infer<typeof schema>>`; let it infer the type from `zodResolver(schema)` (see `form.md`).
- **Binding the search box to the URL**: do not take the input's `value` directly from a URL parameter, or Chinese input methods break (see `table.md`).
- **Syncing state in an effect**: this application enables the `@eslint-react/set-state-in-effect` rule, so calling `setState` synchronously inside an effect fails lint. When loading data, call `setState` only in the request callbacks (see `api.md`); for "update state when props or the URL change", compare with the previous value during render and update there instead.
- **The confirmation dialog title flickers as it closes**: store the open state and the target (the record to delete) separately, and change only the open state when closing (see `overlay.md`).
