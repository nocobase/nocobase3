...
Application composition roots pass `runtimeLogging: runtime.scope.logging` to `new Application(...)`. Preserve this when upgrading templates: Host supplies App, deployment, and runtime identities and capture policy through that boundary. Hosted runtime logs belong to the App volume, never the expanded release directory. Hub deployment logs belong to each deployment operation and are separate from application runtime logs. See the Hub plugin README for paths, retention, API limits, and the `read-log` permission.
