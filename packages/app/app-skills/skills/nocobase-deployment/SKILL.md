...
Create or select the target App, build the artifact for the Host platform, and upload it. Uploading a Release does not switch the running version. Deploy the selected Release with the complete runtime configuration, wait for the operation result, and inspect the deployment record and App runtime. A separately uploaded Release is deployed with `release deploy`; do not re-upload it with `upload --deploy`.
