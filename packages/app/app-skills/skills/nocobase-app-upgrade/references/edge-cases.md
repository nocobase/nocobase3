...
8. Replace every `app` command id in the application's own documentation, CI and scripts: `nocobase app db apply` is `nocobase db apply`, `app i18n:check` is `locales check`, `app upload` is `release upload`, and a plugin topic follows its package name, so the scheduler's `schedule sync` is `scheduler sync`.
