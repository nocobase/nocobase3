...
A target release that drops `@nocobase/app-plugin-notification-provider` moves every notification onto the Base UI toast in `client/components/ui/toast.tsx`, and the replacement host is not optional: that release's Hub, Users, Workflow and AI employee pages call `Toast.useToastManager()`, which throws `Base UI: useToastManager must be used within <Toast.Provider>` until the application mounts one. Remove the plugin with `pnpm nocobase package remove @nocobase/app-plugin-notification-provider`, add the target's `toaster` entry to `client/react-providers.ts` (taking `client/components/ui/toast.tsx` from the target template if the application lacks it), rewrite each `toast.success(message)`, `toast.error(message)` or `toast.info(message)` imported from `sonner` as `toast.add({ type, title: message })` from `@/components/ui/toast`, and remove `sonner` from `package.json` once nothing imports it. Nothing registers a Refine notification provider any more, so application code that called Refine's `useNotification()` moves to `toast.add` as well.
