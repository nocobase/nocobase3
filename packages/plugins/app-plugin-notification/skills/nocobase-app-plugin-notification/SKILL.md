...
| `recipient`       | send: only when a requested Channel needs one | omitted for recipientless Channels such as IM Webhook                         | `user`, `email`, or `phone` shape supported by every requested Channel        | "Who should receive the notification, or should it go directly to the selected Webhook Provider?" |
...
- If a requested Channel needs a recipient and none is supplied, stop before sending or split the operation into explicit compatible sends. Recipientless Channels, such as IM Webhook, may send without one.
...
- Never infer Notification ids, Provider names, user ids, or email addresses from examples.
...
- Before a real send, confirm the recipient or explicit recipientless mode, Channels, content, source identity when relevant, Provider routing, and expected external effect.
...
- Missing required input always blocks mutation or sending; `recipient` is required only when the selected Channel requires one.
...
4. For a send, follow [Sending Notifications](references/sending-notifications.md). Resolve the notification service from the application's shared container and validate the recipient, or the recipientless mode, against every requested Channel.
...
- Every requested recipient is supported by every selected Channel; recipientless Channels are tested without `to`, and one denied recipient case is tested.
...
1. Valid send: one supported recipient, or a recipientless Channel without `to`, and an enabled Channel create a Notification and reach an accepted Delivery in a fake Provider; replaying the same key returns it without a second Provider call.
...
3. Invalid input: an empty recipient/Channel list or unknown Provider is rejected before an external Provider call; a missing recipient for a recipient-bearing Channel creates a failed Delivery without an external call.
