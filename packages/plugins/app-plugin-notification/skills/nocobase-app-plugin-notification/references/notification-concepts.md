...
`accepted` means the Provider accepted the submission, not that the recipient read it. Known transient failures can schedule a same-Provider retry. `unknown` means submission may have succeeded and requires an explicit retry decision. A retry remains bound to the original Channel name and Provider identifier. Removing, disabling, or changing that Provider prevents rerouting.
