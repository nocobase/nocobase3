...
2. Use [frontend-registry.md](frontend-registry.md) for chat, page context, forms, tasks, browser tools, and AI settings pages. It assumes `client/extensions/nocobase-ai` is installed; install that Registry item first when it is missing.
3. Use [api-reference.md](api-reference.md) when the App must consume `/api/ai`; prefer the installed service and transport.
4. Use [agent-service.md](agent-service.md) for the public server-container integration: `AIConversationsManager`, `AgentServiceFactory`, `AgentContextProvider`, and `ConversationPersistence`.
