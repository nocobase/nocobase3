...
Web search is the one to check first, because there is no capability check anywhere else: neither the model selector nor the composer reads `AIModel.supportWebSearch`, so a chat with web search switched on looks identical on a provider that cannot search, unless the page's own toggle reads it — see [chat-surfaces.md § Web search toggle](chat-surfaces.md#web-search-toggle). The `subAgentWebSearch` tool refuses on those providers rather than answering from memory, which is what makes the gap visible at all.
