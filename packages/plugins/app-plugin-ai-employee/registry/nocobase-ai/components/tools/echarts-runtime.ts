...
const groupLoaders: Record<string, () => Promise<unknown>> = {
  common: () => import('./echarts-runtime-common.js'),
  hierarchy: () => import('./echarts-runtime-hierarchy.js'),
  advanced: () => import('./echarts-runtime-advanced.js'),
  components: () => import('./echarts-runtime-components.js'),
};

export const getEChartsRuntimeSignature = (options: Record<string, unknown>) =>
  getRequiredGroups(options).join(':');

export async function prepareEChartsRuntime(options: Record<string, unknown>) {
  await Promise.all(
