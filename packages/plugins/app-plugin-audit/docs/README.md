...
默认自动记录数据操作；`addRule()` 定制规则，`record()` 补充业务事件。本文用订单示例说明。
...
将 `OrdersAuditProvider` 加入业务插件的 `serviceProviders`。应用级覆盖在业务规则之后注册。
...
**预期结果：**订单与自动审计共同保存。默认规则与 `orders.status` 同时命中，只生成一条记录级事件，目标为 `orders`、`{ id: '123' }`，变化为 `status: pending → approved`。
...
**预期结果：**找到对应事件及状态变化。数字键按字符串查询，复合键传完整对象，例如 `{ tenantId: 't1', orderNo: 'SO-001' }`。
...
| 整条替换某条规则 | 用相同 key 注册完整新定义，后定义替换先定义 |
...
规则作用于已有路由，记录请求摘要，不代表审批成功，也不推断订单键。通过 `requestId` 关联同一请求中的事件。多条 HTTP 规则命中时合并命中信息，只生成一条请求摘要。

## 3. 记录审批结果

状态变化自动记录，审批理由用 `record()` 补充。`category` 默认 `business`，也可直接填写 `sales` 等自定义名称，无需注册；`result` 默认 `success`，身份与时间自动填写。

下面的函数供 HTTP 和 Workflow 复用。示例将其放在工作流包内，供 Run 模块本地导入：
...
**预期结果：**`event` 是审批业务事件。按 `{ id: '124' }` 查询历史还能看到数据操作事件，两者通过 `metadata.transactionId` 关联，分别记录数据操作与审批理由，并共同提交或回滚。
...
此调用记录已确认的拒绝结果，不代替业务权限检查。保存失败尝试时，在业务事务结束后记录。
...
**预期结果：**`accepted` 只表示受理。Run 成功后订单与两条事件共同提交，节点结果包含 `eventId`。执行者、发起人和运行关联由 Workflow 入口适配提供，业务参数不传身份。
...
路径按实际安装目录调整。组件负责规范化记录键，通过 Server 查询历史。规则 Settings 只读展示内置规则、扩展规则和最终合并结果；选择 `orders + updateOne` 即可检查采集字段。

权限使用 `audit.resource`，订单资源标识为 `main.orders`：`read` 查看摘要，额外的 `readValues` 查看历史值、关系键和 `data`；导出还需 `export`。隐藏的详情不参与筛选或排序。规则页需审计设置访问权限。

导出入口为 `POST /api/auditEvents:export`，每次最多 1000 条，沿用查询与分页协议，返回 `{ data: { records, exportedCount, operationId } }`。数据准备完成并成功记录导出事件后才返回结果。
