...
const migration: MigrationDefinition = defineMigration({
  ...
  async up({ builder }) {
    ...
    await builder.createCollection(COLLECTIONS.profileAvatars, (collection) => {
      ...
      collection.primary('id', { name: 'pk_file_demo_profile_avatars' });
      collection.unique('profileId', {
        name: 'uq_file_demo_profile_avatars_profile',
      });
      collection.unique(['disk', 'key'], {
        name: 'uq_file_demo_profile_avatars_disk_key',
      });
      collection
    ...
    await builder.createCollection(COLLECTIONS.orders, (collection) => {
      ...
      collection.datetime('createdAt').notNull();
      collection.datetime('updatedAt').notNull();
      collection.unique('number', { name: 'uq_file_demo_orders_number' });
      collection
    ...
    await builder.createCollection(
      ...
      (collection) => {
        ...
        collection.primary('id', {
          name: 'pk_file_demo_order_attachments',
        });
        collection.unique(['disk', 'key'], {
          name: 'uq_file_demo_order_attachments_disk_key',
        });
        collection.index('orderId', {
          name: 'idx_file_demo_order_attachments_order',
        });
        collection
