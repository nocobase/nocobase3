...
async function fixture(overrides: Partial<FileRepositoryApiExposure> = {}) {
  ...
  for (const contribution of defineFileRepositoryApiRoutes({
    ...
    router.route(
      contribution.scope === 'api' ? '/main/api' : '/main',
      await contribution.createRouter({ container, publicBasePath: '/main' }),
    );
  }
