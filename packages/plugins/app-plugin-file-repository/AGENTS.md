...
Implement the approved file Repository contract. Keep the README and design document
in Chinese. The first version deliberately defers route authentication/authorization
and advanced streaming (Range and conditional requests); do not describe those as implemented.
This core plugin owns services and route helpers only. Collection migrations,
concrete resource routes, pages and locales belong to
`packages/examples/app-plugin-file-repository-example`.
