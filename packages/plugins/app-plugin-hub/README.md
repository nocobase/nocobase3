...
`desiredConfigsDir` stores `<appId>/<deploymentId>.yml`; `logging.deployments.directory` stores `<appId>/<deploymentId>.log`. These locations are independent of `host.configPath`. Omitting them preserves the legacy paths beside that file, including the old `app-configs/<appId>/configs/config.<deploymentId>.yml` naming. `host.appRevisionsDir` selects the flat managed revision layout and cannot be combined with `host.appDeploymentsDir`. Persisted deployment configuration paths must be migrated with their files; changing defaults alone does not update database records. See the Hub template's offline `app storage migrate` command.
