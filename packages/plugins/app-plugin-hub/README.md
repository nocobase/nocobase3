...
For Config file deployments, Hub fills a missing, blank, or example-placeholder `auth.secret` with a secure random secret. `session.secret` is optional and inherits `auth.secret` unless explicitly configured. A supplied secret, or one already present in the current Config file, is preserved and reused by later deployments and configuration publications. External configuration does not create or modify a file secret.
