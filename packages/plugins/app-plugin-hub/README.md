...
Each deployment offers a **View logs** drawer with the actual Hub-observed queued, release preparation, Host activation, and completion events. The drawer requests incremental events every two seconds while open and pending; it stops on completion, request failure, or close. A failed request can be retried. Host execution details include actual artifact reads, checksum verification, extraction, entry discovery, activation, and rollback outcomes. They are correlated by operation ID and collected during deployment and from the final response. Execution details appear directly in a console with millisecond timestamps, durations, and highlighted errors. The console follows new output while at the bottom, pauses when scrolled upward, and resumes when scrolled back to the bottom. Users can copy diagnostics or download the retained log. Deployment progress is polled, not streamed, and application stdout/stderr is not captured. Failed events retain the original failing stage even after rollback or cleanup. Continuous application output is not collected.

The `GET /hub/apps/:appId/deployments/:deploymentId/logs?after=0` endpoint requires `read-deployment` for the requested application and verifies deployment ownership. `after` is a nonnegative safe-integer sequence cursor. Responses contain `items`, `nextCursor`, `status`, `truncated`, and `legacy`. Events include an ISO timestamp, phase, status, and an optional diagnostic code and failed phase. Diagnostics include bounded Host error context with credential-pattern redaction, alongside classification codes. Configuration contents and continuous subprocess output are not collected.

Migration `202609170001_add_deployment_events` adds a nullable JSON journal to deployment records. Existing deployments remain readable and report unavailable historical logs; no events are reconstructed. New journals retain at most 64 structured events per deployment, including the newest terminal event, and live as long as the deployment record. Removing an application removes its journals with its deployment history. There is no automatic time-based expiry. Interrupted deployments receive a persisted restart failure event, and repeated restoration does not duplicate it. The journal and deployment state are written together in database transactions.
