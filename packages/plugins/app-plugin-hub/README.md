...
configure the Client factory with `applicationsPath`, `rolesPath`, and
`userAccessNavigation` to expose a control-plane console. The Hub template uses
...
Applications plus a Users & permissions group as its primary navigation. It
orders User management before the role matrix in that group. It
