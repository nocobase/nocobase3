...
This plugin owns the migration in `migrations/` for restriction rules and their subject assignments. The application discovers it through the server plugin's absolute `baseDir` and executes it through its normal migration lifecycle. Run application migrations after registering the plugin; do not create or modify these tables from a business feature.
