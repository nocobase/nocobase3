...
client/routes.ts
  应用自己的 /login、/register、/forgot-password、/reset-password guest 路由
```

认证路由由应用拥有。应用在 `client/routes.ts` 中声明 `/login` 等 guest 路由，页面
组件来自 `client/extensions/nocobase-auth-ui/`。插件只提供认证协议、会话状态、路由
守卫和 headless actions，不声明客户端路由，也没有 route component override 契约。

认证页面和密码表单应使用安装后的 `client/extensions/nocobase-auth-ui/` Registry 源码。
插件内部已不再提供默认页面、fallback 表单或自有的 shadcn 基础组件。需要新增认证方式时，
可以把该 provider 的客户端实现放在 `client/auth/<provider>`，再由应用页面组合入口引用。
...
如果只是在现有认证页面上增加按钮或文案，直接编辑应用 `client/routes.ts` 中 `/login`
等 guest 路由加载的 `client/extensions/nocobase-auth-ui/` 页面即可。只有 provider
callback 确实需要新的 URL 时，才新增应用路由。
