...
export function MailComposer(props: MailComposerProps): ReactElement {
        ...
        <DialogPrimitive.Portal>
          {composer ? (
            <DialogPrimitive.Popup
              initialFocus={recipientInputRef}
              aria-label={t('workspace.composerTitle', {
                defaultValue: 'New message',
              })}
              className='fixed inset-0 z-50 flex h-svh w-full flex-col overflow-hidden bg-background shadow-2xl outline-none sm:inset-auto sm:right-5 sm:bottom-5 sm:h-auto sm:max-h-[calc(100svh-2.5rem)] sm:w-[min(42rem,calc(100vw-2.5rem))] sm:rounded-2xl sm:border'
            >
                    ...
                    <div className='mt-2 flex gap-2'>
                      <Button
                        onClick={() => {
                          ...
                          setComposeAttachments(
                            recoveryOffer.composeAttachments,
                          );
                          setRetainedAttachments(
                ...
                <label className='flex items-center gap-2 text-sm text-muted-foreground'>
                  <input
                    ...
                    disabled={
                      ...
                      autoSaving ||
                      Boolean(
                        composer.draftMessageId || retainedAttachments.length,
                      )
                    }
                    onChange={(event) =>
