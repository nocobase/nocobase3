...
首次同步以用户指定的起始日期为边界，未指定时同步全部历史邮件。默认每批处理 100 封，持续分页直至范围内的邮件全部扫描完成，不再限制累计消息数；旧版 `maxMessages` 参数不再生效。历史同步与增量同步分别保存游标、交替执行。服务重启后从已提交批次继续；游标失效时重新扫描指定范围，不直接跳到当前进度。大邮件和解析失败邮件会保留可见记录，正文可在邮件详情中单独加载或重试，同步日志会显示待补偿数量。
