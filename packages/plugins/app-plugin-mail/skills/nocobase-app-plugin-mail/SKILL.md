...
Use `/mail` for production reading and composing. In development, the workspace links to `/dev/mail/accounts` for account setup. Production hides these links and excludes the account setup page; OAuth callbacks return to the Dev account page. Mailbox navigation uses a drawer when the container is narrow, and phones switch between the list and the selected conversation. The composer owns its draft session and request errors independently from mailbox queries. Closing unsaved content offers Keep editing or Discard unsaved changes; provider acceptance is reported as queued for delivery, not confirmed delivery.
