...
`/dev/mail/accounts` manages the authenticated user's accounts, including suspend/resume, disconnect, manual synchronization, the received-after date in the account association drawer, signature and NocoBase-owned label management, and reusable templates. The account association control lists registered Providers; a Provider without a `mail.providers` entry is shown as unavailable until the server endpoint or OAuth configuration is added. `/settings/mail/accounts` and `GET /api/mail/settings/accounts` show every connected account to administrators granted `page:mail.admin/access`. The managed account view includes `ownerName`, resolved from the owner’s username or name; the account table falls back to the user ID when neither is available. The ordinary `GET /api/mail/accounts` endpoint remains scoped to the authenticated user and requires `page:mail.workspace/access`. Do not bypass Mail Core ownership checks for another user's account.

`GET /api/mail/settings/operation-logs` provides an all-user administration API for synchronization and delivery operations and requires `page:mail.admin/access`. The response includes API-safe account metadata; it excludes credentials, idempotency fingerprints, leases, Provider cursors, and internal Provider error messages. Use `/dev/mail/logs` to view the authenticated user’s personal synchronization, sending, and bulk sending history.

`/dev/mail/management` and `GET /api/mail/management/*` provide an all-user message table and batch actions only to identities granted `page:mail.management/access`; the current implementation does not apply department or organization scope.
...
`/dev/mail/send` opens one shared Mail center composer with Cc/Bcc, attachments, signatures, templates, scheduled sending, and draft saving and recovery. The From address selector lists sendable addresses across accounts, including aliases, without a separate account selector; each account retains its own unfinished message while the page remains open. The inline composer has no cancel or close action and opens a fresh message after sending or saving a draft. Its footer offers Send and Send separately; separate sending deduplicates up to 100 recipients and requires empty Cc/Bcc fields. Batch messages own their attachment references and do not consume the shared draft. The former `/dev/mail/send/compose` and `/dev/mail/send/bulk` child URLs redirect to `/dev/mail/send`. `/dev/mail/logs` groups delivery, batch delivery, and synchronization under its `send`, `bulk`, and `sync` child routes. Batch logs retain expandable recipient results, safe retry, cancellation, pagination, and automatic refresh of pending submissions. The former `/dev/mail/bulk-send`, `/dev/mail/send-logs`, and `/dev/mail/sync-logs` URLs redirect to the composer or corresponding log child routes while preserving query parameters. Account tables, the management message table, and log views share the same pagination controls; changing filters or page size returns to the first page. The shared controls include the last page number. Message queries with `withTotal=true` return the matching message count; synchronization and submission history queries with `withTotal=true` return `{ items, total }`, with bulk history totals counting batches. The Mail center retains its original cursor pagination and 50-message page size. Message list APIs accept a nonnegative `offset` for direct page selection, mutually exclusive with `cursor`. These pages use the authenticated user’s personal Mail APIs and require `page:mail.workspace/access`; the all-user operation-log API remains protected by `page:mail.admin/access`.

Mail log views use 20 entries per page; batch history counts batches rather than recipients. Personal synchronization and submission history APIs accept optional nonnegative `offset` and `limit` (1–100), allowing clients to reach older history without changing the default recent-history calls used by other consumers. Request one extra entry when calculating whether a next page exists; with `groupByBatch=true`, `limit` and `offset` count complete batches.
