    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          className={cn(
            'max-h-(--available-height) min-w-(--anchor-width) overflow-y-auto text-popover-foreground rounded-lg bg-popover p-1 shadow-md ring-1 ring-foreground/10',
            className,
          )}
          {...props}
        >
...
export function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      className={cn(
        'relative flex cursor-default items-center rounded-md py-1.5 pr-8 pl-2 text-sm outline-none data-highlighted:bg-accent data-selected:bg-accent data-disabled:opacity-50',
        className,
      )}
      {...props}
    >
