...
`authz.ui` decides where the permission workspace lists each resource and is never read by a check. The workspace lists top-level sections on the left, each with its subsections as entries, and the selected subsection's resources on the right, under their groups. This plugin registers the sections `pages` (order 0), `business` (100) and `administration` (200); the subsections `pages.page`, titled `sections.page` in its own namespace, and `authorization`; the `page` record type listed in `pages.page`; and the default sections `composite → business` and `settings → administration`. Resource types have no titles, so every heading the workspace shows comes from `authz.ui`.
...
| `recordTypeSection(type, section)`                       | The client lists the records of record type `type` in subsection `section`, as it fills `pages.page` from its route tree; the options mark that subsection with `recordType`.                                                                                                                                                                                                                          |
...
| `uiPlugin`                            | function | `uiPlugin(): UiPlugin`                                                                                                                                                                                                                   | Registers `authz.ui` with the built-in sections, the `pages.page` and `authorization` subsections, the `page` record type subsection and the default sections. |
...
| `AuthorizationUiApi`                  | type     | `sections`, `groups`, `place(target, placement)`, `defaultSection(type, section)`, `defaultSectionOf(type)`, `recordTypeSection(type, section)`, `recordTypeOf(section)`, `placementOf(ref)`, `validate({ resourceTypes, composites? })` | Where the workspace lists each resource.                                                                                                                       |
