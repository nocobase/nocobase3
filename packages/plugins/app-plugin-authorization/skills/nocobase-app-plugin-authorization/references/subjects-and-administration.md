...
Expose `administration: { title, selection, members?, manage? }` for assignment and rule pickers. A collection selection implements `list({ search, page, pageSize }, { authz })` returning `{ items, total }`, and `resolve(ids, { authz })` returning `{ id, title, description? }[]`. The calling endpoint checks the management permission. Enforce additional directory read authorization and row constraints in both callbacks only when the directory has those independent requirements; ordinary management pickers need no extra capability. Fixed audiences use `{ type: 'fixed', id: '*' }`. Picker access does not authorize saving assignments; preserve inaccessible stored subjects by ID instead of silently dropping them.

Two optional capabilities make an assigned subject explainable from the permission workspace. `members(id, { search?, page, pageSize }, { authz })` returns `{ items, total }` of the users the subject contains, one `{ id, title, description? }` per user; a hierarchical subject such as a department returns its effective members, direct members plus descendants', and the assignments tab then offers a read-only Members drawer with an Inspect link per member. `manage(id)` returns the application-relative path of the subject's own management page, including `/settings` and without the deployment base path, or `undefined`; `resolve` responses then carry it as `manage` and the assignments tab links to it. Both run behind the calling surface's settings check, so their visibility follows that page's gate, and both are served at `GET <surface>/subjects/:type/:id/members` and `POST <surface>/subjects/:type/resolve` for every surface. The options mark each type with `members: true` and `manage: true` when implemented. For a full department model built on these, see the application development Skill's `organization.md`.
