...
Use the complete [quote resource declaration](runtime-api.md#declare-a-business-operation) as the starting point and the [permission-set declaration](code-and-seeds.md#share-declarations-not-runtime-instances) to select its data scopes. This reference adds builder semantics and relation capabilities. A business resource can also be written as a plain object, `{ name, title, section?, actions: [{ name, title, dataScopes, grants }] }`; the package README shows both forms.
