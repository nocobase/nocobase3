...
3. In the owning provider's `boot`, add display groups with `authz.groups.add`, opt collections in with `authz.database.collections.add`, register business resources with `authz.business.define`, record access with `authz.recordAccess.define` and settings items with `authz.settings.add`. Pages are not registered on the server: the client route tree lists them. Release subject registrations at shutdown.
