...
| Authorization          | Test the production Route contribution through `createRouter()`: anonymous render fails, denied template use fails before the data loader and Renderer run, and a second principal cannot print protected rows, fields, related records, or private attachments. |
...
- Attachments: allowed and denied IDs, deleted files, wrong App/source, malformed descriptors, and a public content URL presented for a private attachment. The last case must not bypass the authorized file-ID resolver. Verify private output is streamed or downloaded through a protected Route rather than a public `contentUrl`.
