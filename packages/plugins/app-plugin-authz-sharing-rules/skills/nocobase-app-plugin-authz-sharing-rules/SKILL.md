...
description: Configure sharing rules in a NocoBase 3 application's authorization config and register its client and server plugin.
---

# @nocobase/app-plugin-authz-sharing-rules

Sharing Rules management for NocoBase applications. The pure rule engine and Store contract remain in `@nocobase/authorization/sharing-rules`. This package owns the database Store and migration, administration HTTP routes, settings resource, client page and feature translations.

Register the default export from `./client` in the application's client plugins and the default export from `./server` in its server plugins. Import `sharingRules` from `./server` and include `sharingRules()` in `server/config/authorization.ts` alongside any other rule factories. `@nocobase/app-plugin-authorization` must be registered on both sides.

The settings page remains `/settings/authorization/sharing-rules` and contributes to the existing `authorization` group with `parent`. Management endpoints remain `/api/authz/sharing-rules`, with options, record selection and subject selection subroutes. Every endpoint requires authentication and the matching action on `settings` resource `authorization.sharing-rules`. If the rule factory is absent from authorization config, no management endpoints are installed.
