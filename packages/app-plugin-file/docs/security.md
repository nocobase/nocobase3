...
The built-in `/file-demo` management API is intentionally stricter than the
...
The File-owned policy identifies the management resource as
`file.demo/management`. Its management middleware resolves the current
authorization identity through `deps.authz.middleware()` and checks the
effective Permission Sets before any list, upload, metadata, Token, or delete
operation reaches the Store.
...
In an app plugin route registrar, mount a file Route on the supplied public app
when its content endpoint must support unsigned Public access or capability
Tokens. The normal plugin app is authenticated by the host for API routes;
the File Route still applies its own management authentication and administrator
check.
...
inside `createFileRoute`; business modules supply the existing database and
Drive manager, or an intentionally custom scoped `FileStore`, rather than
calling a storage driver directly.
