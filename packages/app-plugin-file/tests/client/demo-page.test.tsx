...
import FileDemoPage from '../../client/default-pages/file-demo-page.js';
...
async function renderReady(): Promise<void> {
  render(<FileDemoPage />);
  expect(
    await screen.findByRole('heading', { name: 'File Demo' }),
  ).toBeVisible();
}
...
describe('FileDemoPage', () => {
  ...
  it('displays max-file and upload errors', async () => {
    ...
    const firstRender = render(<FileDemoPage />);
    await screen.findByRole('heading', { name: 'File Demo' });
    ...
    render(<FileDemoPage />);
    await screen.findAllByRole('heading', { name: 'File Demo' });
  ...
  it('contains no remote sample URL or legacy storage action', async () => {
    const source = await readFile(
      'client/default-pages/file-demo-page.tsx',
      'utf8',
    );
