...
export class LocalStorageDriver implements StorageDriver {
  ...
  async putObject(input: { key: string; body: ReadableStream<Uint8Array>; contentType: string; expectedSize: number; checksumSha256?: string; signal?: AbortSignal }): Promise<StoredObjectStat> {
    ...
    const temp = path.join(this.root, `.tmp-${randomUUID()}`), hash = createHash("sha256"); let size = 0;
    ...
    try {
      ...
      try { await rename(temp, target); } catch (e) { if ((e as NodeJS.ErrnoException).code === "EEXIST") throw new StorageDriverError("conflict", "object already exists"); throw e; }
    ...
    } catch (e) { await rm(temp, { force: true }).catch(() => undefined); if (e instanceof StorageDriverError) throw e; if ((e as Error).name === "AbortError") throw new StorageDriverError("aborted", "operation aborted", { cause: e }); throw new StorageDriverError("unavailable", "storage operation failed", { cause: e }); }
  }
