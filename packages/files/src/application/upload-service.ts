...
export class UploadService {
  ...
  async proxy(workspaceId: string, uploadId: string, token: string, headers: Headers, body: ReadableStream<Uint8Array>) {
    ...
    const upload = await this.o.store.getUpload(workspaceId, uploadId); if (!upload) throw new FilesError("FILES_UPLOAD_NOT_FOUND", "Upload not found"); if (upload.status !== "pending") throw new FilesError(upload.status === "expired" ? "FILES_UPLOAD_EXPIRED" : "FILES_CONFLICT", "Upload is not pending");
  ...
  async complete(actor: ActorContext, workspaceId: string, uploadId: string) {
    ...
    await authorizeFileAction(this.o.authorizer, { action: "files.upload", policy: file.policy, context: file.accessContext, file: { id: file.id, policy: file.policy } }, { actor, workspaceId }); if (upload.status === "completed") return { file: presentFile(file) }; if (upload.status === "failed") throw new FilesError("FILES_CONFLICT", "Upload has failed"); if (upload.status === "expired") throw new FilesError("FILES_UPLOAD_EXPIRED", "Upload has expired"); if (new Date(upload.expiresAt) <= this.o.now()) { await this.o.store.expireUpload(workspaceId, uploadId, this.o.now()); throw new FilesError("FILES_UPLOAD_EXPIRED", "Upload has expired"); }
