...
describe("S3 Kernel lifecycle", () => {
  ...
  beforeEach(async () => {
    db = createTestDatabase(); await filesMigrations["001-create-files-tables"].up!(db); await filesMigrations["002-add-upload-cleanup-status"].up!(db); calls = []; object = null; failDelete = false;
