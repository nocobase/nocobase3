...
describe("G05 proxy upload lifecycle", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), "files-g05-")); db = createTestDatabase(); await filesMigrations["001-create-files-tables"].up!(db); await filesMigrations["002-add-upload-cleanup-status"].up!(db); authorizations = []; now = new Date("2026-08-18T00:00:00.000Z"); let id = 0;
