...
async function resolveInsensitiveWhere(
  ...
  if (connection.driver !== 'knex') {
    throw new Error('Case-insensitive Better Auth filters currently require the internal Knex raw fallback.');
  }
  ...
  return Promise.all(where.map(async (condition) => {
    ...
    const source = connection.query.selectFrom(model)
      ...
      .compile();
    const query = knex
      .from(knex.raw(`(${source.sql}) as ??`, [...source.parameters, 'authsource']))
