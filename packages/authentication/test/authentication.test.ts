...
import { Auth, type AuthEnv } from '../src/index.js';
import { databaseAdapter } from '../src/better-auth/database-adapter.js';
import { createAccountCollection } from '../src/storage/collections/account.js';
import { createSessionCollection } from '../src/storage/collections/session.js';
import { createUserCollection } from '../src/storage/collections/user.js';
import { createVerificationCollection } from '../src/storage/collections/verification.js';

async function createAuthenticationCollections(connection: DatabaseConnection) {
  const { builder } = connection;
  await createUserCollection(builder);
  await createSessionCollection(builder);
  await createAccountCollection(builder);
  await createVerificationCollection(builder);
}
...
describe('Authentication', () => {
  ...
  beforeAll(async () => {
    ...
    await createAuthenticationCollections(connection);
    ...
    app.get('/api/private', auth.requireSession(), (context) =>
      context.json({ ok: true, auth: context.get('auth') }));
    app.get('/api/optional', auth.optionalSession(), (context) =>
  ...
  it('supports optional sessions', async () => {
    ...
    const authenticated = await app.request('/api/optional', { headers: { cookie } });
    expect(authenticated.status).toBe(200);
    expect(await authenticated.json()).toMatchObject({
      auth: { user: { id: expect.any(String) }, session: { id: expect.any(String) } },
    });
  });
  ...
  it('matches email credentials case-insensitively', async () => {
    ...
    await expect(adapter.findOne({
      model: 'user',
      where: [{ field: 'email', value: 'ALICE@EXAMPLE.COM', mode: 'insensitive' }],
...
describe('Authentication naming strategy', () => {
  it('supports underscored: false', async () => {
    ...
    try {
      ...
      await expect(adapter.findOne({
        model: 'user',
        where: [{ field: 'email', value: 'CAMEL@EXAMPLE.COM', mode: 'insensitive' }],
