...
Authorization decisions, resource declarations, permission sets and record-scope rule engines for Node applications. NocoBase applications normally resolve `authorizationToken` from `@nocobase/app-plugin-authorization/server`; this library is the lower-level integration API. Database enforcement, persistence, authentication integration and management UI belong to that application plugin.
...
| `@nocobase/authorization/default-access`    | `defaultAccess`, `defaultAccessRule`, `DefaultAccessApi`, `DefaultAccessStore`                                                                 |
| `@nocobase/authorization/sharing-rules`     | `sharingRules`, `sharingRule`, `SharingRulesApi`, `SharingRuleStore`                                                                           |
| `@nocobase/authorization/restriction-rules` | `restrictionRules`, `restrictionRule`, `RestrictionRulesApi`, `RestrictionRuleStore`                                                           |
...
Factories add `authz.defaultAccess`, `authz.sharingRules` and `authz.restrictionRules` respectively. Rule services are storage/configuration APIs, not permission checks; authorize their administration in the host.

| Service            | Methods                                                                                                                    |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `defaultAccess`    | `set(rule)`, `get(resourceType, resourceId)`, `list()`, `delete(resourceType, resourceId)`, `withTransaction(transaction)` |
| `sharingRules`     | `create(rule)`, `update(key, rule)`, `get(key)`, `list()`, `delete(key)`, `withTransaction(transaction)`                   |
| `restrictionRules` | `create(rule)`, `update(key, rule)`, `get(key)`, `list()`, `delete(key)`, `withTransaction(transaction)`                   |

Default rules are `{ resource, actions: [{ action, scopeKey?, scope }] }`. Sharing rules add `key`, `title?`, `subjects`, `reason?`, and actions use `selection: { type: 'records', ids }` or `{ type: 'policy', policy }`. Restriction rules add the same identity metadata and use `scope`. `scope`/`policy` is an `AccessConstraintValue` interpreted by the resource adapter.

Typed builders bind a declared business resource and validate action/scope names:
...
// quotes is a defineAuthorizationResource declaration with submit/quotes scope.
const target = quotes.reference();
await authz.defaultAccess.set(
  defaultAccessRule(target)
    .scope('submit', 'quotes', databaseScope('recordsIOwn'))
    .build(),
);
await authz.sharingRules.create(
  sharingRule('handover', target)
    .subjects({ type: 'user', id: 'alice' })
    .scope('submit', 'quotes', { type: 'records', ids: ['quote-7'] })
    .build(),
);
await authz.restrictionRules.create(
  restrictionRule('owned-only', target)
    .subjects({ type: 'user', id: 'alice' })
    .scope('submit', 'quotes', databaseScope('recordsIOwn'))
    .build(),
);
...
For database resources, permission-set scope, defaults and sharing provide positive record access; restrictions intersect it. None of these rules grant missing actions or fields. Business rules apply only to their matching resource/action/scope branch. Collection restrictions apply across branches. Relations need explicit permissions and do not acquire target constraints automatically. See the [application API](../../plugins/app-plugin-authorization/README.md) for executable database policies and relation builders, and [user documentation](../../../docs/docs/en/capabilities/authorization/index.md) for configuration guidance.
