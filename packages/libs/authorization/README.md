...
| `CompositeApi`                       | type     | `define(objectOrBuilder): CompositeReference`, `getAction(id, action)`, `list()`, `validate(): readonly string[]`                                                                      | `authz.composites`; `validate` lists data scopes whose target type is unregistered or lacks `recordAccess`. |
