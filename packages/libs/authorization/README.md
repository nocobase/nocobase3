...
| Composite resource    | A built-in core mechanism, with no plugin: a resource whose actions expand into a set of underlying grants, each optionally bound to a named data scope. Stored under the reserved type `composite` and registered with `authz.composites.define`, which every Authorization has; it composes exactly the grants its definition lists, never another composite. |
...
A stored composite grant that no longer expands against the current definition — an unknown action, an unknown or invalid data scope value, a malformed policy, or an unfit data scope target — is skipped for that grant alone: it permits nothing, contributes nothing to the snapshot, and the identity's other grants keep working. A direct check of that composite action, with no other valid grant, denies with `INVALID_GRANT`, naming the grant's source and the problem. `createAuthorization({ onInvalidGrant })` is told once per distinct grant and problem, with `{ source, resource, action, reason }`; the library has no logger of its own. `authz.composites.validateGrant({ resource, action, policy? })` returns the same problem for a stored grant, or `undefined`, so a host can scan its stored grants at startup. The library does not validate grants on save; a host that accepts grants from administrators checks them there.
...
| `CompositeApi`                       | type     | `define(objectOrBuilder): CompositeReference`, `getAction(id, action)`, `list()`, `validate(): readonly string[]`, `validateGrant(grant): string \| undefined`                         | `authz.composites`; `validate` lists data scopes whose target type is unregistered or lacks `recordAccess`; `validateGrant` explains why a stored grant no longer expands. |
