...
const entries = Object.entries(packageMetadata.exports).map(
  ([key, target]) => ({
    specifier:
      key === '.'
...
describe('README export reference', () => {
  const lists = new Map(
    entries.map((entry) => [
      entry.specifier,
      documented(section(entry.specifier)),
    ]),
  );

  for (const entry of entries) {
    it(`lists every export of ${entry.specifier}, and nothing else`, () => {
      const actual = [...exportNames(entry.file)].sort();
      const listed = lists.get(entry.specifier);
      const expected = listed
        ? [...listed]
        : [...lists.values()].flatMap((names) => [...(names ?? [])]);
      expect([...new Set(expected)].sort()).toEqual(actual);
    });
  }
});
