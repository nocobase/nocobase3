...
import { createBackendHarness } from '../helpers/backend-harness.js';
import { createTcpProxy } from '../helpers/tcp-proxy.js';

it('recovers an active job after failed forced shutdown without claiming cleanup confirmation', async () => {
  const harness = await createBackendHarness();
  try {
    if (!(harness.connection instanceof Redis))
      throw new Error('Active recovery requires an independent Redis observer');
    ...
    try {
      ...
      const original = createQueueService({
        ...
        connection: {
          host: '127.0.0.1',
          port: proxy.port,
          // Fixture teardown may close a late official pause reconnect. Do not
          // leave a retrying fixture client after its owned proxy is removed.
          retryStrategy: () => null,
        },
      });
      ...
      replacement.consumer('jobs').consume(async (channel, message, signal) => {
      ...
      try {
        ...
        void closing.catch(() => {});
        await expect.poll(() => shutdownSettled, { timeout: 7000 }).toBe(true);
        ...
        // Strategy A can reconnect after force-close when its original pause
        // finally drains. Preserve that unresolved-cleanup diagnosis instead
        // of equating a fulfilled official close with physical cleanup.
        await expect(closing).rejects.toMatchObject({
          errors: expect.arrayContaining([
            expect.objectContaining({
              message:
        ...
        release();
        await unregister();
        await expect(original.shutdown()).rejects.toMatchObject({
          message: 'Queue shutdown failed',
        });
        expect(originalCalls).toBe(1);
        expect(await observer.getJobState(receipt.jobId)).toBe('completed');
      } finally {
        release();
        try {
          await unregister();
