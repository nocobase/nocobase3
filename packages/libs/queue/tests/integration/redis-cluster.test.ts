...
it('uses official shared Cluster ownership for Unicode queue identity and bulk work', async () => {
  ...
  try {
    ...
    for (const name of ['other-a', 'other-b', 'other-c']) {
      ...
      const other = createQueueIdentity(`${harness.namespace}-应用{one}`, name);
...
it('executes delay, retry, rate limits and drain without CROSSSLOT', async () => {
  const harness = await createBackendHarness();
  const { Cluster } = await import('ioredis');
  if (!(harness.connection instanceof Cluster))
    throw new Error('Expected Cluster fixture');
