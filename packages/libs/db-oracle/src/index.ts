...
export const oracleDriver: DatabaseDriverDefinition<'oracle'> = {
  ...
  createRuntime: ({ dialect, capabilities }) => ({
    ...
    schema: {
      ...
      columnType: ({ column }) => {
        // Let Knex's increments/bigIncrements compiler emit the identity
        // column.  The generic schema adapter then adds the explicit primary
        // key constraint, which Oracle requires before foreign keys can refer
