...
2. **写入后的记录必须仍满足本次操作的 scope，否则在同一事务内回滚。** 判定在数据库里做——事务内、行已锁定时发一条 `SELECT … WHERE <主键> AND <scope>`——而不是在内存里对前像求值：内存求值必须复刻数据库自己的比较语义，而其中列的排序规则是复刻不了的（同一个字符串等值比较，MySQL 默认排序规则匹配、PostgreSQL 不匹配，求值器看不见该用哪一套）。**写入没有触及 scope 引用的字段时整条判定跳过**，所以常规路径仍不产生额外查询；见[执行细节](./policies-internals.md)与[实施清单](./policies-roadmap.md) 1.4。
