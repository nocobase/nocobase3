...
- 可校验：执行前根据 metadata、policy 和数据库当前状态验证整棵 AST。
...
metadata、policy 与数据库约束决定，不通过 Mutation AST 临时指定。
...
已有记录只能通过主键或命名唯一约束定位，不能使用一般 Filter AST：
...
{
  "kind": "unique",
  "constraint": "tenant_email_unique",
  "values": {
    "tenantId": "tenant-1",
...
Repository 必须校验约束存在、字段集合完整且没有额外字段。规范 AST 不接受裸 ID；静态
Fluent Builder 可以把 `connect({ id })` 规范化成主键 selector。
...
没有旧关系。默认预算建议为 `maxDepth: 3`、`maxNodes: 100`，实际值由服务端配置和 policy
收紧。Planner 必须检测循环和预算超限。
...
非主键唯一约束使用 `connectBy(constraint, values)`。Builder 只构造 AST，不执行查询，也
...
不提交给后端，也不能作为并发或权限依据。后端只接收 mutation、可选 `ifVersion` 和
context，并根据数据库当前状态重新校验。

只有完整加载的关系集合才能生成 `replace`。分页、懒加载或权限裁剪后的部分列表只能根据
...
`describeMutation()` 的 capability 结果受当前 Collection metadata 和可信 policy context
约束；它不是一份对所有调用者都相同的静态清单。
  ...
  "relations": [
    {
      ...
      "uniqueConstraints": [{ "name": "primary", "fields": ["id"] }]
    },
    {
      "field": "members",
      "cardinality": "many",
...
`validateMutation()` 可以检查结构、metadata、唯一约束、权限形态、冲突和预算，但不能承诺
...
values + RelationMutationAst
  ...
  -> apply trusted Repository policies
  -> build dependency-aware mutation plan
  -> start/reuse one DatabaseConnection transaction
  -> resolve and authorize existing targets
  -> execute root, nested target and edge writes
  -> optionally reload through Select AST
  -> commit and return result
```

关系的物理位置由 metadata 决定：`belongsTo` 写 source 外键，`hasOne` / `hasMany` 写 target
外键，`belongsToMany` 写 through Collection。调用方不能提交物理表列或 through 两侧外键。

`replace` 必须使用行锁、关系版本或等价并发控制；普通事务本身不能防止两个并发 replace
互相覆盖。Policy 至少分别检查 root 写入、relation action、existing target 引用和 nested
target create。

`createOne()` 和 `updateOne()` 可以接受 Select AST，并在提交事务前重新读取最终记录。含
`clientKey` 的 nested create 应返回 `clientKey` 到真实唯一键的映射，供表单错误和结果回填。
...
- metadata、policy、冲突、并发和资源预算校验；
- 根、嵌套目标和关系边的单事务执行；
- 可选 Select AST 回读和 `clientKey` 结果映射。
...
- 已有目标只用命名唯一约束 selector；新目标只用 `create`。
