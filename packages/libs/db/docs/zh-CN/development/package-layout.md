...
```text
src/
├── database/          DatabaseManager、DatabaseConnection、驱动和连接生命周期
├── collection/        Collection DSL、编译、解析和 Registry
├── metadata/          Metadata Document、Store、Service 和旧定义转换
├── naming/            逻辑标识符到物理标识符的确定性映射
├── query/             QueryAdapter 公共类型和方言实现
├── schema/            Schema operation、能力规划和 Schema Inspector
...
负责多 Connection 管理、配置校验、驱动实例化、事务和底层 client 生命周期。`DatabaseManager` 的便捷入口最终都委托给某个 `DatabaseConnection`。
...
负责 Metadata Document 的定义、校验和存储。当前包含 Database、Module、In-memory 和事务覆盖 Store，以及面向写操作的 Metadata Service。`legacy-extraction.ts` 是显式迁移旧 Collection 定义的专用工具，不是运行时 fallback。

### `query/`

提供数据库层 QueryAdapter。它使用 Connection 命名规则，但不读取 Collection Metadata，也不提供 Repository 语义。

### `schema/`

Schema Adapter 执行 Builder 编译出的操作，capability planner 处理方言差异，`inspector/` 读取真实数据库结构。Schema Inspector 使用物理名称，不负责补充 Metadata。
...
- 新增公开 API 时，从对应模块的 `index.ts` 导出，并确认包级 `src/index.ts` 是否需要重新导出；不要让调用方导入深层实现文件。
