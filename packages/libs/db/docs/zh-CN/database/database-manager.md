...
| Lazy API                  | `connection()`、`builder()`、`query()`、`collections()` |
...
| `db.collections(name?)`            |    否 | `db.connection(name).collections`            |
...
`connection()`、`builder()`、`query()` 和 `collections()` 不需要 `await`。底层 client 由实际操作惰性创建。
