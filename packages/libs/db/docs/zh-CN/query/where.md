...
未来 Repository 可能提供 Collection-aware 的 Filter Builder，根据 Collection metadata 校验字段类型和 operator group。`db.repository()` 当前尚未实现，不要把提案示例复制到运行时代码；详细设计见 [Filter Builder 提案](../proposals/repository/filter-builder.md)。
