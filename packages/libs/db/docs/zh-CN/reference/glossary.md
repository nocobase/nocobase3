...
Repository 是基于 Collection 的公开记录与关系访问层；Filter、Select、Sort 支持 Builder 和可序列化 AST。Values 使用模型形状的输入，RelationMutationAst 表达规范化关系计划，并非 CRUD 根级 relations 参数。当前语义见 [Repository 正式文档](../repository/overview.md)。
