...
| Select                    | [select](./relations/select.test.ts), [limits](./relations/select-limits.test.ts), [isolation](./relations/select-isolation.test.ts): four cardinalities, empty shapes, nested include, local options, Builder/AST parity, depth 20/21 and nodes 200/201, frozen AST and callback reuse across contexts                                                                                                                         | More shared-target deep sibling graphs and combined-budget shapes                                                      |
...
| Aggregate Select          | [combine](./relations/aggregate-select.test.ts), [limits](./relations/select-limits.test.ts), [isolation](./relations/select-isolation.test.ts), [validation](./relations/select-validation.test.ts): exact aggregates, empty/all-null sets, common AND branch filters, option inheritance/overrides, 1/32 versus 0/33 branches, reserved names, malformed results/branches and rejection across all mutation returning methods | More cardinality combinations, nested combine budgets and full nested diagnostic paths                                 |
| Local pagination/Distinct | [pagination](./relations/pagination.test.ts), [distinct](./relations/distinct.test.ts): per-parent limit, shared forward/backward cursor, root representative relations and local Distinct                                                                                                                                                                                                                                      | belongsToMany/shared targets, compound/tied cursor axes, deep branches, invalid local cursors and operation ordering   |
