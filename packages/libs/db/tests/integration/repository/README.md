...
| Aggregate Select          | [combine](./relations/aggregate-select.test.ts), [limits](./relations/select-limits.test.ts), [isolation](./relations/select-isolation.test.ts): exact aggregates, empty/all-null sets, records/count branches, common AND branch filters, local option inheritance/overrides, 1/32 versus 0/33 branches and reserved names | More cardinality combinations, malformed branch structures and nested combine budgets                                  |
