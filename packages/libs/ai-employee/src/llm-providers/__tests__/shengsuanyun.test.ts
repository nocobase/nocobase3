...
vi.mock('../../utils/server-request.js', async (importOriginal) => {
  const original: any =
    await importOriginal<typeof import('../../utils/server-request.js')>();
  return {
    ...original,
...
describe('ShengSuanYunProvider', () => {
  ...
  it('recognizes Chat Completions-compatible models', () => {
    ...
    expect(supportsChatCompletions({ id: 'fallback-model' })).toBe(true);
  });
  ...
  it('loads and filters the account model catalog with the configured X-Title header', async () => {
    ...
    await expect(provider.listModels()).resolves.toEqual({
      models: [{ id: 'deepseek/deepseek-v4-flash' }, { id: 'fallback-model' }],
    });
