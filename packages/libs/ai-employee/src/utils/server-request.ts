...
const SSRF_RISK_IP_RANGES = new Set([
  'loopback',
  'private',
  'linkLocal',
  'uniqueLocal',
...
function matchesIpEntry(hostname: string, entry: string): boolean {
  try {
    ...
    if (
      entryAddr.kind() === 'ipv6' &&
      (entryAddr as ipaddr.IPv6).isIPv4MappedAddress()
    ) {
...
function matchesEntry(hostname: string, entry: string): boolean {
  ...
  return ipaddr.isValid(hostname)
    ? matchesIpEntry(hostname, e)
    : matchesDomainPattern(hostname, e);
}
...
async function warnIfResolvedSsrfRiskTarget(url?: string): Promise<void> {
  ...
  for (const address of addresses) {
    ...
    if (reason) {
      warnSsrfRiskTarget(
        host,
        `resolves to ${address.address} (${reason})`,
        `${host}:resolved-risk`,
      );
      return;
    }
