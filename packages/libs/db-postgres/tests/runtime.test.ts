...
function createRuntime() {
  ...
  const runtime = postgres.driver.createRuntime!({
    dialect: 'postgres',
    sourceConfig: {} as never,
    config: {} as never,
    capabilities: postgres.driver.capabilities as never,
    getClient: () => client,
...
describe('postgres runtime strategy', () => {
  it('exposes PostgreSQL capabilities and native temporal SQL', () => {
  ...
  it('owns PostgreSQL defaults and schema-scoped ownership identity', () => {
    expect(
      postgres.driver.normalizeConnection?.(
        { dialect: 'postgres', database: 'app' } as never,
        {},
      ),
    ).toMatchObject({
      host: '127.0.0.1',
    ...
    expect(
      postgres.driver.resolveOwnershipTarget?.({
        dialect: 'postgres',
