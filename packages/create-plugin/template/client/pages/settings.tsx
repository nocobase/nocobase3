...
export default function __NOCOBASE_SYMBOL_NAME__SettingsPage(): ReactElement {
      ...
      <div className='mx-auto max-w-4xl space-y-6'>
        <header>
          ...
          <p className='mt-2 max-w-2xl leading-7 text-muted-foreground'>
            A polished settings contribution is included so the generated
            plugin feels complete from its first run. Replace these preview
            values with your own persisted configuration.
          </p>
        </header>
        ...
        <section className='overflow-hidden rounded-2xl border bg-card shadow-sm'>
          ...
          <div className='divide-y px-5 sm:px-6'>
            ...
            <div className='grid gap-3 py-5 sm:grid-cols-[minmax(0,1fr)_12rem] sm:items-center'>
              <div>
                <label htmlFor='plugin-retention' className='text-sm font-medium'>
                  Data retention
                </label>
          ...
          <div className='flex flex-col gap-3 border-t bg-muted/20 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6'>
            <p className='text-xs text-muted-foreground'>
              Demo only — connect this form to your server service before shipping.
            </p>
            <button
              type='button'
              onClick={savePreview}
        ...
        <section className='grid gap-4 sm:grid-cols-2'>
          <article className='rounded-xl border bg-card p-5 shadow-sm'>
            ...
            <p className='mt-2 font-mono text-sm'>
              {__NOCOBASE_ROUTE_PATH_LITERAL__}
            </p>
