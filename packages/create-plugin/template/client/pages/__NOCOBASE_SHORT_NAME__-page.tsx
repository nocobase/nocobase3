...
import { use__NOCOBASE_SYMBOL_NAME__ } from '../contexts.js';

interface __NOCOBASE_SYMBOL_NAME__Response {
...
function requestPlugin(
  signal?: AbortSignal,
): Promise<__NOCOBASE_SYMBOL_NAME__Response> {
  return appClient.request<__NOCOBASE_SYMBOL_NAME__Response>(
    __NOCOBASE_SHORT_NAME_LITERAL__,
    { signal },
  );
}
...
function CodeValue({ children }: { readonly children: ReactNode }): ReactElement {
  return (
...
export default function __NOCOBASE_SYMBOL_NAME__Page(): ReactElement {
  const { welcomeMessage } = use__NOCOBASE_SYMBOL_NAME__();
  const [result, setResult] = useState<__NOCOBASE_SYMBOL_NAME__Response>();
      ...
      <div className='mx-auto max-w-6xl space-y-6'>
              ...
              <div className='max-w-2xl'>
                <div className='mb-4 flex items-center gap-3'>
                  <span className='grid size-11 place-items-center rounded-xl bg-primary text-lg font-bold text-primary-foreground shadow-sm'>
                    {__NOCOBASE_DISPLAY_NAME_LITERAL__.slice(0, 1)}
                  </span>
                ...
                <h1 className='mt-1 text-3xl font-semibold tracking-tight'>
                  {__NOCOBASE_DISPLAY_NAME_LITERAL__}
                </h1>
        ...
        <section className='grid gap-4 sm:grid-cols-3'>
          ...
          <StatusCard
            ...
            value={error ? 'Needs attention' : isLoading ? 'Checking' : 'Online'}
            detail={error ? 'Review the response below' : 'API route responded successfully'}
            tone={error ? 'default' : 'success'}
          />
          <StatusCard
            label='Route'
            value={__NOCOBASE_ROUTE_PATH_LITERAL__}
          ...
          <article className='overflow-hidden rounded-2xl border bg-card shadow-sm'>
            <div className='flex items-center justify-between border-b px-5 py-4 sm:px-6'>
              ...
              <span className='font-mono text-xs text-muted-foreground'>
                GET /api/__NOCOBASE_SHORT_NAME__
              </span>
            </div>
              ...
              {isLoading ? (
                ...
                <dl className='grid gap-4'>
                  <div className='rounded-xl border bg-muted/20 p-4'>
                    ...
                    <dd className='mt-2 break-all font-mono text-sm'>{result?.plugin}</dd>
                  </div>
                  <div className='rounded-xl border bg-muted/20 p-4'>
                    <dt className='text-xs font-medium tracking-wide text-muted-foreground uppercase'>
                      Message
                    </dt>
                    <dd className='mt-2 text-sm leading-6'>{result?.message}</dd>
                  </div>
                </dl>
              )}
            </div>
