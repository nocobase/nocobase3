...
describe('migration loader', () => {
  ...
  it('loads only default defineMigration exports and sorts by migration name', async () => {
    ...
    await writeMigration(directory, '202608180002_second', `
    ...
    `);
    await writeMigration(directory, '202608180001_first', `
      import { defineMigration } from '../../../src/index.js';
  ...
  it('accepts irreversible migrations without down', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_cleanup_data', `
      import { defineMigration } from '../../../src/index.js';
  ...
  it('rejects named up and down exports', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_named_exports', `
      export async function up() {}
      export async function down() {}
    `);

    await expect(loadMigrations({ directory }))
      .rejects
      .toThrow('must default export defineMigration({...}).');
  });

  it('rejects default objects that did not go through defineMigration', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_plain_object', `
      export default {
  ...
  it('rejects CommonJS module exports', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_commonjs', `
      module.exports = {
  ...
  it('rejects missing down unless irreversible is true', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_missing_down', `
      import { defineMigration } from '../../../src/index.js';
    ...
    await expect(loadMigrations({ directory }))
      .rejects
      .toThrow('must define down(context) or set irreversible: true.');
  });

  it('rejects file name and migration name mismatches', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_file_name', `
      import { defineMigration } from '../../../src/index.js';
    ...
    await expect(loadMigrations({ directory }))
      .rejects
      .toThrow('but file name requires "202608180001_file_name"');
  });

  it('rejects duplicate migration names', async () => {
    const directory = await createTempDirectory();
    await writeMigration(directory, '202608180001_duplicate', `
      import { defineMigration } from '../../../src/index.js';
    ...
    await expect(loadMigrations({ directory }))
      .rejects
      .toThrow('Duplicate migration name "202608180001_duplicate"');
  });
});
