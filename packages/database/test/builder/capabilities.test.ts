...
import { type DatabaseCapabilities } from '../../src/adapter.js';
import { CollectionBuilder } from '../../src/builder.js';
import { UnsupportedCapabilityError } from '../../src/capabilities.js';
...
describe('CollectionBuilder capability planning', () => {
  ...
  it('warns and skips unsafe materialized view operations in non-strict mode', async () => {
    ...
    const result = await builder.createMaterializedViewCollection('usersSnapshot', (view) => {
      view.string('email');
  ...
  it('skips unsupported refresh materialized view operations', async () => {
    ...
    const result = await builder.refreshMaterializedViewCollection('usersSnapshot', {
      dryRun: true,
      concurrently: true,
    });
  ...
  it('downgrades unsupported partial regular indexes without dropping the index', async () => {
    ...
    if (alterTable?.type === 'alterTable' && alterTable.operations[0].type === 'addIndex') {
