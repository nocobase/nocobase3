...
describe('CollectionBuilder alterCollection', () => {
  it('collects fluent alter operations', async () => {
    ...
    expect(result.schemaOperations?.[0]).toMatchObject({
      ...
      operations: expect.arrayContaining([
        { type: 'addColumn', column: { name: 'paid_at', type: 'datetime', nullable: true } },
  ...
  it('supports field, index, and constraint shortcut methods', async () => {
    ...
    const alterField = await builder.alterField('orders', 'amount', { precision: 14 }, { dryRun: true });
    const dropField = await builder.dropField('orders', 'legacyStatus', { dryRun: true });
    const addIndex = await builder.addIndex('orders', { fields: ['paidAt'] }, { dryRun: true });
    const dropIndex = await builder.dropIndex('orders', 'idx_orders_paid_at', { dryRun: true });
    const addConstraint = await builder.addConstraint(
      'orders',
      { type: 'unique', fields: ['paidAt'], name: 'uk_orders_paid_at' },
      { dryRun: true },
    );
    const dropConstraint = await builder.dropConstraint('orders', 'uk_orders_paid_at', { dryRun: true });

    expect(addField.operations[0]).toMatchObject({ type: 'addField', collection: 'orders' });
    expect(alterField.operations[0]).toMatchObject({ type: 'alterField', field: 'amount' });
    ...
    expect(addConstraint.schemaOperations?.[0]).toMatchObject({
      operations: [{ type: 'addConstraint', constraint: { type: 'unique', columns: ['paid_at'] } }],
    });
    ...
    expect(result.schemaOperations?.[0]).toMatchObject({
      ...
      operations: expect.arrayContaining([
        expect.objectContaining({
          type: 'addColumn',
          column: expect.objectContaining({ name: 'creator_id', type: 'bigInt' }),
        }),
        expect.objectContaining({
          type: 'addIndex',
          index: expect.objectContaining({ columns: ['creator_id'], name: 'idx_orders_creator_id' }),
        }),
        expect.objectContaining({
          type: 'addConstraint',
