...
import { KnexSchemaAdapter } from '../../src/adapters/knex.js';
import type { SchemaOperation } from '../../src/types.js';
...
describe('KnexSchemaAdapter', () => {
  ...
  it('compiles predicate filters for regular indexes and unique constraints', async () => {
    ...
    expect(sql.join('\n')).toContain('create unique index `uk_jobs_account_program`');
  ...
  it('compiles raw views and structured filters with supported operators', async () => {
    ...
    expect(rawSql.join('\n')).toContain('select first_name from users where age >');
  });
  ...
  it('compiles named SQLite primary constraints without object option artifacts', async () => {
    ...
    const output = sql.join('\n');
    expect(output).toContain('constraint `jobs_primary_key` primary key (`email`)');
  ...
  it('compiles standard alter table operations for PostgreSQL', async () => {
    ...
    const output = sql.join('\n');
    expect(output).toContain('alter table "orders" drop column "legacy_status"');
    expect(output).toContain('foreign key ("customer_id") references "customers" ("id")');
    expect(output).toContain('on update RESTRICT on delete CASCADE');
    expect(output).toContain('alter table "orders" drop constraint "fk_orders_customer_id"');
  });
