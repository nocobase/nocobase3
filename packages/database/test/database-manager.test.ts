...
import { createDatabaseManager, defineDatabase, resolveDatabaseCapabilities } from '../src/database.js';
...
describe('DatabaseManager', () => {
  ...
  it('executes query adapter operations against the selected connection', async () => {
    ...
    try {
      ...
      await db.query('sqlite').table('orders').insert({ status: 'paid' });
      await expect(db.query('sqlite').table('orders').select('status').where('status', 'paid')).resolves.toEqual([
        { status: 'paid' },
      ]);
    } finally {
      await db.destroy();
  ...
  it('runs builder and query operations inside one transaction connection', async () => {
    ...
    try {
      await db.transaction(async (connection) => {
        ...
        await connection.query.table('orders').insert({ status: 'paid' });
      }, 'sqlite');

      await expect(db.query('sqlite').table('orders').select('status')).resolves.toEqual([
        { status: 'paid' },
      ]);
    } finally {
      await db.destroy();
