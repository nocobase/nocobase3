...
describeIntegrationDatabases('collection alteration', (context) => {
  it('adds and drops fields on an existing collection', async () => {
    ...
    expect(await context.db.schema.hasColumn(context.table('users'), 'name')).toBe(false);
    expect(await context.db.schema.hasColumn(context.table('users'), 'first_name')).toBe(true);
    expect(await context.db.schema.hasColumn(context.table('users'), 'last_name')).toBe(true);
  });
  ...
  it('uses field, index, and constraint shortcut APIs', async () => {
    ...
    expect(await context.db.schema.hasColumn(context.table('orders'), 'paid_at')).toBe(true);
    ...
    await context.db(context.table('orders')).insert({ amount: 1, paid_at: '2026-08-13 10:00:00' });
    await expectUniqueViolation(
      context.db(context.table('orders')).insert({ amount: 2, paid_at: '2026-08-13 10:00:00' }),
    );

    await context.builder.dropIndex('orders', paidAtIndexName);
    expect((await listIndexes(context, context.table('orders'))).map((index) => index.name)).not.toContain(
      paidAtIndexName,
    );
  });
});
