...
describeIntegrationDatabases('view collections', (context) => {
  it('creates and replaces views', async () => {
    ...
    await context.builder.createViewCollection('adultUsers', (view) => {
      ...
      view.as((query) => query.from('users').select('firstName').where('age', '>', 18));
    });

    await expect(context.db(adultUsersView).select('*')).resolves.toEqual([{ first_name: 'Ada' }]);

    await context.builder.replaceViewCollection('adultUsers', (view) => {
      view.tableName(adultUsersView);
      view.string('firstName', { columnName: 'first_name' });
      view.as((query) => query.from('users').select('firstName').where('age', '>', 16));
    });

    await expect(context.db(adultUsersView).select('*').orderBy('first_name')).resolves.toEqual([
      { first_name: 'Ada' },
      { first_name: 'Tim' },
    ]);
  });
