...
import { describeIntegrationDatabases, useIntegrationDatabase } from '../helpers.js';
...
describeIntegrationDatabases('capability warnings', (context) => {
  it('downgrades safe unsupported capabilities without failing real DDL', async () => {
    const result = await context.builder.createCollection('capabilityEvents', (collection) => {
    ...
    expect(await context.db.schema.hasTable(context.table('capabilityEvents'))).toBe(true);
    expect(await context.db.schema.hasColumn(context.table('capabilityEvents'), 'ip_address')).toBe(true);
  ...
  it('handles partial unique constraints according to dialect capability', async () => {
    const result = await context.builder.createCollection('partialUniqueJobs', (collection) => {
      collection.increments('id');
    ...
    expect(await context.db.schema.hasTable(context.table('partialUniqueJobs'))).toBe(true);
    ...
    if (context.spec.dialect === 'mysql') {
      ...
      await expect(context.db(context.table('partialUniqueJobs')).insert({
        account_id: 1,
        program_id: 1,
...
describe('capability warnings [sqlite materialized view]', () => {
  ...
  it('warns and skips unsupported materialized views without throwing', async () => {
    const result = await context.builder.createMaterializedViewCollection('usersSnapshot', (view) => {
      view.string('email');
    ...
    expect(await context.db.schema.hasTable(context.table('usersSnapshot'))).toBe(false);
  });
});
