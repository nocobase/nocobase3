...
describeIntegrationDatabases('collection rename', (context) => {
  it('renames only collection metadata by default without renaming the backing table', async () => {
    ...
    await context.db(context.table('users')).insert({ email: 'ada@example.com' });
    ...
    expect(await context.db.schema.hasTable(context.table('appUsers'))).toBe(false);
    expect(await context.metadataStore.getCollection('appUsers')).toMatchObject({
      name: 'appUsers',
      tableName: context.table('users'),
    });
    await expect(context.db(context.table('users')).select('email')).resolves.toEqual([
  ...
  it('renames the backing table by convention when requested', async () => {
    ...
    await context.db(context.table('users')).insert({ email: 'ada@example.com' });
    await context.builder.renameCollection('users', 'appUsers', { renameTable: true });

    expect(await context.db.schema.hasTable(context.table('users'))).toBe(false);
    expect(await context.db.schema.hasTable(context.table('appUsers'))).toBe(true);
    await expect(context.db(context.table('appUsers')).select('email')).resolves.toEqual([
      { email: 'ada@example.com' },
    ]);
  });
  ...
  it('renames the backing table to an explicit physical table name', async () => {
    ...
    expect(await context.db.schema.hasTable(context.identifier('legacy_users'))).toBe(false);
