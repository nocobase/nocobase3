...
describeIntegrationDatabases('query where', (context) => {
  ...
  it('supports object filters, eb.ref, eb.val, whereRef operands, and clearWhere', async () => {
    ...
    await context.database.query()
      ...
      .values([
        { tenantId: 'tenant-a', orderNo: 'SO-001', status: 'paid', amount: 120, amountCopy: 120, minAmount: 100 },
        { tenantId: 'tenant-a', orderNo: 'SO-002', status: 'paid', amount: 240, amountCopy: 240, minAmount: 300 },
        { tenantId: 'tenant-a', orderNo: 'SO-003', status: 'completed', amount: 360, amountCopy: 300, minAmount: 100 },
        { tenantId: 'tenant-b', orderNo: 'SO-004', status: 'paid', amount: 480, amountCopy: 480, minAmount: 100 },
      ])
      .execute();
    ...
    await expect(base.pluck<string>('orderNo')).resolves.toEqual(['SO-001', 'SO-002']);
    await expect(base.clearWhere().pluck<string>('orderNo')).resolves.toEqual(['SO-001', 'SO-002']);
    await expect(base.clearWhere().clearLimit().pluck<string>('orderNo')).resolves.toEqual([
      'SO-001',
      'SO-002',
      'SO-003',
      'SO-004',
