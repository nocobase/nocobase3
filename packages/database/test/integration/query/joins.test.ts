...
import { describeIntegrationDatabases, type IntegrationTestContext } from '../helpers.js';
...
describeIntegrationDatabases('query joins', (context) => {
  it('supports portable joins with reference and callback conditions', async () => {
    ...
    await context.database.query().insertInto(customersTable).values([
      { name: 'Ada', status: 'active' },
      { name: 'Grace', status: 'active' },
      { name: 'Linus', status: 'disabled' },
    ]).execute();
    await context.database.query().insertInto(ordersTable).values([
  ...
  it('supports rightJoin, crossJoin, and clearJoins', async () => {
    ...
    const crossRows = await context.database.query()
      .selectFrom(`${ordersTable} as o`)
  ...
  it('supports Kysely-style callback joins with grouped OR expressions', async () => {
    ...
    await context.database.query().insertInto(ordersTable).values([
      { customerId: 1, fallbackCustomerId: null, orderNo: 'SO-001', status: 'paid' },
      { customerId: null, fallbackCustomerId: 3, orderNo: 'SO-002', status: 'manual' },
      { customerId: null, fallbackCustomerId: null, orderNo: 'SO-003', status: 'manual' },
      { customerId: 2, fallbackCustomerId: null, orderNo: 'SO-004', status: 'manual' },
    ]).execute();
...
async function createJoinCollections(context: IntegrationTestContext): Promise<void> {
