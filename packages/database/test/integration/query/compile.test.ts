...
describeIntegrationDatabases('query compile', (context) => {
  it('compiles select SQL with physical identifiers and without connection tablePrefix', async () => {
    const compiled = context.database.query()
      .selectFrom('queryOrders')
      .select(['orderNo', 'createdAt'])
      .where('status', '=', 'paid')
      .compile();

    expect(compiled.sql).toContain('query_orders');
    expect(compiled.sql).not.toContain(context.prefix);
  ...
  it('compiles insert, update, and delete operations', async () => {
    ...
    const deleteSql = context.database.query()
      .deleteFrom('queryOrders')
      .where('orderNo', '=', 'SO-001')
      .compile();
    expect(deleteSql.sql).toContain('query_orders');
