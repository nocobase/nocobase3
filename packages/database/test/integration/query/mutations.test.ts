...
describeIntegrationDatabases('query mutations', (context) => {
  it('inserts, updates, and deletes rows against a real connection', async () => {
    ...
    const insertResult = await context.database.query()
  ...
  it('requires values, set data, or allowAllRows where appropriate', async () => {
    ...
    await expect(
      context.database.query()
        ...
        .set({ status: 'archived' })
        .execute(),
    ).rejects.toThrow('updateTable().execute() requires where() or allowAllRows().');
    ...
    await expect(
      context.database.query()
        .deleteFrom(ordersTable)
        .execute(),
    ).rejects.toThrow('deleteFrom().execute() requires where() or allowAllRows().');
  ...
  it('updates and deletes with expression callbacks and immutable clearWhere', async () => {
    ...
    const updatePaid = context.database.query()
      .updateTable(ordersTable)
      .set({ status: 'settled' })
      .where('status', '=', 'draft')
      .clearWhere()
    ...
    const deleteDrafts = context.database.query()
      .deleteFrom(ordersTable)
