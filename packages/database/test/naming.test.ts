...
import { DefaultNamingStrategy, snakeCase } from '../src/naming.js';
...
describe('DefaultNamingStrategy', () => {
  it('normalizes collection and field names to database identifiers', () => {
    ...
    expect(naming.indexName('orders', ['customer_id', 'status'])).toBe('idx_orders_customer_id_status');
    expect(naming.foreignKeyName('orders', ['customer_id'], 'customers')).toBe('fk_orders_customer_id_customers');
  });
  ...
  it('supports explicit naming options for table prefix and non-underscored identifiers', () => {
    ...
    expect(prefixed.collectionToTableName('orderItems')).toBe('tbl_order_items');
  ...
  it('truncates generated index and foreign key names with a stable hash', () => {
    ...
    expect(indexName).toMatch(/^idx_cbt_123456_abcdef_very_long_order_items_table/);
    expect(foreignKeyName).toMatch(/^fk_cbt_123456_abcdef_very_long_order_items_table/);
    expect(indexName).toBe(naming.indexName(
      'cbt_123456_abcdef_very_long_order_items_table',
      ['very_long_customer_identifier_column', 'very_long_status_column'],
    ));
  });
