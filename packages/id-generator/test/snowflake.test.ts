...
describe('SnowflakeIdGenerator', () => {
  ...
  it('accepts an epoch in milliseconds', () => {
    ...
    expect(generator.parse(generator.generate()).timestamp).toBe(SNOWFLAKE_EPOCH_SECONDS + 10);
  });

  it('rejects invalid worker IDs', () => {
    expect(() => new SnowflakeIdGenerator({ workerId: -1 })).toThrow('between 0 and 31');
    expect(() => new SnowflakeIdGenerator({ workerId: 32 })).toThrow('between 0 and 31');
    expect(() => new SnowflakeIdGenerator({ workerId: 1.5 })).toThrow('integer');
  });

  it('fails when the clock moves backwards', () => {
    let now = (SNOWFLAKE_EPOCH_SECONDS + 20) * 1_000;
    const generator = new SnowflakeIdGenerator({ workerId: 0, clock: () => now });
    generator.generate();
    now -= 1_000;
  ...
  it('rejects invalid IDs', () => {
    ...
    expect(() => generator.parse(Number.MAX_SAFE_INTEGER + 1)).toThrow('safe integer');
  });
});
