export const SNOWFLAKE_EPOCH_SECONDS = 1_605_024_000;
export const SNOWFLAKE_MAX_WORKER_ID = 31;
export const SNOWFLAKE_MAX_SEQUENCE = 65_535;
...
export class SnowflakeIdGenerator implements IdGenerator<number> {
  ...
  constructor(options: SnowflakeIdGeneratorOptions) {
    if (!options || !Number.isSafeInteger(options.workerId)) {
      throw new Error('Snowflake workerId must be an integer between 0 and 31.');
    }
    if (options.workerId < 0 || options.workerId > SNOWFLAKE_MAX_WORKER_ID) {
      throw new Error(`Snowflake workerId must be between 0 and ${SNOWFLAKE_MAX_WORKER_ID}.`);
    }
  ...
  generate(): number {
    ...
    const id = timestampOffset * WORKER_COUNT * SEQUENCE_COUNT
      + this.workerId * SEQUENCE_COUNT
      + this.sequence;
    if (!Number.isSafeInteger(id)) {
      throw new Error('Snowflake ID exceeds the JavaScript safe integer range.');
    }
  ...
  parse(id: number | string): SnowflakeIdParts {
    ...
    if (typeof id === 'string') {
      if (id.trim() === '') {
        throw new Error('Snowflake ID must be a non-negative JavaScript safe integer.');
      }
      numericId = Number(id);
    } else {
      numericId = id;
    ...
    const timestampOffset = Math.floor(numericId / (WORKER_COUNT * SEQUENCE_COUNT));
  ...
  private timestamp(): number {
    ...
    if (!Number.isFinite(now) || now < 0) {
      throw new Error('Snowflake clock must return a non-negative Unix timestamp in milliseconds.');
    }
