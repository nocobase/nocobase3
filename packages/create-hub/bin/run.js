...
const useDist =
  process.env.NOCOBASE_CREATE_HUB_USE_DIST === '1' || !existsSync(srcEntry);
const entry = useDist ? '../dist/create.js' : '../src/create.ts';
const manifest = JSON.parse(
  readFileSync(path.join(root, 'package.json'), 'utf8'),
);
const { createHub } = await import(entry);

const exitCode = await createHub({
  argv: process.argv.slice(2),
  binary: 'pnpm create @nocobase/hub',
