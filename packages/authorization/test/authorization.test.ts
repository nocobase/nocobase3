...
function createAuthorization(relationWritable = true) {
        ...
        permissions: [
          {
            ...
            actions: [
              ...
              {
                action: 'update',
                inputFields: relationWritable ? ['amount', 'customer'] : ['amount'],
  ...
  authorization.resources.register({
    ...
    fields: {
      id: scalar(), name: scalar(), email: scalar(), ownerId: scalar(), createdById: scalar(),
    },
  ...
  authorization.resources.register({
    ...
    fields: {
      id: scalar(), amount: scalar(), ownerId: scalar(), approvalComment: scalar(), status: scalar(),
...
describe('Authorization', () => {
  it('unions fields and Record Access for the same Action', async () => {
    const plan = await createAuthorization().plan({ id: 'bob' }, {
      resource: 'customers',
      action: 'read',
      fields: { output: ['name', 'email'] },
    });
    expect(plan.allowed).toBe(true);
    expect(plan.fields?.output).toEqual(['id', 'name', 'ownerId', 'email', 'createdById']);
  ...
  it('builds requested Action plans together for repository batch capability checks', async () => {
    const plans = await createAuthorization().planActions({ id: 'alice' }, {
      resource: 'orders',
  ...
  it('does not allow unreadable fields in filters, sorting, or grouping', async () => {
    ...
    for (const fields of [
      ...
      await expect(authorization.can({ id: 'alice' }, {
        resource: 'orders', action: 'read', fields, record: { ownerId: 'alice' },
      })).resolves.toBe(false);
    }
  });
  ...
  it('requires source, relation, and target permissions for traverse', async () => {
    const authorization = createAuthorization();
    await expect(authorization.canRelation({ id: 'alice' }, {
      resource: 'orders', field: 'customer', action: 'traverse',
      sourceRecord: { ownerId: 'alice' }, targetRecord: { id: 'customer-1', createdById: 'alice' },
    })).resolves.toBe(true);
    await expect(authorization.canRelation({ id: 'alice' }, {
      resource: 'orders', field: 'customer', action: 'traverse',
      sourceRecord: { ownerId: 'bob' }, targetRecord: { id: 'customer-1', createdById: 'alice' },
    })).resolves.toBe(false);
  });
  ...
  it('intersects connect target access with the target Read plan', async () => {
    const authorization = createAuthorization();
    await expect(authorization.canRelation({ id: 'alice' }, {
      resource: 'orders', field: 'customer', action: 'connect',
      sourceRecord: { ownerId: 'alice' }, targetRecord: { id: 'customer-1', createdById: 'alice' },
    })).resolves.toBe(true);
    await expect(authorization.canRelation({ id: 'alice' }, {
      resource: 'orders', field: 'customer', action: 'connect',
      sourceRecord: { ownerId: 'alice' }, targetRecord: { id: 'customer-2', createdById: 'bob' },
    })).resolves.toBe(false);
  });
  ...
  it('uses the same writable relation field permission for connect and disconnect', async () => {
    const authorization = createAuthorization();
    await expect(authorization.canRelation({ id: 'alice' }, {
      resource: 'orders', field: 'customer', action: 'disconnect',
      sourceRecord: { ownerId: 'alice' }, targetRecord: { id: 'customer-1' },
    })).resolves.toBe(true);
  });
    ...
    const store = new MemoryAuthorizationStore({
      permissionSets: [{
        ...
        permissions: [{
          resource: 'logs',
          actions: [{
            action: 'read', outputFields: '*', recordScope: [{ policy: 'recordsIOwn' }],
          }],
        }],
      }],
      assignments: [{ id: 'a1', subject: { type: 'user', id: 'alice' }, target: { type: 'permissionSet', key: 'reader' } }],
    });
    const authorization = new Authorization({ store });
    authorization.resources.register({ name: 'logs', actions: ['read'], fields: { id: scalar() } });
    const plan = await authorization.plan({ id: 'alice' }, { resource: 'logs', action: 'read' });
