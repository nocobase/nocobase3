import { createDatabaseManager, type DatabaseConnection } from '@nocobase/database';
...
describe('DatabaseAuthorizationStore', () => {
  ...
  beforeAll(async () => {
    ...
    await connection.query.insertInto('authzPermissionSets').values({
      id: 'ps1', key: 'order-reader', title: 'Order reader',
      createdAt: now, updatedAt: now,
    }).execute();
    await connection.query.insertInto('authzObjectPermissions').values({
      id: 'op1', permissionSetId: 'ps1', resource: 'orders',
    ...
    await connection.query.insertInto('authzAssignments').values({
      id: 'a1', subjectType: 'user', subjectId: 'alice', targetType: 'permissionSet', targetId: 'ps1',
      startsAt: null, expiresAt: null, createdAt: now, updatedAt: now,
    }).execute();
    await connection.query.insertInto('authzOrganizationWideDefaults').values({
      id: 'owd1', resource: 'orders', access: 'private', createdAt: now, updatedAt: now,
    }).execute();
    await connection.query.insertInto('authzSharingRules').values({
      id: 'share1', key: 'share-order-1', title: 'Share order 1', resource: 'orders',
      actions: JSON.stringify(['read']),
      subjects: JSON.stringify([{ type: 'user', id: 'alice' }]),
      recordType: 'records', scopes: null, startsAt: null, expiresAt: null, reason: null,
      createdAt: now, updatedAt: now,
    }).execute();
    await connection.query.insertInto('authzSharingRuleRecords').values({
      id: 'share-record1', sharingRuleId: 'share1', recordId: 'order-1', createdAt: now,
    }).execute();
  });
  ...
  it('loads authorization data through the Database Query API', async () => {
    const authorization = new Authorization({ store: new DatabaseAuthorizationStore(database.connection()) });
    ...
    const plan = await authorization.plan({ id: 'alice' }, { resource: 'orders', action: 'read' });
    expect(plan.allowed).toBe(true);
    expect(plan.filter?.root.items).toEqual(expect.arrayContaining([
  ...
  it('loads explicit record shares from the normalized child collection', async () => {
    ...
    authorization.resources.register({
      ...
      fields: { id: { type: 'scalar' }, name: { type: 'scalar' }, ownerId: { type: 'scalar' } },
    ...
    await expect(authorization.can({ id: 'alice' }, {
      resource: 'orders', action: 'read', record: { id: 'order-1', ownerId: 'bob' },
    })).resolves.toBe(true);
    await expect(authorization.can({ id: 'alice' }, {
      resource: 'orders', action: 'read', record: { id: 'order-2', ownerId: 'bob' },
    })).resolves.toBe(false);
  });

  it('creates application relations without physical foreign keys', async () => {
    const knex = await database.connection().client<Knex>();
    for (const table of ['authz_object_permissions', 'authz_permission_set_group_items', 'authz_sharing_rule_records']) {
...
describe('Authorization collection naming', () => {
  it('supports underscored: false', async () => {
    const database = createDatabaseManager({
      ...
      connections: {
        main: {
          driver: 'knex', client: 'better-sqlite3', connection: { filename: ':memory:' },
          useNullAsDefault: true, naming: { underscored: false },
        },
      },
    });
    try {
      ...
      await createCollections(connection);
      await connection.query.insertInto('authzPermissionSets').values({
        id: 'ps1', key: 'reader', title: 'Reader',
        createdAt: new Date(), updatedAt: new Date(),
      }).execute();
      await expect(connection.query.selectFrom('authzPermissionSets').select('key').executeTakeFirst())
