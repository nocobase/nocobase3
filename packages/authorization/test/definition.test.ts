...
describe('Authorization definitions', () => {
  it('normalizes Object DSL into a business-only definition', () => {
    ...
    expect(authz.describe().resources.map((resource) => resource.name)).toEqual(['customers', 'orders']);
    expect(authz.describe().policies.map((policy) => policy.key)).toContain('recordsIOwn');
  });
  ...
  it('returns structured diagnostics using the instance catalog', () => {
    ...
    definition.permissionSets[0]!.permissions[0]!.actions[0]!.outputFields = ['missing'];
    definition.permissionSetGroups = [{ key: 'sales', permissionSets: ['missing-set'] }];
    ...
    expect(result.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        code: 'UNKNOWN_FIELD',
        path: ['permissionSets', 0, 'permissions', 0, 'actions', 0, 'outputFields', 0],
      }),
      expect.objectContaining({
  ...
  it('validates Sharing and Restriction Rule scopes against their intended policy usage', () => {
    ...
    expect(authz.validate(definition)).toEqual({ valid: true, diagnostics: [] });
    ...
    expect(result.valid).toBe(false);
    expect(result.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({ code: 'EMPTY_VALUE', path: ['sharingRules', 0, 'actions'] }),
      expect.objectContaining({ code: 'EMPTY_VALUE', path: ['sharingRules', 0, 'subjects'] }),
      expect.objectContaining({ code: 'UNKNOWN_POLICY', path: ['sharingRules', 0, 'records', 'scopes', 0, 'policy'] }),
      expect.objectContaining({ code: 'UNKNOWN_RESOURCE', path: ['restrictionRules', 0, 'resource'] }),
      expect.objectContaining({ code: 'EMPTY_VALUE', path: ['restrictionRules', 0, 'scopes'] }),
    ]));
  });
  ...
  it('produces a business-only operation plan without applying it', () => {
    ...
    expect(authz.diff(current, desired)).toEqual([
      ...
      { type: 'upsertSharingRule', key: 'share-orders', value: desired.sharingRules[0] },
    ]);
  });
});
