...
function createAuthorization(
  ...
  options: Omit<MemoryAuthorizationStoreOptions, 'permissionSets' | 'assignments'> = {},
...
function action(actionName: string, recordScope?: ActionPermission['recordScope']): ActionPermission {
  return {
    action: actionName,
    inputFields: actionName === 'create' || actionName === 'update' ? ['amount'] : [],
...
describe('record access semantics', () => {
  it('never lets OWD create a missing Object Permission capability', async () => {
    ...
    await expect(authorization.can(principal, { resource: 'orders', action: 'update' })).resolves.toBe(false);
  });

  it('denies a private resource with no positive record scope', async () => {
    const authorization = createAuthorization([action('read')]);
    await expect(authorization.can(principal, { resource: 'orders', action: 'read' })).resolves.toBe(false);
  });

  it('adds implicit owner access only when the resource declares owner semantics', async () => {
    const authorization = createAuthorization([action('read')], {}, { owner: 'ownerId' });
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'read', record: { ownerId: 'alice' },
    })).resolves.toBe(true);
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'read', record: { ownerId: 'bob' },
    })).resolves.toBe(false);
  });
  ...
  it('maps OWD to standard record actions without granting delete or custom actions', async () => {
    ...
    await expect(readOnly.can(principal, { resource: 'orders', action: 'read' })).resolves.toBe(true);
    await expect(readOnly.can(principal, { resource: 'orders', action: 'update' })).resolves.toBe(false);
    ...
    await expect(readWrite.can(principal, { resource: 'orders', action: 'read' })).resolves.toBe(true);
    await expect(readWrite.can(principal, { resource: 'orders', action: 'update' })).resolves.toBe(true);
    await expect(readWrite.can(principal, { resource: 'orders', action: 'delete' })).resolves.toBe(false);
    await expect(readWrite.can(principal, { resource: 'orders', action: 'approve' })).resolves.toBe(false);
  });
  ...
  it('authorizes create from capability and input fields without an existing-record filter', async () => {
    ...
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'create', fields: { input: ['amount'] },
    })).resolves.toBe(true);
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'create', fields: { input: ['ownerId'] },
    })).resolves.toBe(false);
  });
  ...
  it('unions direct, criteria, and explicit-record sharing scopes and ignores expired rules', async () => {
      ...
      sharingRules: [
        {
          ...
          records: {
            ...
            scopes: [{
              ...
              params: { filter: filter('orders', condition('department', '$eq', 'sales')) },
            }],
          },
        },
        {
    ...
    for (const record of [
      ...
      await expect(authorization.can(principal, { resource: 'orders', action: 'read', record, now })).resolves.toBe(true);
    }
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'read', record: { id: 'expired-order', ownerId: 'bob', department: 'support' }, now,
    })).resolves.toBe(false);
  });
  ...
  it('intersects every positive grant with Restriction Rules and never treats a restriction as a grant', async () => {
    ...
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'read', record: { department: 'sales' },
    })).resolves.toBe(true);
    await expect(authorization.can(principal, {
      resource: 'orders', action: 'read', record: { department: 'support' },
    })).resolves.toBe(false);

    const restrictionOnly = createAuthorization([action('read')], { restrictionRules });
    await expect(restrictionOnly.can(principal, {
      resource: 'orders', action: 'read', record: { department: 'sales' },
    })).resolves.toBe(false);
  });
});
