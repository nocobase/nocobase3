...
export class DatabaseAuthorizationStore implements AuthorizationStore {
  ...
  async getPermissionSet(key: string): Promise<PermissionSet | undefined> {
    const set = await this.connection.query.selectFrom('authzPermissionSets')
      .select(['id', 'key', 'title'])
      .where('key', '=', key)
      .executeTakeFirst();
    ...
    return {
      id: String(set.id),
      key: String(set.key),
      title: set.title == null ? undefined : String(set.title),
  ...
  async getPermissionSetGroup(key: string): Promise<PermissionSetGroup | undefined> {
    const group = await this.connection.query.selectFrom('authzPermissionSetGroups')
      .select(['id', 'key', 'title'])
      .where('key', '=', key)
      .executeTakeFirst();
    ...
    return {
      ...
      title: group.title == null ? undefined : String(group.title),
      permissionSets: keys,
    };
  }

  async getOrganizationWideDefault(resource: string): Promise<OrganizationWideDefault | undefined> {
    const row = await this.connection.query.selectFrom('authzOrganizationWideDefaults')
      .select('access')
  ...
  async findSharingRules(
    ...
    const subjectKeys = new Set(subjects.map((subject) => `${subject.type}:${subject.id}`));
    const rows = await this.connection.query.selectFrom('authzSharingRules')
      .select(['id', 'key', 'title', 'resource', 'actions', 'subjects', 'recordType', 'scopes', 'startsAt', 'expiresAt', 'reason'])
      .where('resource', '=', resource)
      .execute();
    ...
    return candidates.map(({ row, actions, ruleSubjects, startsAt, expiresAt }) => {
      ...
      if (row.recordType === 'criteria') {
        ...
        throw new Error(`Unknown Sharing Rule record type: ${String(row.recordType)}`);
      }
      return {
        id: String(row.id),
        key: String(row.key),
  ...
  async findRestrictionRules(
    ...
    const subjectKeys = new Set(subjects.map((subject) => `${subject.type}:${subject.id}`));
    const rows = await this.connection.query.selectFrom('authzRestrictionRules')
      .select(['id', 'key', 'title', 'resource', 'actions', 'subjects', 'scopes'])
      .where('resource', '=', resource)
      .execute();
