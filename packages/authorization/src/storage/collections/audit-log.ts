...
export function createAuditLogCollection(builder: CollectionBuilder) {
  return builder.createCollection('authzAuditLogs', (collection) => {
    ...
    collection.index(['event', 'createdAt'], { name: 'idx_authz_audit_logs_event_created' });
    collection.index(['actorType', 'actorId'], { name: 'idx_authz_audit_logs_actor' });
  });
}
