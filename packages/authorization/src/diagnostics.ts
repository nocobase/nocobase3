import type { AuthorizationCatalog, AuthorizationDefinition, PolicyDescriptor } from './definition.js';
import type {
  ActionPermission,
  RecordScope,
  ResourceDefinition,
...
function duplicateDiagnostics<T>(
  ...
  values.forEach((value, index) => {
    ...
    if (first !== undefined) {
      add({ severity: 'error', code: 'DUPLICATE_KEY', path: [...path, index], message: `Duplicate key "${valueKey}"; first declared at index ${first}` });
    } else {
...
function policyMap(catalog: AuthorizationCatalog): Map<string, PolicyDescriptor> {
...
function validateScope(
  ...
  if (!policy) {
    add({ severity: 'error', code: 'UNKNOWN_POLICY', path: [...path, 'policy'], message: `Unknown policy "${scope.policy}"` });
  } else if (!policy.appliesTo.includes(usage)) {
    add({ severity: 'error', code: 'INVALID_POLICY_SCOPE', path: [...path, 'policy'], message: `Policy "${scope.policy}" cannot be used for ${usage}` });
  }
}
...
function validateAction(
  ...
  if (!resource.actions.includes(action.action)) {
    add({ severity: 'error', code: 'UNKNOWN_ACTION', path: [...actionPath, 'action'], message: `Unknown action "${action.action}" on resource "${resource.name}"` });
  }
  ...
  for (const [scopeIndex, scope] of (action.recordScope ?? []).entries()) {
    validateScope(scope, [...actionPath, 'recordScope', scopeIndex], 'recordScope', policies, add);
  }
}
...
export function validateAuthorization(
  ...
  const add = (diagnostic: AuthorizationDiagnostic) => diagnostics.push(diagnostic);
  const resources = new Map(catalog.resources.map((resource) => [resource.name, resource]));
  ...
  duplicateDiagnostics(definition.permissionSets, (item) => item.key, ['permissionSets'], add);
  definition.permissionSets.forEach((set, setIndex) => {
    ...
    set.permissions.forEach((permission, permissionIndex) => {
      ...
      if (!resource) {
        add({ severity: 'error', code: 'UNKNOWN_RESOURCE', path: [...permissionPath, 'resource'], message: `Unknown resource "${permission.resource}"` });
        return;
      }
  ...
  duplicateDiagnostics(definition.permissionSetGroups, (item) => item.key, ['permissionSetGroups'], add);
  ...
  for (const resource of Object.keys(definition.organizationWideDefaults)) {
    ...
    if (!resourceDefinition) {
      add({ severity: 'error', code: 'UNKNOWN_RESOURCE', path: ['organizationWideDefaults', resource], message: `Unknown resource "${resource}"` });
    }
  }

  duplicateDiagnostics(definition.sharingRules, (item) => item.key, ['sharingRules'], add);
  definition.sharingRules.forEach((rule, ruleIndex) => {
    ...
    if (!resourceDefinition) {
      add({ severity: 'error', code: 'UNKNOWN_RESOURCE', path: [...rulePath, 'resource'], message: `Unknown resource "${rule.resource}"` });
    } else {
    ...
    if (rule.records.type === 'criteria') {
      ...
      add({ severity: 'error', code: 'EMPTY_VALUE', path: [...rulePath, 'records', 'ids'], message: 'At least one shared record id is required' });
    }
  });

  duplicateDiagnostics(definition.restrictionRules, (item) => item.key, ['restrictionRules'], add);
