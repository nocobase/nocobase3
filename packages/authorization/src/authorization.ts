...
function fieldsAllowed(requested: readonly string[] | undefined, allowed: '*' | readonly string[]): boolean {
  return !requested || allowed === '*' || requested.every((field) => allowed.includes(field));
}

export class Authorization {
  readonly resources = new ResourceRegistry();
  readonly policies = new PolicyRegistry();
  ...
  define(input: AuthorizationDefinitionInput | AuthorizationDefinitionCallback = {}): AuthorizationDefinition {
  ...
  diff(current: AuthorizationDefinition, desired: AuthorizationDefinition): AuthorizationOperation[] {
    return diffAuthorization(current, desired);
  }

  async plan(principal: Principal, request: AuthorizationRequest): Promise<AuthorizationPlan> {
  ...
  async planActions(
    ...
    const plans = await Promise.all(uniqueActions.map(async (action) => [
      action,
      await this.planInternal(principal, {
  ...
  private async planInternal(
    ...
    if (!resource || !resource.actions.includes(request.action)) {
      return this.denied(reasons, 'UNKNOWN_RESOURCE_OR_ACTION', `Unknown resource or action: ${request.resource}.${request.action}`);
    }

    try {
      const now = context?.now ?? request.now ?? new Date();
      const subjects = context?.subjects ?? this.resolveSubjects(principal);
      const sets = context?.sets ?? await this.resolvePermissionSets(subjects, now);
      const grants = sets.flatMap((set) => set.permissions
      ...
      if (!grants.length) {
        return this.denied(reasons, 'NO_OBJECT_PERMISSION', `No Object Permission allows ${resource.name}.${request.action}`);
      }

      this.validateActionPermissions(resource, grants.map(({ action }) => action));
      for (const { set } of grants) {
        reasons.push({ type: 'permissionSet', key: set.key, message: `${set.key} allows ${resource.name}.${request.action}` });
      }

      const input = unionFields(grants.map(({ action }) => action.inputFields));
      const output = unionFields(grants.map(({ action }) => action.outputFields));
      if (!this.requestedFieldsAllowed(request.fields, input, output)) {
        return this.denied(reasons, 'FIELD_NOT_ALLOWED', 'One or more input, output, filter, sort, or group fields are not allowed');
      }
      reasons.push({ type: 'fieldSecurity', key: request.action, message: `Field permissions resolved for ${request.action}` });
      ...
      await this.addSharingFilters(principal, subjects, resource, request.action, now, recordFilters, reasons);
      if (!recordFilters.length) {
        return this.denied(reasons, 'NO_RECORD_ACCESS', `No Record Access allows ${resource.name}.${request.action}`);
      }
  ...
  async planRelation(principal: Principal, request: RelationAuthorizationRequest): Promise<RelationAuthorizationPlan> {
    const reasons: AuthorizationReason[] = [];
    try {
      ...
      if (!targetPlan.allowed) {
        return { allowed: false, sourceFilter: sourcePlan.filter, targetFilter: targetPlan.filter, reasons };
      }

      const targetFilters: FilterAst[] = targetPlan.filter ? [targetPlan.filter] : [allRecords(target.name)];
      const targetFilter = andFilters(target.name, targetFilters);
      if (request.targetRecord && !await matchesFilterAsync(
        targetFilter,
        request.targetRecord,
        (node, value) => this.store.matchesFilterMembership(node, value),
      )) {
        reasons.push({ type: 'recordAccess', key: 'relationTargetMismatch', message: 'The target record does not match the effective relation target filter' });
        return { allowed: false, sourceFilter: sourcePlan.filter, targetFilter, reasons };
      }
      return { allowed: true, sourceFilter: sourcePlan.filter, targetFilter, reasons };
    } catch (error) {
      return this.deniedRelation(reasons, 'RELATION_AUTHORIZATION_FAILED', this.errorMessage(error));
    }
  }

  async can(principal: Principal, request: AuthorizationRequest): Promise<boolean> {
    return (await this.plan(principal, request)).allowed;
  }

  async canRelation(principal: Principal, request: RelationAuthorizationRequest): Promise<boolean> {
    return (await this.planRelation(principal, request)).allowed;
  }

  explain(principal: Principal, request: AuthorizationRequest): Promise<AuthorizationPlan> {
  ...
  private async resolvePermissionSets(subjects: readonly AssignmentSubject[], now: Date): Promise<PermissionSet[]> {
  ...
  private async compileScopes(
    ...
    for (const scope of scopes) {
      ...
      reasons.push({ type: 'recordAccess', key: scope.policy, message: `${scope.policy} contributes a record scope` });
    }
    return filters;
  }
  ...
  private async addSharingFilters(
    ...
    const rules = await this.store.findSharingRules(subjects, resource.name, action, now);
  ...
  private async compileRestrictionFilters(
    ...
    const rules = await this.store.findRestrictionRules(subjects, resource.name, action);
    const filters: FilterAst[] = [];
    for (const rule of rules) {
  ...
  private async addBaselineFilters(
    ...
    if (
      ...
      reasons.push({ type: 'organizationWideDefault', key: access, message: `${resource.name} is ${access}` });
    }
  ...
  private validateActionPermissions(resource: ResourceDefinition, actions: readonly ActionPermission[]): void {
  ...
  private denied(reasons: AuthorizationReason[], key: string, message: string): AuthorizationPlan {
    return { allowed: false, reasons: [...reasons, { type: 'error', key, message }] };
  }
  ...
  private errorMessage(error: unknown): string {
    return error instanceof Error ? error.message : 'Authorization resolution failed';
  }
}
