...
- For URL-addressable dialogs and drawers, first add a child route, then place the owning page's `Outlet`, and finally render the child with `RouteDialog` or `RouteDrawer`. Use `useRouteOverlay()` for closing. Read the child-routes guide before implementing nested overlays or close guards.
