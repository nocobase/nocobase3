...
describe('app server', () => {
  it('represents the NocoBase application separately from its Hono router', () => {
    const app = createTestApp({
      publicBasePath: '/app-template-examples',
    });
  ...
  it('registers application-owned providers and routes from the runtime', async () => {
    const pluginServiceToken = createServiceToken<string>(
      '@nocobase/app-template-examples/tests/plugin-service',
    );
    ...
    > {
      public readonly name: string =
        '@nocobase/app-template-examples/runtime-test';
  ...
  it('loads and explicitly reloads plugin config from the selected YAML file', async () => {
    const configDir = mkdtempSync(
      path.join(tmpdir(), 'nocobase-app-template-examples-config-'),
    );
  ...
  it('serves embedded production SPA routes from the stripped app-host path', async () => {
    const root = mkdtempSync(
      path.join(tmpdir(), 'nocobase-app-template-examples-embedded-client-'),
    );
    tempDirs.push(root);
    writeFileSync(
      path.join(root, 'index.html'),
      '<div id="root"></div><script type="module" src="/app-template-examples/assets/index.js"></script>',
    );
    ...
    expect(response.status).toBe(200);
    expect(html).toContain('window.APP_BASE_PATH = "/app-template-examples/";');
    expect(html).toContain(
      'window.NOCOBASE_API_URL = "/app-template-examples/api";',
    );
  });
  ...
  it('injects browser runtime config when serving the production SPA index', async () => {
    const root = mkdtempSync(
      path.join(tmpdir(), 'nocobase-app-template-examples-client-'),
    );
    tempDirs.push(root);
  ...
  it('serves production SPA assets before the SPA fallback', async () => {
    ...
    writeFileSync(
      ...
      'console.log("app-template-examples asset");',
    );
  ...
  it('does not return the SPA index for missing production SPA assets', async () => {
    ...
    writeFileSync(
      path.join(root, 'index.html'),
      '<main>app-template-examples app</main>',
    );
...
function createTestApp(options: CreateTestAppOptions = {}): TestApp {
  ...
  const configValues = {
    app: {
      name: resolveAppNameFromBasePath(publicBasePath, 'app-template-examples'),
...
function createResolvedTestServerPlugins(
  ...
  return {
    appPackageName: '@nocobase/app-template-examples',
...
function createEmbeddedTestScope(
  ...
  const databaseDir = mkdtempSync(
    path.join(tmpdir(), 'nocobase-app-template-examples-database-'),
  );
...
async function createIsolatedStandaloneServer(
  ...
  const sourceRoot = path.resolve(import.meta.dirname, '../..');
  const databaseDir = mkdtempSync(
    path.join(tmpdir(), 'nocobase-app-template-examples-standalone-database-'),
  );
  tempDirs.push(databaseDir);
