...
describe('assetUrl', () => {
  it('resolves public asset names under the current app assets path', () => {
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        APP_BASE_PATH: '/app-template-examples/',
      },
    });

    expect(assetUrl('logo.png')).toBe('/app-template-examples/assets/logo.png');
    expect(assetUrl('/logo.png')).toBe(
      '/app-template-examples/assets/logo.png',
    );
    ...
    expect(assetUrl('/app-template-examples/assets/logo.png')).toBe(
      '/app-template-examples/assets/logo.png',
    );
  });
