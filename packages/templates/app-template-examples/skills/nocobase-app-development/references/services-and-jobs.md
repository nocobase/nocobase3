...
Module defaults are editable under `server/config/`: `ai.ts`, `app.ts`, `auth.ts`, `caching.ts`, `database.ts`, `drive.ts`, `heartbeat.ts`, `i18n.ts`, `logging.ts`, `notification.ts`, `queue.ts`, `server.ts`, `session.ts`, `snowflake.ts`, `spa.ts`, `workflow.ts`. The client defaults are `client/config/app.ts`, `api.ts`, and `auth.ts`.
