...
Edit `server/config/authorization.ts` using `AuthorizationConfig` from `@nocobase/app-plugin-authorization/server`. Permission sets, page and database authorization are built in. Configure the set names with `permissionSets: { rootSet: 'root', defaultSet: 'member' }`. Optional default access, sharing rules and restriction rules come from `@nocobase/app-plugin-authz-default-access/server`, `@nocobase/app-plugin-authz-sharing-rules/server` and `@nocobase/app-plugin-authz-restriction-rules/server`: import each named factory into the config and register its default server and client plugin in the application plugin lists. Each feature owns its management endpoints, settings resource, UI and database migration. Removing a feature requires removing its config factory and runtime registrations together. `authorizationToken` resolves the authorization instance and `permissionSetsToken` its permission-set API; `authz.db` provides database authorization.

Module defaults are editable under `server/config/`: `ai.ts`, `app.ts`, `auth.ts`, `authorization.ts`, `caching.ts`, `database.ts`, `drive.ts`, `heartbeat.ts`, `i18n.ts`, `logging.ts`, `notification.ts`, `queue.ts`, `server.ts`, `session.ts`, `snowflake.ts`, `spa.ts`, `workflow.ts`. The client defaults are `client/config/app.ts`, `api.ts`, and `auth.ts`.
