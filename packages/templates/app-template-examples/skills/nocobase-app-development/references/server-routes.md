...
A collection is only grantable once the application registers it. The articles provider does this with `authz.getResource('database.collection').items.add({ name: 'articles', title: '文章 / Articles' })` before it seeds its Permission Set, and the authorization example plugin registers its task collection from a provider of its own. Registration belongs at boot, in the provider that owns the module, not in a route file: a route file builds routes. Repeating an identical registration is harmless, so a provider whose boot runs twice needs no guard around it. Skip the registration and every request is denied with `COLLECTION_NOT_REGISTERED` — including one from a root holder, since unrestricted access skips grants rather than the permission model. A registration states intent only: the name, and a title or description the permission UI shows. Field names, the primary key, and whether the database generates it come from db's own Collection metadata. `recordsIOwn` and `recordsICreated` take the column to compare as `params.field`, defaulting to `ownerId` and `createdById`.

### Repository API endpoints

When a route exposes a collection through `defineRepositoryApiRoutes()` rather than a handler of its own, `authorization.db.repositories()` authorizes it in place and registers nothing, so the collection still needs its own `add` — that is what gives it the title the permission UI shows. Each exposure names the `resource` its rows belong to and declares the static Policy shape it offers; the middleware narrows that shape with the caller's grants:
...
This is a summary. Configuring Permission Sets and binding the Repository Policy that `policyFor()` returns are covered by the authorization plugin's own Skill — read `nocobase-app-plugin-authorization` in `.agents/skills/` before building ownership rules, and its `references/orders-module.md` for a complete worked example with an `ownerId`. This application already ships `@nocobase/app-plugin-authorization-example`; open `/authorization-example` and read its `server/routes/index.ts` alongside this page.
