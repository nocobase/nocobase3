...
`server/config/database.ts` declares the dialects the application has, in the first call of `defineDatabaseConfig`. This template registers four, so that its connections can demonstrate more than one:
