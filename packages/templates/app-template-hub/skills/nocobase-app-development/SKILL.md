...
Configure the application in root `config.yml`, using `config.example.yml` as the reference. Treat `/main` in examples as a default, never as a fixed route. Determine the remote application's actual public mount path separately; local and remote paths may differ. For example, a local `/local` mount and a remote `/crm` mount map `/local/api` to `/crm/api` and `/local/ws` to `/crm/ws`.

For local client debugging against another running application, follow the remote backend development procedure in the application's README.MD. Open the printed Local URL. Client edits reload locally; backend edits require running or deploying the target separately. Requests use the target's data and permissions. Keep application configuration in `config.yml`.
