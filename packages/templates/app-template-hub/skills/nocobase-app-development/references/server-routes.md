...
Register each collection the permission model covers: `authz.getResource('database.collection').items.add({ name: 'customers', title: 'Customers' })`, usually from the provider that owns the module. An unregistered collection is denied with `COLLECTION_NOT_REGISTERED` whoever asks — unrestricted access bypasses grants, not the model — which keeps the Hub's own bookkeeping tables out of every permission list. Every collection under permissions is registered by hand from the provider that owns it — nothing infers one, and a route file builds routes rather than declaring them — and an identical repeat is tolerated, because boot runs more than once in some hosts. A registration holds a name plus an optional title and description for the permission UI; field names, the primary key, and whether the database generates it are read from db's own Collection metadata. `recordsIOwn` and `recordsICreated` take the column to compare as `params.field`, defaulting to `ownerId` and `createdById`.

### Repository API endpoints

When a route exposes a collection through `defineRepositoryApiRoutes()` rather than a handler of its own, `authorization.db.repositories()` authorizes it in place without registering anything; the collection joins the permission model through its own `add()`, which is where the title a permission list shows comes from. Each exposure names the `resource` its rows belong to and declares the static Policy shape it offers; the middleware narrows that shape with the caller's grants:
