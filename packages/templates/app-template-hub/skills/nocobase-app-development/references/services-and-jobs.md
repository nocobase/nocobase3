...
Edit `server/config/authorization.ts` for authorization options, using `AuthorizationConfig` from `@nocobase/app-plugin-authorization/server`. The plugin installs Permission Sets itself and the application configures it through `permissionSets: { rootSet: 'root', defaultSet: 'member' }`: `rootSet` names the Permission Set that confers unrestricted access and `defaultSet` the one every signed-in user holds; both are protected as the library's own. `plugins` is the optional rest this application installs — `pages()`, `defaultAccess()`, `sharingRules()`, `restrictionRules()` — so dropping a capability is deleting a line: each of those plugins serves its own HTTP surface under `/api/authz`, which the plugin mounts as one dispatcher, so a capability the application left out has no route rather than a broken one. The plugin always registers the identity step. `authorizationToken` resolves to the `Authorization` instance and `permissionSetsToken` to the Permission Sets api, which is what code that manages sets and assignments resolves. The plugin installs database authorization itself as well, reached as `authz.db`; neither built-in plugin is listed here. It ships no default list beyond those two, so leaving `plugins` out installs nothing else.
