...
Module defaults are editable under `server/config/`: `app.ts`, `auth.ts`, `caching.ts`, `database.ts`, `drive.ts`, `hub.ts`, `i18n.ts`, `logging.ts`, `queue.ts`, `server.ts`, `session.ts`, `snowflake.ts`, `spa.ts`. The client defaults are `client/config/app.ts`, `api.ts`, and `auth.ts`.
