...
4. The Appearance popover discovers registry entries automatically. Preview cards use the same CSS variables, not a second palette in JavaScript. The early HTML bootstrap loads each registered preset's matching CSS file.
...
`scripts/theme-bootstrap.ts` compiles the shared preference reader into HTML. Its module marker allows the existing SPA server to inject the runtime base before restoration. Preserve this ordering and the shared CSS source when changing startup.

## Verify

Run the theme preference, bootstrap and client-theme tests, then the App's checks. In a browser, verify refresh before React loads, all mode/preset combinations, narrow screens, keyboard selection/Escape/focus return, same-App cross-tab sync and isolation from a second App on the same origin. Test unavailable storage and a removed preset. In the source monorepo, apply template framework changes to Default and Hub together.
