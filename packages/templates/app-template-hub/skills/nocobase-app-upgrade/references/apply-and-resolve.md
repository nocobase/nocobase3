...
`.env`, `.gitignore`, `pnpm-workspace.yaml`, and `app-dist/` were written by the generator, not copied from the template. They are in neither tarball and appear in no diff. `.env` still needs attention when `.env.example` gains a key — see [the manifest, config, and plugin roots](manifest-and-config.md).
