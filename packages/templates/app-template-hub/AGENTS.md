...
`.agents/skills/nocobase-deployment/` covers moving this hub from source to a production server: building for the target platform, what the archive does and does not carry, migrations and business data, production configuration, and what to verify afterwards, including each hosted App. Read it before building for deployment, and when the hub starts in production but does not work.
...
`client/extensions/` holds the items the template preinstalled from the NocoBase UI Library, which belong to the application like the rest of its source: `PageContainer` and `PageHeader` in `nocobase-page-ui/`, `RouteDialog`, `RouteDrawer`, `RouteChildPage` and `useRouteOverlay` in `nocobase-route-overlay-ui/`, and the authentication layout and forms in `nocobase-auth-ui/`. Import them from there, for example `@/extensions/nocobase-page-ui/components/page-header`. Each directory's `README.md` lists its exports; `pnpm exec shadcn add @nocobase/<item>` adds another item from the library.
