...
For a protected page, put the same `{ resource, action }` on the Route's
`access` and the resource's `meta.access`. The sidebar keeps that resource
hidden until access is allowed, while the Route check handles direct URLs.
