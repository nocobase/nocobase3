...
const boxTypeStyles: Record<string, string> = {
  [MailBoxType.IN]:
    'border-blue-500/25 bg-blue-500/12 text-blue-600 dark:text-blue-400',
  [MailBoxType.OUT]:
    'border-green-500/25 bg-green-500/12 text-green-600 dark:text-green-400',
  [MailBoxType.DRAFT]:
    'border-cyan-500/25 bg-cyan-500/12 text-cyan-600 dark:text-cyan-400',
  [MailBoxType.SCHEDULED]:
    'border-amber-500/25 bg-amber-500/12 text-amber-600 dark:text-amber-400',
  [MailBoxType.SPAM]:
    'border-red-500/25 bg-red-500/12 text-red-600 dark:text-red-400',
  [MailBoxType.MASS]:
    'border-purple-500/25 bg-purple-500/12 text-purple-600 dark:text-purple-400',
...
const DATA_COLUMN_DEFS: Record<MailColumnId, MailColumnDef> = {
  ...
  date: columnHelper.accessor(
    (row) => row.relatedMessageLatestDate ?? row.date,
    {
      id: 'date',
      ...
      cell: (info) => (
        <span className='tabular-nums text-muted-foreground'>
          {formatDate(
            info.row.original.relatedMessageLatestDate ??
  ...
  user: columnHelper.accessor(
    (row) => row.user?.nickname ?? row.user?.email ?? '',
    {
      id: 'user',
              ...
              table.getRowModel().rows.map((row) => {
                ...
                return (
                  <TableRow
                    key={row.id}
                    data-state={
                      selectedIds.has(message.id) ? 'selected' : undefined
                    }
                    className={cn(
                      'cursor-pointer',
