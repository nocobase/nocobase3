...
| Confirm that something completed, with optional undo | `toast.tsx`        | Base UI toast, not sonner: mount `<Toaster />` once, then `toast.add({ type, title, description })`; `toast.promise` exists |
