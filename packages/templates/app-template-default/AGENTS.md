...
**Reference pages are application pages.** `client/pages/reference/examples/` holds complete business screens on mock data — orders today — and `client/pages/reference/components/` holds one page per shadcn/ui primitive showing its variants and a realistic use. Both share the frame in `client/pages/reference/shared.tsx`. An example is a folder holding its page beside the mock data that page reads — `examples/orders/orders.tsx` and `orders.data.ts` — so the screen and its records move together; the four examples whose page is still to write already have their folder and data module, and nothing outside them imports them. `client/routes.ts` declares them with `defineAppRoutes()`, grouped as `examples` and `components`, so every signed-in user reaches them through `authz: 'skip'` and a production build ships them. The set is still being filled in, and a route is declared together with the page it loads, never before it: the entries whose page is still to write stay commented out there. Read the closest example before building a page and copy its structure: `PageContainer` and `PageHeader`, `Card` grids for summaries, `DataTable` for lists, `Sheet` or `Dialog` for detail and create flows, `AlertDialog` before a destructive action, `toast` for confirmation.
