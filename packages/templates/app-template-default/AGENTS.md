...
**Reference pages live under `/dev`.** `client/pages/dev/examples/` holds complete business screens on mock data — orders today — and `client/pages/dev/components/` holds one page per shadcn/ui primitive showing its variants and a realistic use. Both share the frame in `client/pages/dev/shared.tsx`. The reference set is still being filled in, so `client/routes.ts` declares a dev route only together with the page it loads and the entries whose page is still to write stay commented out there. Read the closest example before building a page and copy its structure: `PageContainer` and `PageHeader`, `Card` grids for summaries, `DataTable` for lists, `Sheet` or `Dialog` for detail and create flows, `AlertDialog` before a destructive action, `toast` for confirmation. They are declared with `defineDevRoutes()` in `client/routes.ts`, so a production build drops them; do not import from them in product code.
