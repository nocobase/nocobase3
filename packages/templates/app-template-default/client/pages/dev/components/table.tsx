...
export default function TableExamplePage(): ReactElement {
  ...
  const invoiceStatusLabel: Record<InvoiceStatus, string> = {
    paid: t('devCommon.statusPaid'),
    pending: t('devCommon.statusPending'),
    failed: t('devCommon.statusFailed'),
  };
  const lineStatusLabel: Record<LineStatus, string> = {
    shipped: t('devCommon.statusShipped'),
    processing: t('devCommon.statusProcessing'),
    pending: t('devCommon.statusPending'),
  };
  ...
  return (
    <ExamplePage
      title={t('devComponents.table.title')}
      description={t('devComponents.table.description')}
      source='client/pages/dev/components/table.tsx'
      docs='https://ui.shadcn.com/docs/components/table'
    >
      <ExampleSection
        title={t('devComponents.table.invoices')}
        description={t('devComponents.table.invoicesDescription')}
        contentClassName='block'
      >
        <Table>
          <TableCaption>
            {t('devComponents.table.invoicesCaption')}
          </TableCaption>
          <TableHeader>
            <TableRow>
              <TableHead className='w-36'>
                {t('devComponents.table.invoice')}
              </TableHead>
              <TableHead>{t('devCommon.customer')}</TableHead>
              <TableHead>{t('devCommon.status')}</TableHead>
              <TableHead>{t('devComponents.table.paymentMethod')}</TableHead>
          ...
          <TableFooter>
            <TableRow>
              <TableCell colSpan={4}>{t('devCommon.total')}</TableCell>
              <TableCell className='text-right tabular-nums'>
      ...
      <ExampleSection
        title={t('devComponents.table.compact')}
        description={t('devComponents.table.compactDescription')}
        contentClassName='block'
      >
      ...
      <ExampleSection
        title={t('devComponents.table.selectable')}
        description={t('devComponents.table.selectableDescription')}
        contentClassName='block'
      >
        <div className='flex flex-col gap-3'>
          <div className='flex items-center justify-between gap-4 text-sm text-muted-foreground'>
            <span>
              {t('devComponents.table.selectedCount', {
            ...
            <TableHeader>
              <TableRow>
                <TableHead className='w-10'>
                  <Checkbox
                    aria-label={t('devComponents.table.selectAll')}
                    checked={allSelected}
                ...
                <TableHead className='w-12'>
                  <span className='sr-only'>{t('devCommon.actions')}</span>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
      ...
      <ExampleSection
        title={t('devComponents.table.inCard')}
        description={t('devComponents.table.inCardDescription')}
        contentClassName='block'
      >
        <Card>
          <CardHeader>
            <CardTitle>{t('devComponents.table.recentOrders')}</CardTitle>
            <CardDescription>
              {t('devComponents.table.recentOrdersDescription')}
            </CardDescription>
            <CardAction>
          ...
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t('devComponents.table.order')}</TableHead>
                  <TableHead>{t('devCommon.customer')}</TableHead>
                  <TableHead>{t('devCommon.date')}</TableHead>
                  <TableHead>{t('devCommon.status')}</TableHead>
                  <TableHead className='text-right'>
                    {t('devCommon.amount')}
                  </TableHead>
