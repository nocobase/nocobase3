...
- `pages/reference/` holds worked examples of application pages: `examples/` are complete business screens on mock data, `components/` shows one shadcn/ui primitive per page, and `shared.tsx` is the frame both use. An example is a folder holding its page beside the data that page reads, as `examples/orders/orders.tsx` and `orders.data.ts`; the examples whose page is still to write hold their data module already. Copy their structure when building a page. They are ordinary application routes under the `examples` and `components` groups, so every signed-in user reaches them and a production build ships them. The reference set is still being filled in, so `routes.ts` declares one route together with its page and the pages still to write have their route commented out.
...
Desktop sidebar collapse state is shared by App, Settings and Dev layouts through `useSidebarPreference`, persisted under the fixed LocalStorage key `nocobase:sidebar:collapsed` across applications on the same origin. Mobile sidebar visibility remains local to each layout.
