...
- `pages/dev/` holds the reference pages behind the Dev entry: `examples/` are complete business screens on mock data, `components/` shows one shadcn/ui primitive per page, and `shared.tsx` is the frame both use. Copy their structure when building a page; they are dev-only routes and never reach a production build. The reference set is still being filled in, so `routes.ts` declares a dev route only together with its page and the pages still to write have their route commented out.
