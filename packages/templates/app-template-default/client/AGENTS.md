...
- `access` on an app route names the resource and action checked before the page loads; omitting it authorizes the page as `resource: <route name>, action: 'access'`. `access: false` says the opposite: being signed in is enough, every signed-in user reaches the page, and no permission change can take it away. Use it only for a page that must never be lockable — the landing page a signed-in user has to land on. It is not a way to postpone adding permissions to a page: a page that should be restricted later has to be restricted now, because removing `access: false` takes the page away from everyone who has no grant for it.
