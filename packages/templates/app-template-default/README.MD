...
The Default template enables the Hub publishing commands with `"nocobase": { "cli": { "publishing": true } }` in `package.json`, which registers `release upload` and `release deploy`. Run them from the App root.
