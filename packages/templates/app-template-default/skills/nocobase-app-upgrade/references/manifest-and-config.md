...
`config.yml` holds the application's real settings including generated secrets. It is gitignored, it was written by the generator rather than copied from the template, and it appears in no delta. Never edit it as part of the merge, and never print its contents.

`config.example.yml` does ship with the template and is merged like any other file — and a new key there is the only signal that `config.yml` needs a corresponding edit:

```bash
diff "$WORK/$BASE/config.example.yml" "$WORK/$TARGET/config.example.yml"
```

For every key the target added, check whether the running `config.yml` needs it. A new key with a working default needs nothing. A new key with no default is a startup failure waiting for the first restart — tell the user what to add and where, and let them edit the file, since it holds secrets they may not want read back.

The hub template is configured through `.env` rather than `config.yml`, with `.env.example` in the same role.
...
A template release can ship a migration under `database/`. Copy it in like any other added file, then run it:
