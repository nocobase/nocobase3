...
Run the owning application's `pnpm nocobase db apply`, which migrates and then seeds. The seed runner records completion, so later runs skip it and retain user edits and deletions. Once the migration has been merged and published, subsequent schema changes require a new migration.
