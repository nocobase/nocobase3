...
- Relationship-write page: `/repository-example/relation-mutations`, in the Repository examples group. Seven independent cards each provide preparation, a form, a target table, a request preview and the last executed response for create/connect/disconnect/set/update/upsert/delete. Choose hasOne profile, hasMany tasks or belongsToMany tags; set is to-many only (20 supported combinations). Preparation creates a fresh project and four isolated targets, requires migrations but no seeds, and retains old records when repeated. Page load performs no writes. Target tables distinguish association state from record existence, retain absent rows after delete and show through.role. Tag create/connect/set expose a role input. Upsert can update the same target on repeated execution; set checkboxes may all be cleared. Each root HTTP mutation is transactional; multi-request preparation is not. Write failures preserve inputs; refresh failures after a successful write require a read-only table refresh. The page calls only `api.repository(name)` through authenticated routes. See `client/relation-lab.ts`.
...
- Select combine page: `/repository-example/select-combine` in Repository examples. Five read-only cards run real `findMany` JSON ASTs: order preview/count/status branches, item records and COUNT/SUM/AVG/MIN/MAX, nested orders/items/product includes, shared draft-task filtering plus branch-local null filtering, and many-to-many tag records/counts. Requests are visible before execution; results show field-path columns and nested tables for record branches, with raw JSON in a collapsible panel. NULL, zero and empty relations are displayed distinctly. Queries show the first 10 roots by ID; branch limits apply per parent. Empty relations retain empty lists and zero counts, with null non-count aggregates. Reuses existing CRM and relationship seeds. Definitions live in `client/select-combine.ts`.
- Sort page: `/repository-example/sort` under Repository examples. Fifteen read-only cards cover default/ascending/descending/multi-field order, automatic primary-key ties, NULL placement, to-one paths, count/sum/avg/min/max relation sorting, independent include order, and two expected INVALID_SORT cases (duplicate targets and direct to-many paths). Builder snippets and actual serialized JSON AST requests accompany response-order tables and raw JSON. Relation aggregates are selected explicitly; filtered projections do not change sort scope. Empty sort SUM is 0 while selected SUM is NULL. Reuses existing seeds, with at most 10 root records. See `client/sort.ts`.
