...
Declare each identity-sensitive runtime above as a `peerDependency`. One declaration is enough: pnpm links a `workspace:^` peer to this repository's copy for development. `pnpm peers:check` enforces this.
