...
import { nocobaseClient, NocoBaseHttpError } from '../src/client/index.ts';
...
describe('Portal access denial', () => {
  it('retains only a structured Portal access denial in ACL state', async () => {
    ...
    vi.spyOn(nocobaseClient, 'request').mockRejectedValue(portalDeniedError());
  ...
  it.each(['users:setDefaultRole', 'auth:syncCookies'])(
    ...
    async (failedAction) => {
      ...
      if (failedAction === 'auth:syncCookies') {
        expect(
          actions.filter((action) => action === failedAction),
        ).toHaveLength(2);
      }
    },
  );
