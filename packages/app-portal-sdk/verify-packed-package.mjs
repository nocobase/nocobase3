...
const packageRoot = process.argv[2] ? path.resolve(process.argv[2]) : undefined;
...
if (
  JSON.stringify(workspaceEntryPoints) !== JSON.stringify(packedEntryPoints)
) {
  throw new Error('Workspace and packed Portal SDK entry points do not match.');
}
...
for (const [entryPoint, conditions] of Object.entries(exportsMap)) {
  if (!conditions || typeof conditions !== 'object') {
    throw new Error(
      `Packed export ${entryPoint} must declare import and types.`,
    );
  }
