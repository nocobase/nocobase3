...
export interface AppWebSocketEvents {
  ...
  onPong?: (event: AppWebSocketPongEvent, ws: AppWebSocket) => void | Promise<void>;
}
...
class NodeAppWebSocket implements AppWebSocket {
  ...
  private handleFrame(frame: ParsedWebSocketFrame): void {
    ...
    if (frame.opcode === 0xA) {
      this.invoke(this.events.onPong, { type: 'pong', data: bufferToArrayBuffer(frame.payload) }, this);
      return;
    }
  ...
  private invoke<TEvent extends AppWebSocketOpenEvent | AppWebSocketMessageEvent | AppWebSocketCloseEvent | AppWebSocketErrorEvent | AppWebSocketPongEvent>(
