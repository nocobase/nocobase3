...
4. **Merge the translations into the plugin's own locales.** Under a plugin's route, keys resolve in the plugin's namespace first and fall back to the application's, so spread the item's `locales/` files into the plugin's `client/locales/` rather than relying on the application to have the keys. The files compile under the plugin's declaration build as they are. A plugin without `client/locales/` yet sets them up as the [plugin i18n reference](../.agents/skills/nocobase-plugin-development/references/i18n.md) describes.
