...
│   └── page/                   page-ui and route-overlay-ui, which the templates preinstall
...
│   └── demo/<group>/<item>/    each item's demo routes and the mocks they run against
...
The three application templates ship installed copies of some items under `client/extensions/nocobase-<item>/`, and this library is the source of truth for all of them. `page-ui` and `route-overlay-ui` are preinstalled because every template page is built from them: each template carries exactly the files the item installs, plus its README, and `tests/scripts/template-ui-library.test.mjs` fails until a change to either item has been copied into all three templates. That part of the pull request changes published packages, so it needs a changeset. The templates' `auth-ui` copies diverged before that check existed and are due to be removed; until they are, a change the templates should also get has to be carried into them by hand.
