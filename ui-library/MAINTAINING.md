...
│   └── demo/<group>/           each item's demo routes and the mocks they run against
...
- **Relative imports reach only files the item installs.** Only the item's own files are installed, each at its `target`, so a path to anything else points at nothing in the consumer. Keep every source file in the same position relative to the others as its `target`: a block's files sit in the block's directory, and the components sit side by side in `registry/components/`, as they do in `client/components/`, which is also what lets two components install one shared file.
- **Import shadcn primitives as `@/components/ui/<name>` and the `cn` helper from `@/lib/utils`, and list them in `registryDependencies`.** These are the only `@/` imports an item may use. shadcn installs each listed primitive into the consumer's own `components/ui/`, and the `utils` dependency provides `lib/utils.ts` to a project that has none, so both aliases resolve. Merge class names with `cn`, as the consumer's own components do, rather than importing `clsx` or `tailwind-merge` directly. Never copy a primitive into an item. The primitive also has to exist under `website/components/ui/` for the item to type-check here; add a missing one with `pnpm exec shadcn add <name>` from `ui-library/` and review what it changes.
...
- **List a component's keys in its README.** A component ships no locale files, since `client/components/` has no place for them. Name each key it looks up in `docs`, list it with its English and Chinese wording in the group's README, and keep such keys few; the consumer adds them to its own locale resources.
...
- **Write `docs` and the README for different readers.** shadcn prints the item's `docs` after installing it, but does not install the README, which is not listed in `files`. Put what a consumer needs before using the item in `docs`, in a sentence or two. The README — one per block, and one for the whole group of components — documents the item in this repository: its entry points, prerequisites, how to merge its translations, and what a consumer is expected to customize.
