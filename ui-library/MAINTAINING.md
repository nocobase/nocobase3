...
- **Style with the shared theme tokens.** Use the utilities in [theme.md](../packages/app/app-skills/skills/nocobase-app-development/references/frontend/theme.md) so the item follows the consumer's theme. `website/styles.css` loads the default template's theme, so the preview renders the tokens the way an application does.
