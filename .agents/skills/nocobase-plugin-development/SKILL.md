...
- Pages with Tabs use child routes by default: declare children under the parent route, render `Outlet`, derive selection from the URL, and redirect the parent to the default accessible Tab with replace while preserving query parameters. Respect an explicit user request for another interaction.
