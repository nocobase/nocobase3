...
本文是规范设计稿。业务弹窗使用子路由、ID 可选、嵌套路径命名、关闭返回父路由，以及模板提供 Dialog 和 Drawer 的方向已确认。具体路由声明 API、组件 API 和注册结果结构仍需审核，本文中的拟议接口不代表当前已经可用。
...
业务组件负责标题、内容、表单、数据加载和操作结果。默认弹窗组件负责呈现与基础交互，路由衔接负责打开和关闭。具体通过组件组合还是单独的路由包装实现，待组件 API 审核确定。
...
自定义样式不改变 URL、父子关系、关闭目标和键盘交互。只有需求明确影响整个应用时，才修改共享默认样式。组件导出名、属性、尺寸选项和样式定制入口在实现前单独确认，不在本文中预设一套新的配置协议。
...
6. 默认组件的导出名、属性、关闭入口、历史写入策略及查询参数保留规则。
...
| `client/components/dialog.tsx`、`client/components/drawer.tsx`（拟新增） | 默认业务弹窗组件 |
...
| `client/pages/<feature>/dialogs/<dialog>.tsx` | 实现业务内容，复用默认 Dialog 或 Drawer，默认导出路由组件 |
...
4. 使用默认 Dialog 或 Drawer 实现内容，并按标准放置子路由出口。
