---
title: App 页面子路由规范
description: 应用与插件页面子路由的路径、Outlet、Tab、弹窗与 AI 开发规范
---
...
本次实现覆盖三类页面路由的递归子路由、导航分组、树结构解析和路由驱动菜单。App、Settings、Dev 都在路由中声明 navigation，App 菜单不再由 Refine resources 生成。组件覆盖递归定位 App 页面，Inspector 遍历全部层级。

页面由 AI 手动放置 Outlet；导航分组由路由层透传。分组可递归，path 可选；页面可同时是菜单链接和下级菜单的父节点。无 navigation 的页面仍可访问。路径由 AI 按业务自行设计，不支持跨所有者追加子路由。

下文 RouteDialog、RouteDrawer 及其关闭 Hook 是后续设计提案，本次不实现。它们计划由模板和插件各自维护本地实现，不由核心提供 UI 导出。

## 目标与范围

本规范覆盖所有页面子路由，包括业务弹窗、页面 Tab 和普通子页面。AI 在所属父路由下声明子路由，手动放置 `Outlet`，根据呈现方式选择组件。URL 表达当前页面内容和嵌套层级，支持链接直达、刷新恢复与浏览器前进后退。

后续弹窗方案计划由 Default、Examples 和 Hub 三套模板提供本地通用路由弹窗组件，插件也可维护相同的本地实现，统一样式和基础交互。AI 默认复用，用户要求定制时可以调整样式，但仍遵循同一套路由和关闭规则。
...
路径由 AI 根据业务需求和项目现有约定自行设计，不限定“名称 + 可选 ID”、路径段数量、参数数量或大小写风格，不需要为不同路径形态申请例外。路径格式不作为核心校验或 AI 验收标准。

路由声明与导航链接必须匹配，使用路由 API 支持的语法并避免冲突。取消命名限制不代表已经新增 index、通配等尚未确定的 API 能力。

所有场景共用递归 `children`、相对子路径、树结构解析和父级权限约束，不另加 `tab`、`dialog` 路由类型字段。由业务组件决定呈现方式：

| 场景 | 组件 | 手动 Outlet 位置 |
| --- | --- | --- |
| 弹窗 | 本地 `RouteDialog` / `RouteDrawer` | 父内容之后，嵌套时放在父包装之外 |
| 页面 Tab | React Router `NavLink` 或已有路由 Tab 组件 | 导航下方的内容区域 |
| 子页面 | 现有页面布局与导航组件 | 父布局的可切换内容区域 |

Tab 的选中态从路由得到，不用独立 `activeTab` 再控制同一份内容。Tab 切换和普通子页面导航不套用弹窗关闭时的 `replace` 规则，查询参数按业务归属决定。默认 Tab 及父路径的展示方式由页面明确，涉及 index 路由或默认重定向的 API 仍需确认。

模板现有组件清单不应被当作已经包含 Tabs；可直接使用 `NavLink` 实现页面导航。需要新增 Tabs 组件时沿用本地 UI 规范，并验证路由同步、键盘和焦点行为。普通子页面不需要弹窗包装。
...
以下展示一种可行的业务路径组织，AI 可以根据需求采用其他写法，不必套用名称与 ID 成对的格式。
...
参数按业务需要设置，确保组件能够准确读取所需值，避免参数覆盖造成歧义。导航入口应与所声明路径保持一致。
...
后续计划在三套模板中均提供本地 `RouteDialog` 和 `RouteDrawer`，需要弹窗的插件提供一致的本地实现。Dialog 用于居中展示的业务内容，Drawer 用于侧边展开的业务内容；二者使用相同的路由规则，可以嵌套组合。
...
App 页面声明增加 navigation 和 children。三种路由都区分页面与导航分组：页面有 componentLoader，分组没有，不能仅根据 children 判定。分组 path 可选，支持递归组和可点击页面下的菜单。

解析结果保留树结构；Settings、Dev 的旧页面/分组列表由树派生，用于既有工具消费。路由渲染、菜单和父链权限以树为准。App 没有页面祖先的页面保持默认 name/access 检查，页面子节点仅增加显式 access；Settings、Dev 保持显式权限约定。

页面入口按声明生成菜单，翻译归所属包命名空间。Refine resources 保留给 CRUD 等功能。动态参数或通配符不是可直接点击的菜单目标，不能配置 navigation；这属于菜单可用性校验，不规定 URL 命名格式。
...
`RouteDialog` 和 `RouteDrawer` 计划放在三套模板自己的 `client/components/` 中，插件按需在自己的相同目录提供本地实现。应用代码导入应用组件，插件代码导入插件组件，不从消费应用的 `@/components` 或模板源码导入。Registry 安装到应用的源码由应用拥有，使用应用的本地组件。
...
开发指引提供统一的完整参考源码和一致性验证步骤，创建或修复标准实现时同步三套模板及已采用它的插件。参考源码的存放位置及检查方式仍需确定，不先引入新的代码同步生成器。

### 权限与验证

父路由必须通过认证和权限检查后才渲染页面及其 `Outlet`，因此父页面不可访问时，其弹窗也不渲染，直接访问深层 URL 同样如此。子路由如声明额外 `access`，需在父级允许的基础上继续检查，不能替代或放宽父级限制。子路由继承父链的登录模式，解析时拒绝不同的 auth 声明。Client 权限检查不替代 Server API 的独立认证和授权。子弹窗的记录参数还需经过相应业务校验，不能只因父记录可访问就假定子记录也可访问。
...
子路径按示例使用相对路径，不重复父路径。沿用原有路由 ID 生成及重复检查范围；解析树保留每个节点的归属、权限和 loader，现有路由覆盖、冲突检查及 Inspector 改为遍历树，不能只检查顶层节点。

`defineSettingsRoutes()` 同样支持页面子路由。弹窗子路由不配置 `navigation`，不进入设置导航；设置页面或现有导航分组仍按原有方式配置。设置导航分组与页面子路由是不同概念，不能继续仅以存在 `children` 判断是否为导航分组。页面与分组通过 componentLoader 区分，分组支持递归且 path 可选。

开发工具页面与设置页面共用递归类型，整棵开发路由子树在生产环境移除。

声明采用父路由下的相对路径，下面展示已支持的递归结构：
...
通用路由已实现递归声明、树解析、分组判定及父链认证。后续弹窗阶段仍需审核：

1. 本文组件属性和 `useRouteOverlayClose()` 提案，以及默认尺寸、遮罩关闭策略。
2. 路由模块加载失败或子级权限被拒绝、包装组件尚未挂载时的可关闭提示方案。
3. 弹窗本地标准实现的参考源码位置、一致性检查方式和底层 UI 依赖。
...
以下模板相对路径同时适用于 `packages/templates/app-template-default/`、`packages/templates/app-template-examples/` 和 `packages/templates/app-template-hub/`。新增文件的名称是建议值，待 API 确认后固定。
...
Skills 的总入口调整为“页面子路由”，先讲路径设计由 AI 自主决定，以及统一的声明、权限和 `Outlet`，再按 Tab、普通子页面和弹窗选择组件。补充 `references/client-child-routes.md` 作为通用指引，现有弹窗参考作为其场景专篇，不将弹窗的关闭、查询参数和包装规则套用到所有子路由。

Tab 示例覆盖固定内容和带参数的内容，演示路由选中态、导航与内容出口；子页面示例演示父布局保持和局部内容切换。应用与插件均提供对应文件清单。日常修改集中在所属路由声明、父布局/导航、子内容组件、翻译和测试；只有弹窗才需要路由包装组件。

后续弹窗操作规范计划放在三套模板自带的 `references/client-dialog-routes.md`，随生成应用交付。`SKILL.md` 的任务索引加入“新增或修改业务弹窗”，已有页面路由文档和 Agent 入口链接到同一正文，使不同 AI 通过现有开发入口发现规则。

规范应按固定步骤写明：

1. 判断是短暂提示还是业务弹窗。
2. 找到直接父路由，根据业务需求设计路径和参数。
...
每个示例必须给出完整文件路径、可用导入、完整组件和对应路由声明。至少覆盖无 ID 的新增、带 ID 的详情或编辑，以及订单详情中的商品详情、客户详情。路径示例只说明用法，不构成命名限制；AI 自行设计业务路径，源码文件沿用项目组织方式。

只有在 API 已实现并通过验证后，才将拟议代码写成 Skills 中的可执行指令。三套模板提供相同框架规范，各自保留产品内容和身份。
...
| 验证项 | 预期结果 |
| --- | --- |
| 页面 Tab | 选中态与 URL 一致，业务路径和参数解析正确，直达刷新可恢复，导航与键盘行为正常 |
| 普通子页面 | 父布局保留，内容在指定 Outlet 区域切换，权限和前进后退正确 |
| 新增弹窗 | 无 ID 路径可打开和关闭 |
...
实现阶段对三套模板分别运行 `pnpm --filter <模板包名> check`。修改核心时，对 `@nocobase/app-client` 及受影响消费方运行 `lint`、`typecheck`、`test` 和 `build`；通过浏览器验证焦点、遮罩、滚动和嵌套交互。涉及发布产物时按仓库规则补充 changeset。
