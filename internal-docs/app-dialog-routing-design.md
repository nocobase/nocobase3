...
本文是规范设计稿。业务弹窗使用子路由、ID 可选、嵌套路径命名、关闭返回父路由，以及模板提供 Dialog 和 Drawer 的方向已确认。默认业务弹窗确定采用独立的路由包装方式，Dialog 和 Drawer 包装组件分别暂定名为 `RouteDialog` 和 `RouteDrawer`。这两个通用组件暂定由 `@nocobase/app-client` 提供，应用和插件共用。具体公共导出路径、路由声明 API、组件属性和注册结果结构仍需审核，本文中的拟议接口不代表当前已经可用。
...
`@nocobase/app-client` 提供通用路由弹窗组件，两套模板默认接入，统一样式和基础交互。AI 默认复用，用户要求定制时可以调整样式，但仍遵循同一套路由和关闭规则。
...
两套模板均接入 `@nocobase/app-client` 提供的 `RouteDialog` 和 `RouteDrawer`。Dialog 用于居中展示的业务内容，Drawer 用于侧边展开的业务内容；二者使用相同的路由规则，可以嵌套组合。

共享组件统一维护以下默认行为和样式，模板通过已有主题 Token 提供应用外观：
...
两种包装组件遵循相同的路由与关闭规则。包装组件的具体属性、业务取消与提交成功后的关闭调用方式、子路由出口位置，以及加载和错误外壳的组合方式仍需审核。选择路由包装不代表新增公开 Hook、Context 或注入协议，公共导出路径仍需确认，应用和插件通过同一公开入口使用组件。
...
`RouteDialog` 和 `RouteDrawer` 暂定由 `@nocobase/app-client` 提供公共导出，应用与插件复用同一实现。模板负责默认接入和主题 Token，不保留一份独立的路由弹窗实现；插件也不复制模板组件，或从消费应用的 `@/components` 导入。

组件只依赖通用路由、UI 基础能力和主题约定，不依赖模板私有模块或业务插件内容。底层 UI 依赖、样式产物的交付方式、公共导出路径和组件属性需要在实现前审核；确定包归属不代表这些接口已经确定。

现有[插件组件规范](development/plugin-development/client-components.md)要求普通 UI primitives 由插件本地维护。实现本方案时，应明确补充路由弹窗使用 `@nocobase/app-client` 公共组件的规则，避免 AI 继续生成插件本地路由包装；普通 UI 的源码所有权规则保持适用。不能直接假设已有 `@nocobase/app-client/ui` 等导出可用。

插件沿用 `@nocobase/app-client` 的 peerDependency 与 devDependency 声明，避免引入第二份应用运行时。最终开发 Skills 必须给出经验证的公共导入及样式接入步骤，应用和插件均使用这一默认路径。
...
建议优先评审第一种方案。`@nocobase/app-client` 提供通用父子路由能力和路由弹窗组件；模板负责应用装配与主题，业务内容留在应用或插件中，不向核心添加业务插件依赖。
...
7. `@nocobase/app-client` 的组件公共导出路径、底层 UI 依赖和样式交付方式，以及应用与插件共用路由协议时的所有权和覆盖行为。跨所有者追加子路由不隐式包含在本次扩展中。
...
| `client/styles.css`、主题文件及构建配置（按需） | 接入共享组件样式与已有主题 Token，不复制路由包装实现 |
| `client/locales/` | 应用弹窗文案；共享组件默认文案的归属随组件 API 确认 |
...
共享组件暂定归属 `@nocobase/app-client`；结合待审核的子路由声明方案，核心候选范围为：

| 文件或目录 | 拟议变化 |
| --- | --- |
| `packages/app/app-client/src/` 下路由弹窗组件文件（具体目录待定） | 实现 `RouteDialog`、`RouteDrawer` 及所需内部 UI，统一关闭导航 |
| `packages/app/app-client/package.json`、对应公共导出入口 | 审核后的组件 exports、依赖及样式发布配置 |
| `packages/app/app-client/tests/` 下组件测试 | 默认交互、嵌套、关闭与主题接入验证 |
...
| `client/pages/` 中对应业务弹窗组件 | 实现业务内容，通过 `@nocobase/app-client` 审核后的公共入口复用路由弹窗组件 |
...
| 插件组件边界 | 应用和插件通过 `@nocobase/app-client` 公共入口复用组件，不依赖模板私有路径；发布产物可正常解析，默认样式及混合嵌套在目标应用中正常 |
