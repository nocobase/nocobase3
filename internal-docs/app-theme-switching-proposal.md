...
AI 通过修改主题 CSS 创建不同的视觉风格。用户选择完整主题，不需要分别设置字体、字号或颜色。模板提供 Default、Compact 和 Ant Design 三个主题。Compact 仅调整间距、行高和圆角等尺寸；颜色、字体和阴影与 Default 保持一致。Ant Design 是基于现有组件的风格预设，采用蓝色强调色、中性背景、14px 基准正文、6px 基准圆角和轻阴影，支持亮暗模式，不引入 Ant Design 组件库。
...
点击立即应用并保存，无需提交，面板保持打开以便比较。主题与显示模式互不改变。预览跟随实际亮暗模式，窄屏时限制面板宽度。当前未设置面板最大高度和内部滚动，增加较多主题时仍需处理纵向溢出。
...
│   ├── theme-context.ts        主题预设上下文与读取 Hook
│   ├── theme-preferences.ts    存储键、偏好读取与启动恢复
│   ├── theme-provider.tsx      亮暗模式与主题状态
│   ├── theme-settings.tsx      外观面板
│   └── themes/
│       ├── default.css        默认主题 Token
│       ├── compact.css        紧凑主题 Token
│       └── ant-design.css     Ant Design 风格主题 Token
└── locales/                   主题名称与界面翻译
```
...
两套模板均已接入完整主题 Token。默认主题保留原有 18 个通用颜色值和基础圆角值，字体、字号、间距和阴影沿用应用或 Tailwind 默认值；其他预设通过各自的 CSS 定义差异。
...
圆角已从加减固定像素改为统一比例，并补齐大圆角档位。侧边栏选中项由独立的背景与文字 Token 控制，组件不再固定计算主色的 10%；默认和紧凑主题仍在 CSS 中使用主色与侧边栏背景按 10% 混色，其他主题可以独立配色。

后续维护继续同步两套模板，并按下述要求验证行为与视觉效果。涉及新的公共配置、props 或扩展点时，先审核再实现。

## 验证与验收

- **完整性**：所有主题包含完整 Token，亮暗差异和预览作用域正确；不固定断言品牌颜色字面量。
- **CSS 编译**：最小样例覆盖标准字体、全部字号及行高、间距、七档圆角和盒阴影，确认工具类保留运行时变量引用。
- **浏览器效果**：切换 Default / Compact / Ant Design 与 Light / Dark，检查根节点、预览及 Portal 的实际样式。测试用变量值要有明显差异，避免“切换成功但样式没变”。
